...
<p align="center">
  ...
  <strong><a href="https://openpaths.io">openpaths.io</a></strong>
</p>

<p align="center">
  <a href="https://www.npmjs.com/package/@openpaths/coding-agent"><img src="https://img.shields.io/npm/v/@openpaths/coding-agent?style=flat&colorA=222222&colorB=CB3837" alt="npm version"></a>
  <a href="https://github.com/lee101/op/blob/main/packages/coding-agent/CHANGELOG.md"><img src="https://img.shields.io/badge/changelog-keep-E05735?style=flat&colorA=222222" alt="Changelog"></a>
  <a href="https://github.com/lee101/op/actions"><img src="https://img.shields.io/github/actions/workflow/status/lee101/op/ci.yml?style=flat&colorA=222222&colorB=3FB950" alt="CI"></a>
  <a href="https://github.com/lee101/op/blob/main/LICENSE"><img src="https://img.shields.io/github/license/lee101/op?style=flat&colorA=222222&colorB=58A6FF" alt="License"></a>
...
<p align="center">
  <strong>op</strong> — the OpenPaths agent. Fork of <a href="https://github.com/can1357/oh-my-pi">oh-my-pi</a> by <a href="https://github.com/can1357">@can1357</a>, itself a fork of <a href="https://github.com/badlogic/pi-mono">Pi</a> by <a href="https://github.com/mariozechner">@mariozechner</a> 
</p>
...
```sh
curl -fsSL https://openpaths.io/install | sh
```
...
`op` generates its own completion scripts for **bash**, **zsh**, and **fish** from the live command/flag metadata, so they never drift from the actual CLI. Subcommands, flags, and enum values complete statically; model names (`--model`, `--smol`, `--slow`, `--plan`) resolve against the bundled model catalog and `--resume` against your on-disk sessions.

```sh
# zsh — add to ~/.zshrc (or write the output into a file on your $fpath)
eval "$(op completions zsh)"

# bash — add to ~/.bashrc
eval "$(op completions bash)"

# fish
op completions fish > ~/.config/fish/completions/op.fish
```

## Every tool, _benchmaxxed_.

Edits that land on the first attempt. Reads that summarize files instead of dumping their content. Searches that return instantly. Pick any model — op will get it right.
...
Originally built on [Mario Zechner](https://github.com/mariozechner)'s wonderful [Pi](https://github.com/badlogic/pi-mono), op adds everything you're missing.
...
![op TUI: a single eval session with `[1/2] pandas describe` (Python) printing a real DataFrame.describe() table, followed by `[2/2] top scorer` (JavaScript) running a reduce. Footer: 'Both kernels ran in one session.'](https://omp.sh/captures/eval.webp)

### 02 · LSP wired into every write

Ask for a rename and you get a rename. The call goes through workspace/willRenameFiles, so re-exports, barrel files, and aliased imports update before the file moves. Everything your IDE knows, the agent knows.

![op TUI: `LSP references` returns five hits across three files for the symbol `formatBytes`, then `LSP rename` applies the change with edits to format.ts/report.ts/cli.ts, then a `Search formatBytes 0 matches` confirmation. Final line: 'Rename complete. Five edits across three files…'.](https://omp.sh/captures/lsp.webp)
...
![op TUI: /advisor status shows the advisor running on openai-codex/gpt-5.5; after the main agent scopes a catch to ENOENT instead of swallowing every error, an amber 'Advisor 1 note (concern)' card warns the fix no longer matches the user's literal acceptance criterion.](https://omp.sh/clips/advisor-poster.webp)

_[Watch the capture ↗](https://omp.sh/clips/advisor.mp4)_

### 07 · Hand someone the link, they're in.

/collab puts your live session on a relay and hands back a link — and a QR. A teammate joins from another terminal with op join, or just opens it in a browser. Share read-write to pair on the same agent, or /collab view for a read-only link anyone can watch but no one can steer. Frames are sealed client-side; the relay never sees your keys.

![op TUI: /collab view prints 'Collab session started!' with an op join command, a my.openpaths.io browser link, the note 'Anyone with this link can watch the session but cannot prompt the agent', and a large scannable QR code.](https://omp.sh/clips/collab-poster.webp)
...
![op TUI: web_search returns 10 ranked Perplexity sources for inference-time compute scaling, the agent picks an arxiv paper, calls read https://arxiv.org/pdf/2604.10739v1, and summarizes the paper's headline result with real numbers.](https://omp.sh/clips/web-poster.webp)

_[Watch the capture ↗](https://omp.sh/clips/web.mp4)_

### 09 · Unapologetically native. Even on Windows.

Other agents shell out to rg, grep, find, and bash. On many machines those binaries don't exist, and on the ones where they do, every call costs a fork-exec round-trip. op links the real implementations into the process. ripgrep, glob, find: in-process. brush is the bash — with sessions that survive across calls, and 58 command-line utilities (ls, sed, sort, xargs, even jq) ported into the builtins crate and run in-process, zero fork/exec. The same op binary runs on macOS, Linux, and Windows — no WSL bridge.
...
![op TUI reading pr://lee101/op/1063 and then /diff/1, showing hunk headers, added lines, and a [MODIFIED] (+12 -0) summary.](https://omp.sh/captures/pr.webp)
...
![op TUI: ✓ Read src/session.ts (⚠ 1 conflict), then ✓ Write conflict://1 · 1 line with content @theirs, then a confirmation 'Resolved.'](https://omp.sh/clips/conflict-poster.webp)
...
![op TUI: ✓ AST Edit: console.log($X) (proposed) 3 replacements · 1 file, then ✓ Accept: 3 replacements in 1 file (AST Edit), followed by 'Applied 3 replacements in src/auth.ts.'](https://omp.sh/clips/codemod-poster.webp)
...
![op TUI driving the browser tool against DuckDuckGo](https://omp.sh/captures/browser.webp)
...
[Full reference →](https://openpaths.io/docs/tools)
...
Define custom providers in `~/.op/agent/models.yml`:
...
Run `op models spark` to verify discovery. Then run `op setup` and choose the model in the default-model step, or open `/model` in a session and assign it to the `default` role.

To preconfigure the default without the picker, add the selector to `~/.op/agent/config.yml`:
...
- **Custom providers** — Declare anything that speaks `openai-completions`, `openai-responses`, `openai-codex-responses`, `azure-openai-responses`, `anthropic-messages`, `bedrock-converse-stream`, `google-generative-ai`, `google-gemini-cli`, or `google-vertex` in `~/.op/agent/models.yml`.
...
Full provider & routing reference at [openpaths.io/docs/providers](https://openpaths.io/docs/providers).
...
[`web_search` reference ↗](https://openpaths.io/docs/tools#web_search)
...
Same engine, four wrappers. `op` runs the TUI. `op -p` answers a single prompt and exits. The Node SDK embeds the session in your process. `op --mode rpc` and `op acp` hand the wheel to another program over stdio.
...
![op TUI: the ask tool renders an option picker with three choices, a (Recommended) badge on the first, and 'up/down navigate · enter select · esc cancel' footer.](https://omp.sh/captures/ask.webp)
...
Full reference: [openpaths.io/docs/sdk](https://openpaths.io/docs/sdk).

## A harness worth keeping is one you _don't_ outgrow.

Pick it up at **[openpaths.io](https://openpaths.io)**.

op is a fork of [Pi](https://github.com/badlogic/pi-mono) by [Mario Zechner](https://github.com/mariozechner), rewritten as a coding-first surface: sessions, subagents, slash commands, extensions — all TypeScript, all MIT, all on [GitHub](https://github.com/lee101/op). Shape it from config, hook it from outside, or read the source when you need to.
...
On first run op inherits whatever is already on disk: rules, skills, and MCP servers from `.claude`, `.cursor`, `.windsurf`, `.gemini`, `.codex`, `.cline`, `.github/copilot`, and `.vscode`. No migration script.

### Extensibility

Ask op to write the piece you're missing, then `/reload-plugins`. Keep it local, ship it in a `marketplace`, or publish it to npm.

## Philosophy

op is a fork of [pi-mono](https://github.com/badlogic/pi-mono) by [Mario Zechner](https://github.com/mariozechner), extended with a batteries-included coding workflow.
...
`bun setup` installs Bun workspaces and builds `@openpaths/natives`. Re-run `bun run build:native` after changing Rust crates or `packages/natives`.
...
Build and smoke-test the distributable Nix package with `nix build .#op`. Wayland screencast support is off by default (linking libpipewire adds ~750 MB of runtime closure); enable it with `op.override { withWaylandScreencast = true; }`. `nix/bun.nix` is generated only when `bun.lock` changes; releases regenerate it automatically. For dependency changes, run:
...
| Package                                                                       | Description                                                                 |
| ----------------------------------------------------------------------------- | --------------------------------------------------------------------------- |
| **[@openpaths/collab-web](packages/collab-web)**                               | Browser guest client, mock host, and local relay for collab live sessions   |
| **[@openpaths/ai](packages/ai)**                                            | Multi-provider LLM client with streaming and model/provider integration     |
| **[@openpaths/catalog](packages/catalog)**                                  | Model catalog: bundled model database, provider descriptors, and identity   |
| **[@openpaths/agent-core](packages/agent)**                                 | Agent runtime with tool calling and state management                        |
| **[@openpaths/coding-agent](packages/coding-agent)**                        | Interactive coding agent CLI and SDK                                        |
| **[@openpaths/tui](packages/tui)**                                          | Terminal UI library with differential rendering                             |
| **[@openpaths/natives](packages/natives)**                                  | N-API bindings for grep, shell, image, text, syntax highlighting, and more  |
| **[@openpaths/stats](packages/stats)**                                     | Local observability dashboard for AI usage statistics                       |
| **[@openpaths/optype](packages/optype)**                                     | ArkType-compatible schema validation with lazy JIT compilation              |
| **[@openpaths/utils](packages/utils)**                                      | Shared utilities (logging, streams, dirs/env/process helpers)               |
| **[@openpaths/wire](packages/wire)**                                        | Shared collab live-session protocol types and relay constants               |
| **[@openpaths/hashline](packages/hashline)**                                   | Line-anchored patch language and applier behind the `edit` tool             |
| **[@openpaths/mnemopi](packages/mnemopi)**                                  | Local SQLite memory engine for OpenPaths agents                              |
| **[@openpaths/snapcompact](packages/snapcompact)**                             | Bitmap-frame context compression package and SQuAD eval suite               |
| **[@openpaths/browser-relay](packages/browser-relay)**                         | Chrome extension that lets the browser tool drive your existing tabs        |
| **[@openpaths/metaharness](packages/metaharness)**                          | Unified benchmark runners, Harbor run storage, REST/SSE API, live dashboard |
| **[@openpaths/typescript-edit-benchmark](packages/typescript-edit-benchmark)** | Edit benchmark suite built on TypeScript source mutations                   |

### Rust Crates

| Crate                                              | Description                                                                                         |
| -------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| **[pi-natives](crates/pi-natives)**                | Core Rust native addon (N-API `cdylib`) used by `@openpaths/natives`; aggregates the crates below |
...
- [openpaths.io](https://openpaths.io)
- [GitHub](https://github.com/lee101/op)
- [Changelog](https://github.com/lee101/op/blob/main/packages/coding-agent/CHANGELOG.md)
- [npm](https://www.npmjs.com/package/@openpaths/coding-agent)
- [Discord](https://discord.gg/4NMW9cdXZa)
- [MIT](https://github.com/lee101/op/blob/main/LICENSE)
