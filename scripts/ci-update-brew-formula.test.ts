...
const SUMS = {
	"op-darwin-arm64": "darwin_arm64_sha",
	"op-darwin-x64": "darwin_x64_sha",
	"op-linux-arm64": "linux_arm64_sha",
	"op-linux-x64": "linux_x64_sha",
};
...
describe("renderFormula", () => {
	...
	// outside the staging CWD, `Dir["op-*"].first` returns `nil`, and
	// `bin.install nil => "op"` raises (issue #2398).
	it("attaches `using: :nounzip` to every per-platform url stanza", () => {
		const matches = formula.match(/using: :nounzip/g) ?? [];
		expect(matches).toHaveLength(4);
		for (const arch of ["op-darwin-arm64", "op-darwin-x64", "op-linux-arm64", "op-linux-x64"]) {
	...
	// popened binary doesn't touch the real `~/.op` (denied by Homebrew's
	...
	it("wraps `generate_completions_from_executable` with a HOME redirect to buildpath", () => {
		expect(formula).toMatch(
			/with_env\(HOME: buildpath\) do\n\s+generate_completions_from_executable\(bin\/"op", "completions", shells: \[:bash, :zsh, :fish\]\)\n\s+end/,
		);
