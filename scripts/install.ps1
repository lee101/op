# OP Coding Agent Installer for Windows
# Usage: irm https://raw.githubusercontent.com/lee101/op/main/scripts/install.ps1 | iex
#
# Or with options:
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/lee101/op/main/scripts/install.ps1))) -Source
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/lee101/op/main/scripts/install.ps1))) -Binary
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/lee101/op/main/scripts/install.ps1))) -Source -Ref v3.20.1
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/lee101/op/main/scripts/install.ps1))) -Source -Ref main
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/lee101/op/main/scripts/install.ps1))) -Binary -Ref v3.20.1
...
$Repo = "lee101/op"
$Package = "@openpaths/coding-agent"
$InstallDir = if ($env:PI_INSTALL_DIR) { $env:PI_INSTALL_DIR } else { "$env:LOCALAPPDATA\op" }
$BinaryName = "op-windows-x64.exe"
...
function Configure-BashShell {
    try {
        $settingsDir = Join-Path $env:USERPROFILE ".op\agent"
        ...
        if ($bashPath) {
            ...
            Write-Host ""
            Write-Host "No bash shell found - OP will use its built-in shell." -ForegroundColor Cyan
...
function Install-ViaBun {
    ...
    if ($Ref) {
        ...
        $tmpRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("op-install-" + [System.Guid]::NewGuid().ToString("N"))
    ...
    Write-Host ""
    Write-Host "[OK] Installed op via bun" -ForegroundColor Green

    Configure-BashShell

    Write-Host "Run 'op' to get started!"
}
...
function Install-Binary {
    ...
    $OutPath = Join-Path $InstallDir "op.exe"
    Invoke-WebRequest -Uri $BinaryUrl -OutFile $OutPath -TimeoutSec 900

    Write-Host ""
    Write-Host "[OK] Installed op to $OutPath" -ForegroundColor Green
    ...
    if ($needsRestart) {
        Write-Host "Restart your terminal, then run 'op' to get started!"
    } else {
        Write-Host "Run 'op' to get started!"
    }
}
