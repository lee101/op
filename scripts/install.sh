...
# OP Coding Agent Installer
# Usage: curl -fsSL https://raw.githubusercontent.com/lee101/op/main/scripts/install.sh | sh
#
# Options:
...
install_via_bun() {
    ...
    echo ""
    echo "✓ Installed op via bun"
    echo "Run 'op' to get started!"
}
...
install_binary() {
    ...
    curl -fsSL --connect-timeout 10 --speed-limit 1024 --speed-time 30 "$BINARY_URL" -o "${INSTALL_DIR}/op"
    ...
    if ! SMOKE_OUTPUT="$("${INSTALL_DIR}/op" --version 2>&1)"; then
        echo ""
        echo "✗ op was downloaded to ${INSTALL_DIR}/op but cannot start:"
        echo "$SMOKE_OUTPUT" | sed 's/^/    /'
        if [ "$PLATFORM" = "linux-musl" ]; then
            echo ""
            echo "The musl build links libstdc++/libgcc dynamically. Install them, then re-run 'op':"
    ...
    echo ""
    echo "✓ Installed op to ${INSTALL_DIR}/op"

    # Check if in PATH
    case ":$PATH:" in
        *":$INSTALL_DIR:"*) echo "Run 'op' to get started!" ;;
        *) echo "Add ${INSTALL_DIR} to your PATH, then run 'op'" ;;
    esac
}
