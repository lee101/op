 ...
 * `OP_RELEASE_NOTES_FLOOR=v15.12.4` to override (empty string forces
 * single-version mode, matching the pre-#2596 behavior). `OP_REPO` /
...
const REPO = process.env.OP_REPO ?? process.env.GITHUB_REPOSITORY ?? "lee101/op";
 ...
 *
 * Failure semantics:
 *   - `OP_RELEASE_NOTES_FLOOR` set → honored verbatim (`""` forces null).
 ...
 *     Local runs without `gh` should set `OP_RELEASE_NOTES_FLOOR=` to opt
 *     into legacy mode explicitly.
 */
async function resolvePublishedFloorTag(targetVersion: string): Promise<string | null> {
	const override = process.env.OP_RELEASE_NOTES_FLOOR;
	...
	if (res.exitCode !== 0) {
		...
		throw new Error(
			`gh release list exited ${res.exitCode}.\nstderr: ${stderr || "(empty)"}\n` +
				...
				`Locally without gh, set OP_RELEASE_NOTES_FLOOR= to fall back to single-version notes.`,
		);
	}
	let raw: unknown;
	try {
