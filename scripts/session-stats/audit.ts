#!/usr/bin/env bun
/**
 * Token-usage audit over the local op session corpus (~/.op/agent/sessions/).
 *
 ...
 * asks a small model (default: anthropic/claude-sonnet-4-6 via @openpaths/ai)
 * to judge:
 ...
 *
 * Auth: resolves an API key for the classifier provider through op's auth
 * storage (~/.op/agent/agent.db: stored key, OAuth, or env var fallback).
 */
...
import { getAgentDbPath, isEnoent } from "@openpaths/utils";
import SYSTEM_PROMPT from "./audit-prompt.md" with { type: "text" };

const SESSIONS_ROOT = path.join(os.homedir(), ".op", "agent", "sessions");
const DEFAULT_MODEL = "anthropic/claude-sonnet-4-6";
const CACHE_PATH = path.join(os.homedir(), ".op", "stats-audit-cache.json");
...
function parseCli(argv: string[]): CliOptions {
	...
	if (values.help) {
		console.log(
			`session audit — token usage analysis over ~/.op/agent/sessions\n\n` +
				...
				`  --no-cache               skip the verdict cache (~/.op/stats-audit-cache.json)\n` +
...
async function openClassifier(modelSpec: string): Promise<Classifier> {
	...
	if (!apiKey) {
		throw new Error(`no credentials for provider "${provider}" (op login or env var required)`);
	}
	return { model, apiKey };
}
