...
(`~/.op/agent/sessions/`). SQLite-backed; data is synced once into the same
`~/.op/stats.db` that `packages/stats` uses, then queried by short Python
scripts.
...
scripts/session-stats/
  sync.py          # walks ~/.op/agent/sessions/ and populates ss_* tables
...
The LLM phase (default `anthropic/claude-sonnet-4-6` via `@openpaths/ai`,
credentials resolved through op's auth storage — stored key, OAuth, or env
...
Verdicts are cached in `~/.op/stats-audit-cache.json` (keyed by session id +
