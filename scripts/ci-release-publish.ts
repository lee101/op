 ...
 *
 * The default mode publishes public JS packages and the `@openpaths/natives`
 ...
 *      Packages flagged `publishJs` (optype) additionally emit transpiled
...
export const packages: PublishPackage[] = [
	...
	{ dir: "packages/optype", kind: "typescript", publishJs: true },
	...
	{ dir: "packages/coding-agent", kind: "typescript", publishBin: { op: "dist/cli.js" } },
];
...
function buildNativeOptionalDependencies(version: string): JsonObject {
	...
	for (const target of LEAF_TARGETS) {
		optionalDependencies[`@openpaths/natives-${target.tag}`] = version;
	}
...
async function packAndPublish(dir: string, name: string): Promise<void> {
	...
	const packDir = await fs.mkdtemp(path.join(os.tmpdir(), "op-pack-"));
	try {
