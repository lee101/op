 ...
 * to a small model (default `google-antigravity/gemini-3.7-flash` via `@openpaths/ai`)
 ...
 *
 * Auth: resolves the provider API key through op's auth storage
 * (~/.op/agent/agent.db: stored key, OAuth, or env var fallback).
 */

import * as path from "node:path";
import { parseArgs } from "node:util";
import { type } from "@openpaths/optype";
import { type Api, completeSimple, Effort, type Model, type Tool, type ToolCall } from "@openpaths/ai";
import { discoverAuthStorage } from "@openpaths/ai/auth-broker";
...
async function openModel(modelSpec: string): Promise<RewriteModel> {
	...
	if (!apiKey) {
		throw new Error(
			`no credentials for provider "${provider}" via ${storage.sourceLabel ?? "auth storage"} (check broker or run \`op login\`)`,
		);
	}
