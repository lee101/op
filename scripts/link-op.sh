#!/bin/sh
# Install the dev `op` wrapper into Bun's global bin directory.
#
...
# `packages/coding-agent/scripts/op`. See that wrapper's header comment for the
...
# through that error would expand `$(bun pm -g bin)/op` to `/op` and try to
# write under `/` — see https://github.com/lee101/op/issues/3701.
set -e

repo_root=$(CDPATH='' cd -- "$(dirname -- "$0")/.." && pwd -P)
target=$repo_root/packages/coding-agent/scripts/op

if [ ! -x "$target" ]; then
	echo "link-op: target wrapper not found or not executable: $target" >&2
	exit 1
fi
...
mkdir -p "$global_bin"
ln -sfn "$target" "$global_bin/op"
echo "link-op: linked $global_bin/op -> $target"
