...
def resolve_omp_bin(raw: str | None) -> str:
    ...
    if not found:
        raise SystemExit("Could not find `op` on PATH. Set --op-bin or OP_BIN.")
    return found
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run OpenRouter fixture evaluations through op RPC mode."
    )
    parser.add_argument("--op-bin", default=os.environ.get("OP_BIN"))
...
async def run_all(args: argparse.Namespace) -> int:
    ...
    results_dir = (
        ...
        if args.results_dir
        else tmp_root / f"op-fixture-runs-{timestamp}"
    )
