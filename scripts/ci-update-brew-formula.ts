#!/usr/bin/env bun
//
// Render the Homebrew formula for `op` from a published GitHub release and write
// it to a tap checkout. The release publishes per-platform bare binaries
// (op-<platform>-<arch>); this reads their sha256 digests straight from the
// release metadata so the formula never drifts from the shipped assets.
//
// Usage:
//   bun scripts/ci-update-brew-formula.ts <tag> --out <path/to/Formula/op.rb>
//   bun scripts/ci-update-brew-formula.ts v15.10.3        # prints to stdout

import { $ } from "bun";

const REPO = process.env.OP_REPO ?? "lee101/op";
const HOMEPAGE = "https://openpaths.io";
...
export function renderFormula(version: string, sums: Record<string, string>): string {
	...
	// which nests the file outside the staging CWD; `Dir["op-*"].first` then
	// returns `nil` and `bin.install nil => "op"` raises.
	//
	...
	// not touch the real `/Users/<user>/.op` (denied by Homebrew's sandbox
	// profile, which would otherwise fail the popen).
	return `class Op < Formula
  desc "${DESC}"
  homepage "${HOMEPAGE}"
  version "${version}"
  license "MIT"
  ...
  on_macos do
    on_arm do
      url "https://github.com/${REPO}/releases/download/v#{version}/op-darwin-arm64",
          using: :nounzip
      sha256 "${sums["op-darwin-arm64"]}"
    end
    on_intel do
      url "https://github.com/${REPO}/releases/download/v#{version}/op-darwin-x64",
          using: :nounzip
      sha256 "${sums["op-darwin-x64"]}"
    end
  end

  on_linux do
    on_arm do
      url "https://github.com/${REPO}/releases/download/v#{version}/op-linux-arm64",
          using: :nounzip
      sha256 "${sums["op-linux-arm64"]}"
    end
    on_intel do
      url "https://github.com/${REPO}/releases/download/v#{version}/op-linux-x64",
          using: :nounzip
      sha256 "${sums["op-linux-x64"]}"
    end
  end
  ...
  def install
    ...
    with_env(HOME: buildpath) do
      generate_completions_from_executable(bin/"op", "completions", shells: [:bash, :zsh, :fish])
    end
  end
...
async function main(): Promise<void> {
	...
	const targets = ["op-darwin-arm64", "op-darwin-x64", "op-linux-arm64", "op-linux-x64"];
