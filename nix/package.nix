...
let
  ...
  platform =
    ...
    .${stdenv.hostPlatform.system} or (throw "Unsupported OP platform: ${stdenv.hostPlatform.system}");
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    install -Dm755 packages/coding-agent/dist/op "$out/bin/op"
  ...
  preFixup = ''
    remove-references-to -t ${bun} "$out/bin/op"
  '';
  ...
  installCheckPhase = ''
    ...
    HOME="$TMPDIR" "$out/bin/op" --smoke-test | grep -q "smoke-test: ok"
    BUN_BE_BUN=1 "$out/bin/op" -e \
  ...
  meta = {
    ...
    homepage = "https://openpaths.io";
    changelog = "https://github.com/lee101/op/releases/tag/v${packageJson.version}";
