...
`scripts/probe.ts`: `@openpaths/ai` `completeSimple`; production-matching model/auth/provider behavior.

```bash
bun .op/skills/tool-prompt-optimization/scripts/probe.ts \
...
```bash
bun .op/skills/tool-prompt-optimization/scripts/probe-builtin.ts --tool <name> [--no-summary] [--show]
```
...
```ts
import { arkToWireSchema } from "@openpaths/ai"; // or toolWireSchema(tool)
