...
Typed Python bindings for the `op --mode rpc` protocol used by the coding agent.
...
- typed startup options for common `op --mode rpc` flags such as thinking level,
...
```python
from op_rpc import MessageUpdateEvent, RpcClient
...
from op_rpc import RpcClient, host_tool
...
```python
from op_rpc import RpcClient, host_uri
...
```python
from op_rpc import assistant_text, assistant_text_with_thinking
