...
from op_rpc.client import _RpcFrameDecoder
...
class RpcClientTests(unittest.TestCase):
    ...
    def test_xd_dispatched_custom_tool_events_carry_host_tool_name(self) -> None:
        ...
        With `tools.xdev` on, op invokes a custom tool through `write
        ...
        (openpaths#6696). `tool_execution_start` precedes the `host_tool_call`
...
class TerminatesProcessGroupTests(unittest.TestCase):
    ...
    the op process. Before the fix, stop() signalled only the leader pid, so
    such grandchildren reparented to the container init and kept running —
    once ballooning to tens of GB of RAM. op is now spawned in its own
    ...
    def test_stop_kills_grandchild_spawned_by_server(self) -> None:
        ...
        # Fake op server: spawn the long-lived grandchild, signal ready, then
