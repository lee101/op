...
`roboomp` is a self-hosted GitHub triage-and-fix bot that drives [`op --mode rpc`](https://github.com/lee101/op) as a subprocess. On every issue opened in an allowlisted repository it classifies the issue, applies labels, then branches into one of: reproduce → fix → PR (`bug` / `documentation`), single-comment answer (`question`), single thoughtful comment (`enhancement` / `proposal`), or brief comment (`invalid` / `duplicate`). Follow-up comments and PR review comments resume the same op session so the agent keeps its prior reasoning. If the orchestrator restarts mid-task, the dispatcher resumes the same session via `op --continue` from the per-issue `session_dir`, so an interrupted task re-enters its prior reasoning instead of restarting from scratch. The orchestrator runs as a single FastAPI process inside Docker with SQLite-backed durable event state.

## Architecture & Data Flow

Webhook → durable queue → async dispatcher → per-issue git worktree → op RPC subprocess + host tools.
...
6. `tasks.*` dispatchers build `TaskInputs` and call `worker.run_task()` which spawns `op --mode rpc` with `cwd=worktree`, persistent `session_dir`, and a randomly-picked model from `ROBOMP_MODEL` (CSV pool). When `<session_dir>/*.jsonl` already exists the worker passes `--continue`, so both follow-up events and crash-restarted events resume the same session.
7. Inside the subprocess the agent uses **built-in op tools** (read/edit/write/bash/lsp, scoped to the worktree) and **host tools** from `host_tools.py` (the only surface allowed to mutate GitHub or write audit rows).
...
- `/Dockerfile` (pi root) — produces `openpaths/pi:dev` (pi runtime image: python + bun + rustup + pi-natives + op_rpc + `/usr/local/bin/op` shim + the full pi source under `/pi`). Stages: `natives-builder` → `wheel-builder` → `pi-base` → `pi-runtime` (default). Built via `bun run pi:image`. Robomp's image extends `pi-base` via `FROM ${PI_BASE}` in `/Dockerfile.robomp`.
...
```
bun run test:py                   # pytest -x python/op-rpc/tests python/robomp/tests
bun run robomp:test:integration   # ROBOMP_INTEGRATION=1, requires op on PATH
...
```
bun run pi:image                  # build openpaths/pi:dev (one-time / on pi change)
bun run pi:run                    # docker run -it openpaths/pi:dev (smoke-test the shim)
...
- **Async style**: FastAPI handlers and `queue.WorkerPool` are async. `worker.run_task` is **synchronous** and runs in a worker thread because `op-rpc` is blocking — keep it that way; don't try to async it. CLI commands wrap with `asyncio.run`.
...
- `src/worker.py` — synchronous op RPC driver, prompt assembly via `persona`.
...
- `/Dockerfile.robomp` (pi root) — robomp's image. `FROM ${PI_BASE}` (default `openpaths/pi:dev`), adds the SolidJS dashboard bundle, the robomp Python package, and the `robomp-entrypoint` shim. Tini entrypoint, exposes `8080`, `VOLUME /data`. The toolchain (python + bun + rustup + pi-natives + op_rpc + `op` shim) comes from `pi-base` — no duplication in this file.
- `docker-compose.yml` — `build.args.PI_BASE`, mounts `$PI_ROOT:/work/pi:ro`, `./data:/data`, `~/.op/agent/models.container.yml:ro` (mapped to `models.yml` inside the container — kept separate from the host's `~/.op/agent/models.yml` so the host op doesn't pick up gateway routing intended only for the container), `extra_hosts: llm-gateway.internal:host-gateway`.
...
- **Container runtime**: Docker Compose v2. The image embeds Bun 1.3.14 + a rustup launcher and exposes `op` via a `/usr/local/bin/op` shim; `ROBOMP_OMP_COMMAND=op` should not need changing.
...
- **PI_ROOT resolution**: roboomp lives inside the openpaths monorepo at `python/robomp/`. `bun run pi:image` builds the parent monorepo (`../..`) as its docker build context to produce `openpaths/pi:dev`; `docker-compose.yml` extends that image via `PI_BASE` and mounts the same parent path read-only at `/work/pi` for the orchestrator to see live source. Override `PI_ROOT` only when pointing the build/mount at a different openpaths checkout. Inside the container the path is always `/work/pi`. Build invalidation stays bounded: Python-only edits in roboomp never trigger a natives recompile.
...
- **Integration**: `tests/test_worker_smoke.py` is gated by `ROBOMP_INTEGRATION=1` (uses `pytestmark.skipif`) and needs `op` on `PATH`. Don't enable it in default `bun run test:py`.
