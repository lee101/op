...
Ownership: cache root is provisioned ``root:op 02770`` by ``entrypoint.sh``
so slot subprocesses (group ``op``) can capture under setgid inheritance.
        ...
        *,
        ...
        with _flock(self.lockfile(repo)):
            ...
            try:
                ...
                # the setgid `op` model that lets other slots read it.
                # A copy creates a fresh inode owned by the orchestrator
                # (root) and inherits gid `op` from the setgid 2770
                # cache root.
