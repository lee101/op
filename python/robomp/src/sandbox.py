...
1. **Workspace tree** (`/data/workspaces/<key>/`, including `repo/`,
   `.op-session/`, `context/`, `artifacts/`, `.op-tmp/`, `.op-xdg`):
   single-owner. Owned by the active slot UID/GID (`op-N`) when slot
...
2. **Clone pool** (`/data/workspaces/_pool/<owner>__<repo>/`): genuinely
   multi-slot. Owned by `root:op` (gid 2000) with setgid `02770`; cross-slot
...
3. **Language tool caches** (`/data/cache/{cargo,cargo-target,rustup,bun-cache}`):
   multi-slot. Owned by `root:op` with setgid `02770`; provisioned by
   `entrypoint.sh`.
...
def _prepare_slot_tmpdir(workspace: Workspace, slot_uid: int | None) -> Path:
    ...
    - replaces any non-directory at ``.op-tmp`` (symlink-protection: a user
...
def _prepare_slot_runtime_env(workspace: Workspace, slot_uid: int | None) -> dict[str, str]:
    ...
    and are group-shared via ``op``. Bun's install cache is explicitly
    ...
    xdg_root = workspace.root / ".op-xdg"
...
def _provision_runtime_dirs(ws_root: Path) -> None:
    ...
    picks up ``.op-tmp`` and the per-workspace XDG tree. Without this,
    ...
    Symlink-safe on ``.op-tmp`` (replaces a planted non-directory in place).
    """
    tmpdir = ws_root / ".op-tmp"
    try:
        st = tmpdir.lstat()
    except FileNotFoundError:
        pass
...
def _share_git_metadata_with_slots(repo_dir: Path, slot_uid: int | None) -> None:
    ...
    `op-N` user, so the pool-side worktree gitdir, refs, reflogs, and object
    directories must stay writable through the shared `op` group.
    """
    if not _slot_permissions_active(slot_uid):
        return
...
class SandboxManager:
    ...
    def ensure_workspace(
        ...
        *,
        ...
        with self._repo_lock(repo):
            ...
            session_dir = ws_root / ".op-session"
            ...
            # keeps its `root:op` ownership (the slot reads through group `op`);
    ...
    def _populate_natives_cache(self, workspace: Workspace, *, slot_uid: int | None = None) -> None:
        ...
        `.node` files are LEFT at `root:op` ownership — chowning them
        would chown the cache file too (shared inode), breaking the
        cross-slot sharing model. The slot reads them via group `op`.
        """
    ...
    def _chown_natives_for_slot(native_dir: Path, hit: CacheHit, *, slot_uid: int) -> None:
        ...
        hardlinks back into the root:op cache and the slot reads them via
        group `op`. Everything else (the directory itself, copied
