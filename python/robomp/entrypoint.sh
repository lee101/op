...
# proxy role does NOT need a $PI_ROOT pi checkout — it never runs op.
set -euo pipefail

# Shared git metadata under /data/workspaces/_pool is intentionally group
# writable by the `op` group so interrupted work can resume on a different
...
for i in $(seq 1 "$max_slots"); do
    ...
    id -u "$user" >/dev/null 2>&1 || /usr/sbin/useradd -u "$slot_id" -g "$slot_group" -G op -M -N -s /usr/sbin/nologin "$user"
    /usr/sbin/usermod -g "$slot_group" -a -G op "$user"
done
...
chown -R root:op /data/cache /data/workspaces/_pool
...
rm -rf /srv/agent-home/.agent /srv/agent-home/.op/agent
mkdir -p /srv/agent-home/.agent /srv/agent-home/.op/agent
if [ -e /srv/agent-home-stage/.agent ]; then
    cp -a /srv/agent-home-stage/.agent/. /srv/agent-home/.agent/
fi
if [ -e /srv/agent-home-stage/.op/agent ]; then
    cp -a /srv/agent-home-stage/.op/agent/. /srv/agent-home/.op/agent/
fi
...
# op registers daemon project presence under ~/.op/run at startup, nesting
...
# op keeps the whole tree group-writable (entrypoint umask 0002 carries into
# slot processes, so new entries stay group-writable too).
mkdir -p /srv/agent-home/.op/run
chgrp -R op /srv/agent-home/.op/run
chmod -R g+rwX /srv/agent-home/.op/run
find /srv/agent-home/.op/run -type d -exec chmod g+s {} +
chmod 2770 /srv/agent-home/.op/run
