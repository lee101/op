...
import httpx
import pytest
from op_rpc import HostToolContext, RpcCommandError
...
def test_repo_command_env_scrubs_secrets_and_uses_workspace_cache(
    ...
    assert env["BUN_INSTALL_CACHE_DIR"] == str(bindings.workspace.root / ".op-xdg" / "cache" / "bun-install")
    assert env["XDG_CACHE_HOME"] == str(bindings.workspace.root / ".op-xdg" / "cache")
    assert env["TMPDIR"] == str(bindings.workspace.root / ".op-tmp")
    ...
    assert (bindings.workspace.root / ".op-tmp").is_dir()
...
def test_run_repo_command_uses_slot_identity_kwargs(
    ...
    assert kwargs["env"]["BUN_INSTALL_CACHE_DIR"].endswith("/.op-xdg/cache/bun-install")
