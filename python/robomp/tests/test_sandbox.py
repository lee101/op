...
def test_chown_workspace_runs_chown_and_chmod_as_root_on_linux(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # 2001 is the slot-private GID matching the slot UID, not the shared op group.
    assert calls == [
...
def test_provision_runtime_dirs_replaces_tmpdir_symlink_and_creates_xdg_tree(tmp_path: Path) -> None:
    ...
    assert target.is_dir()
    assert stat.S_IMODE(tmpdir.stat().st_mode) == 0o700
    for base in (tmp_path / ".op-xdg" / "data", tmp_path / ".op-xdg" / "state", tmp_path / ".op-xdg" / "cache"):
        assert base.is_dir()
        assert (base / "op").is_dir()
    assert (tmp_path / ".op-xdg" / "cache" / "bun-install").is_dir()
...
def test_prepare_slot_runtime_env_returns_workspace_private_paths_without_chown(
    ...
    bun_cache = ws.root / ".op-xdg" / "cache" / "bun-install"

    env = _prepare_slot_runtime_env(ws, 2001)

    assert env["TMPDIR"] == str(ws.root / ".op-tmp")
    assert env["XDG_CACHE_HOME"] == str(ws.root / ".op-xdg" / "cache")
    assert env["BUN_INSTALL_CACHE_DIR"] == str(bun_cache)
    for base in (ws.root / ".op-xdg" / "data", ws.root / ".op-xdg" / "state", ws.root / ".op-xdg" / "cache"):
        assert base.is_dir()
        assert (base / "op").is_dir()
...
def test_ensure_workspace_provisions_and_slot_owns_runtime_dirs(
    ...
    def record_chown(ws_root: Path, slot_uid: int | None) -> None:
        ...
        paths = [
            ws_root / ".op-tmp",
            ws_root / ".op-xdg" / "data",
            ws_root / ".op-xdg" / "data" / "op",
            ws_root / ".op-xdg" / "state",
            ws_root / ".op-xdg" / "state" / "op",
            ws_root / ".op-xdg" / "cache",
            ws_root / ".op-xdg" / "cache" / "op",
            ws_root / ".op-xdg" / "cache" / "bun-install",
        ]
    ...
    assert set(runtime_paths) == {
        ws.root / ".op-tmp",
        ws.root / ".op-xdg" / "data",
        ws.root / ".op-xdg" / "data" / "op",
        ws.root / ".op-xdg" / "state",
        ws.root / ".op-xdg" / "state" / "op",
        ws.root / ".op-xdg" / "cache",
        ws.root / ".op-xdg" / "cache" / "op",
        ws.root / ".op-xdg" / "cache" / "bun-install",
    }
...
# ---------------------------------------------------------------------------
# Partial-clone blob backfill (openpaths#1818)
# ---------------------------------------------------------------------------
...
def test_fetch_ref_backfills_missing_blobs_into_partial_clone(tmp_path: Path) -> None:
    """Regression for openpaths#1818: ``fetch_ref`` is called immediately before
...
def _seed_reclaimable_workspace(mgr: SandboxManager, repo: str, number: int) -> Path:
    ...
    (ws_root / ".op-session/session.jsonl").write_text("{}", encoding="utf-8")
    (ws_root / ".op-xdg/cache/bun-install/pkg.tgz").write_text("x", encoding="utf-8")
    (ws_root / ".op-xdg/state/op/state.json").write_text("{}", encoding="utf-8")
    (ws_root / ".op-tmp/scratch").write_text("x", encoding="utf-8")
...
def test_reclaim_workspace_caches_strips_dep_caches_and_preserves_state(tmp_path: Path) -> None:
    ...
    assert not (ws_root / ".op-xdg/cache").exists()
    assert not (ws_root / ".op-tmp").exists()
    assert (ws_root / "repo/src/keep.ts").read_text(encoding="utf-8") == "keep"
    assert (ws_root / ".op-session/session.jsonl").exists()
    assert (ws_root / ".op-xdg/state/op/state.json").exists()
