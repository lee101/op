...
def _require_linux_root_toolchain() -> None:
    if platform.system() != "Linux" or os.geteuid() != 0:
        pytest.skip("slot permission e2e tests require Linux root so subprocesses can drop to op-N UIDs")
...
def test_slot_workspace_runs_bun_biome_cargo_and_git_after_root_reentry(
    ...
    stale_bun_cache = first.root / ".op-xdg" / "cache" / "bun-install" / "root-owned-stale"
...
def _prepare_shared_natives_cache(slot_tmp_path: Path) -> NativesCache:
    """Provision `/data/cache/pi-natives` shape (root:op, setgid 2770)."""
...
def test_natives_cache_shares_artifacts_across_slot_workspaces(
    ...
    1. A capture from a slot-owned workspace lands in the shared cache with
       group `op` setgid inheritance so any other slot can read it.
    ...
    # Cache root is setgid `op`; new files inherit gid `op` so any slot
    # with `extra_groups=[op]` can read them.
    ...
    # Slot 2 must be able to read the populated artifacts (group op + 0660
