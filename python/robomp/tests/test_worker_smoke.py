...
Runs only when ROBOMP_INTEGRATION=1 and `op` is available on PATH (or via
...
- a real `op --mode rpc` subprocess driven by `worker.run_task`.
...
pytestmark = pytest.mark.skipif(
    not INTEGRATION,
    reason="ROBOMP_INTEGRATION=1 required to run the op-backed smoke test",
)
