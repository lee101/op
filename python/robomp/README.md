...
Self-hosted GitHub triage bot. Drives [`op --mode rpc`](https://github.com/lee101/op)
...
Follow-up issue comments and PR review comments resume the same op session
...
- **robomp** — FastAPI + sqlite event queue + `WorkerPool` running `op` in
...
→ `worker.run_task` spawns `op --mode rpc` with `cwd=worktree`,
persistent `session_dir`, model randomly drawn from `ROBOMP_MODEL` (CSV).

The agent uses op's built-in tools (`read`/`edit`/`bash`/`lsp`, scoped to
...
`~/.op/agent/models.container.yml` points at (mounted into the container as `models.yml`; kept under a separate filename on the host so the host op doesn't route through the gateway). roboomp lives inside the openpaths
...
`PI_ROOT` only if you want a different openpaths checkout backing the build
and runtime.
...
runtime layer; editing pi source rebuilds `openpaths/pi:dev`, which
...
The integration test spawns a real `op --mode rpc` against an
`httpx.MockTransport` GitHub and a local bare repo, so it needs `op` on
...
| Container exits with `PI_ROOT … missing` | `/work/pi` mount empty inside the container; on the host either run `docker compose` from `python/robomp/` so `PI_ROOT` defaults to `../..`, or export `PI_ROOT` to a valid openpaths checkout. |
...
| `No API key found for <provider>` | `~/.op/agent/models.container.yml` mount missing or provider id mismatch with `ROBOMP_MODEL`. |
...
src/
  ...
  worker.py          synchronous op RPC driver, prompt assembly, env scrubbing
