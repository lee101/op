...
import { optypeCandidate } from "./candidates/optype";
import { typeboxCandidate } from "./candidates/typebox";
import { FIXTURES, generateUniqueDefs } from "./fixtures";
import type { Def } from "./ir";

const registry: Candidate[] = [optypeCandidate, arktypeCandidate, typeboxCandidate];
...
function report(title: string, rows: Row[]): void {
	const baseline = rows.find(row => row.candidate === "optype");
	console.log(`\n== ${title} ==`);
	for (const row of rows) {
		let relative = "";
		if (baseline && row.candidate !== "optype") {
