...
import { OpTypeError } from "../src/errors";
...
describe("zod-like parsing", () => {
	it("exposes callable optype schemas with JSON Schema metadata", () => {
		...
		expect(schema({ website: "https://openpaths.io" })).toEqual({
			name: "Ada",
	...
	it("dispatches min and max by string, number, and array kind", () => {
		...
		expect(() => z.boolean().min(1)).toThrow(OpTypeError);
	});

	it("supports string and number refinements plus nullable and optional values", () => {
		expect(z.string().regex(/^op$/).url().safeParse("op").success).toBe(false);
		expect(z.string().url().parse("https://openpaths.io")).toBe("https://openpaths.io");
	...
	it("validates records with string key and value schemas", () => {
		...
		expect(() => z.record(z.number() as never, z.string())).toThrow(OpTypeError);
