...
import { fromJsonSchema, OpErrors, type } from "../src";
...
describe("fromJsonSchema", () => {
	it("round-trips schemas emitted by toJsonSchema", () => {
		...
		expect(imported({ name: "job", mode: "delete", tags: [] })).toBeInstanceOf(OpErrors);
		expect(imported({ name: "", mode: "read", tags: [] })).toBeInstanceOf(OpErrors);
		expect(imported({ mode: "read", tags: [] })).toBeInstanceOf(OpErrors);
		expect(imported({ name: "job", mode: "read", tags: [1] })).toBeInstanceOf(OpErrors);
	});
	...
	it("supports enum, const, unions, intersections, and boolean schemas", () => {
		...
		expect(bounded(4)).toBeInstanceOf(OpErrors);
		expect(fromJsonSchema(true)("anything")).toBe("anything");
		expect(fromJsonSchema(false)("anything")).toBeInstanceOf(OpErrors);
	});

	it("validates string formats and patterns", () => {
		const email = fromJsonSchema({ type: "string", format: "email" });
		expect(email("a@openpaths.io")).toBe("a@openpaths.io");
		expect(email("nope")).toBeInstanceOf(OpErrors);
		const patterned = fromJsonSchema({ type: "string", pattern: "^G-[A-Z0-9]+$", minLength: 3 });
		expect(patterned("G-X1")).toBe("G-X1");
		expect(patterned("bad")).toBeInstanceOf(OpErrors);
		expect(patterned("G-")).toBeInstanceOf(OpErrors);
	});
	...
	it("imports tuples with variadic tails and rejects undeclared keys on closed objects", () => {
		...
		expect(pair([1])).toBeInstanceOf(OpErrors);
		...
		expect(closed({ a: "x", b: 1 })).toBeInstanceOf(OpErrors);
	});
	...
	it("emits recursive scopes as $defs/$ref and round-trips them", () => {
		...
		expect(imported({ value: 1, next: { value: "two" } })).toBeInstanceOf(OpErrors);
	...
	it("emits io-aware schemas: input keeps morph sources and optional defaults, output the reverse", () => {
		...
		expect(imported({ raw: 42 })).toBeInstanceOf(OpErrors);
	});
	...
	it("io output uses the .to() target for piped schemas", () => {
		...
		expect(Uid(0)).toBeInstanceOf(OpErrors);
