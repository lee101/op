...
import { OpErrors, Type, type } from "../src";
...
function normalized(result: unknown): unknown {
	return result instanceof OpErrors ? { error: result.summary } : result;
}
...
describe("validation", () => {
	...
	it("rejects wrong primitive, bad literal, broken bound, and missing key with path-aware errors", () => {
		...
		if (missing instanceof OpErrors) {
			expect(missing[0].path).toEqual(["path"]);
			expect(missing.summary).toContain("path");
		}
		expect(tool({ path: "x", mode: "delete" })).toBeInstanceOf(OpErrors);
		expect(tool({ path: "x", mode: "read", offset: 0 })).toBeInstanceOf(OpErrors);
		expect(tool({ path: "x", mode: "read", offset: 1.5 })).toBeInstanceOf(OpErrors);
		expect(tool({ path: "x", mode: "read", tags: ["a", 1] })).toBeInstanceOf(OpErrors);
	});
	...
	it("bounds: numbers (exclusive/inclusive) and string length", () => {
		...
		expect(s({ t: 0, name: "ok" })).toBeInstanceOf(OpErrors);
		expect(s({ t: 3601, name: "ok" })).toBeInstanceOf(OpErrors);
		expect(s({ t: 1, name: "" })).toBeInstanceOf(OpErrors);
		expect(s({ t: 1, name: "toolongname" })).toBeInstanceOf(OpErrors);
	});
	...
	it("array suffix binds outside a trailing bound: string>0[]", () => {
		...
		expect(s({ files: [""] })).toBeInstanceOf(OpErrors);
		expect(s({ files: "" })).toBeInstanceOf(OpErrors);
	});

	it("string.url validates parseability", () => {
		const s = type("string.url");
		expect(s("https://openpaths.io/x")).toBe("https://openpaths.io/x");
		expect(s("not a url")).toBeInstanceOf(OpErrors);
	});

	it("index signatures validate every value", () => {
		const s = type({ env: { "[string]": "string" } });
		expect(s({ env: { A: "1" } })).toEqual({ env: { A: "1" } });
		expect(s({ env: { A: 1 } })).toBeInstanceOf(OpErrors);
	});
	...
	it("null/undefined union members", () => {
		...
		expect(s("x")).toBe("x");
		expect(s(5)).toBeInstanceOf(OpErrors);
	});
});
...
describe("methods", () => {
	...
	it(".narrow() with ctx.mustBe records the expectation", () => {
		...
		expect(bad).toBeInstanceOf(OpErrors);
		if (bad instanceof OpErrors) expect(bad.summary).toContain("a body unless deleting");
	});

	it(".narrow() runs after morphs (sees defaults)", () => {
		const s = type({ n: "number = 5" }).narrow(v => v.n === 5);
		expect(s({})).toEqual({ n: 5 });
		expect(s({ n: 6 })).toBeInstanceOf(OpErrors);
	});
	...
	it(".and() merges object schemas", () => {
		...
		expect(s({ b: 2 })).toBeInstanceOf(OpErrors);
	});
	...
	it("type.enumerated builds literal unions from runtime arrays", () => {
		...
		expect(s("py")).toBe("py");
		expect(s("rs")).toBeInstanceOf(OpErrors);
	});
...
describe("embedded schemas", () => {
	...
	it("union failures discriminate object members by literal props", () => {
		...
		for (let i = 0; i < JIT; i++) {
			...
			expect(out).toBeInstanceOf(OpErrors);
...
describe("advanced ArkType compatibility", () => {
	it("re-exports recursive scopes through the Ark compatibility facade", () => {
		...
		expect(invalid).toBeInstanceOf(OpErrors);
		if (invalid instanceof OpErrors) expect(invalid[0].path).toEqual(["next", "value"]);
	});
	...
	it("resolves recursive scopes, modules, utility generics, and runtime generics", () => {
		...
		expect(bad).toBeInstanceOf(OpErrors);
		if (bad instanceof OpErrors) expect(bad[0].path).toEqual(["manager", "name"]);
		...
		expect(box("number")({ value: "2" })).toBeInstanceOf(OpErrors);
	});
	...
	it("supports fixed, optional, defaulted, and variadic tuple elements", () => {
		...
		expect(variadic(["x", true])).toBeInstanceOf(OpErrors);
	...
	it("validates Date literals and bounds before and after JIT compilation", () => {
		...
		for (let index = 0; index < JIT; index++) {
			...
			expect(exact(new Date("2024-01-03T00:00:00.000Z"))).toBeInstanceOf(OpErrors);
			expect(bounded(new Date("2024-01-01T00:00:00.000Z"))).toEqual(new Date("2024-01-01T00:00:00.000Z"));
			expect(bounded(new Date("2023-12-31T23:59:59.999Z"))).toBeInstanceOf(OpErrors);
		}
	});
	...
	it("supports disjointness-aware intersections and semantic comparison", () => {
		...
		expect(bounded(0)).toBeInstanceOf(OpErrors);
	...
	it("maps and selects structural nodes and distributes unions", () => {
		...
		expect(distributed(["c"])).toBeInstanceOf(OpErrors);
	});
	...
	it("exposes nested keyword modules and configurable error codes", () => {
		...
		expect(type.parse.url("https://openpaths.io")).toBeInstanceOf(URL);
...
describe("Standard Schema V1", () => {
	...
	it("materializes root defaults for undefined input at the standard boundary", () => {
		...
		expect(staticDefault(5)).toBeInstanceOf(OpErrors);
...
describe("type.withJsonSchema", () => {
	it("emits the override verbatim even when embedded, and still validates", () => {
		...
		expect(inner("c")).toBeInstanceOf(OpErrors);
		expect(object({ mode: "b" })).toEqual({ mode: "b" });
		expect(object({ mode: "c" })).toBeInstanceOf(OpErrors);
	});
