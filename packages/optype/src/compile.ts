 ...
 * - failure allocates a single `OpErrors` (`E(path, expected, data)`); path
...
class CompiledMorphContext implements MorphContext {
	...
	error(expectation: string): OpErrors {
		return new OpErrors(this.#path, expectation, this.#data);
	}
...
class Builder {
	...
	buildAllows(ir: IR): (value: unknown) => value is unknown {
		...
		return make(this.#refs, OpErrors, own);
	}
}

function prefixErrors(errs: OpErrors, parts: PropertyKey[]): OpErrors {
	for (let i = parts.length - 1; i >= 0; i--) errs.prefix(parts[i]);
	return errs;
}

const kWalk = Symbol("optype.boundWalk");
...
export function compileAllows(ir: IR): (value: unknown) => value is unknown {
	...
	if (validator === undefined) {
		...
		allowsCache.set(root, ((value: unknown) =>
			built === undefined ? !(walk(root, value) instanceof OpErrors) : built(value)) as (
			value: unknown,
		) => value is unknown);
