 ...
 * `result instanceof type.errors` / `instanceof OpErrors`, lazy `.summary`,
...
export const MISSING: unique symbol = Symbol("optype.missing");
...
export class OpErrors implements Iterable<OpError> {
	#path: StoredPath;
	#expected: string;
	#data: unknown;
	#entry: OpError | undefined;
	#entries: OpError[] | undefined;
	#config: ErrorConfig | undefined;
	#separator = "\n";
	...
	static single(path: PropertyKey[], expected: string, data: unknown, config?: ErrorConfig): OpErrors {
		return new OpErrors(path, expected, data, config);
	}

	#getEntry(): OpError {
		if (this.#entry) return this.#entry;
		const path = this.#path === undefined ? [] : Array.isArray(this.#path) ? [...this.#path] : [this.#path];
		const entry = new OpError(path, this.#expected, this.#data, this.#config);
		this.#entry = entry;
		return entry;
	}

	/** Append all failures from `other`, preserving traversal order. */
	append(other: OpErrors): this {
	...
	get byPath(): Readonly<Record<string, OpError>> {
		const result: Record<string, OpError> = {};
		for (const entry of this) result[entry.path.map(String).join(".")] = entry;
		return result;
	}

	map<result>(fn: (error: OpError, index: number, errors: OpErrors) => result): result[] {
		const result: result[] = [];
		let index = 0;
	...
	filter(fn: (error: OpError, index: number, errors: OpErrors) => unknown): OpError[] {
		const result: OpError[] = [];
		let index = 0;
	...
	*[Symbol.iterator](): IterableIterator<OpError> {
		if (this.#entries) {
			yield* this.#entries;
		} else {
			yield this.#getEntry();
...
export class TraversalError extends Error {
	constructor(readonly errors: OpErrors) {
 ...
 * validation failures, which are returned as {@link OpErrors}.
 */
export class OpTypeError extends Error {
	constructor(message: string) {
		super(message);
		this.name = "OpTypeError";
	}
}
