 ...
 * output, or an `OpErrors` on failure (`result instanceof type.errors`).
 */
import { compile, compileAllows } from "./compile";
import { type ErrorConfig, OpErrors, OpTypeError, TraversalError } from "./errors";
...
export interface NarrowContext {
	readonly path: readonly PropertyKey[];
	error(error: string | NarrowErrorInput): OpErrors;
	mustBe(expectation: string): false;
	reject(problem: string | NarrowErrorInput): OpErrors | false;
}
...
interface FluentMethods<t, i> {
	...
	filter(fn: (data: i, ctx: NarrowContext) => boolean | OpErrors): FluentType<t, i>;
	narrow<narrowed extends t>(fn: (data: t, ctx: NarrowContext) => data is narrowed): FluentType<narrowed, i>;
	narrow(fn: (data: t, ctx: NarrowContext) => boolean | OpErrors): FluentType<t, i>;
...
interface PipeMethod<t, i> {
	<r>(fn: (data: t, ctx: NarrowContext) => r): FluentType<Exclude<r, OpErrors>, i>;
...
class Ctx implements NarrowContext {
	...
	error(input: string | NarrowErrorInput): OpErrors {
		const detail = typeof input === "string" ? { expected: input } : input;
		const error = OpErrors.single([...(detail.path ?? detail.relativePath ?? [])], detail.expected, this.#data, {
			preserveActual: true,
	...
	reject(input: string | NarrowErrorInput): OpErrors | false {
...
const kBase = Symbol("optype.base");
const kSteps = Symbol("optype.steps");
...
function invalidDefault(label: string, errors: OpErrors): never {
	...
	throw new OpTypeError(`ParseError: ${heading} ${error.problem}`);
}

function rejectMutableStaticDefault(value: unknown): void {
	if (value !== null && typeof value === "object" && !(value instanceof Date)) {
		throw new OpTypeError("ParseError: A mutable default value must be specified as a factory");
	}
}
	...
	switch (ir.k) {
		case "object":
			for (const prop of ir.props) {
				...
				if (output instanceof OpErrors) invalidDefault(`Default for ${String(prop.key)}`, output);
		...
		case "tuple":
			for (let index = 0; index < ir.prefix.length; index++) {
				...
				if (output instanceof OpErrors) invalidDefault(`Default for [${index}]`, output);
...
const typeMethods = {
	...
	default(this: InternalType, value: unknown): InternalType {
		...
		if (output instanceof OpErrors) invalidDefault("Default", output);
	...
	overlaps(this: InternalType, def: Def): boolean {
		try {
			...
			if (error instanceof OpTypeError) return false;
			throw error;
		}
	},
	...
	divisibleBy(this: InternalType, divisor: number): InternalType {
		if (this.ir.k !== "number") throw new OpTypeError(`cannot apply divisibility to ${this.ir.k}`);
		if (!Number.isFinite(divisor) || divisor === 0) throw new OpTypeError("divisor must be non-zero");
	...
	allows(this: InternalType, data: unknown): data is unknown {
		...
		if (out instanceof OpErrors) return false;
		for (const step of steps) {
	...
	assert(this: InternalType, data: unknown): unknown {
		...
		if (out instanceof OpErrors) throw new TraversalError(out);
		return out;
	},
	...
	get(this: InternalType): StandardSchemaV1.Props {
		...
		return {
			...
			validate: (value: unknown): StandardSchemaV1.Result<unknown> => {
				...
				return out instanceof OpErrors
	...
	const validate: Validator =
		steps.length === 0
			...
			: (data: unknown, contextPath: readonly PropertyKey[] = []): unknown => {
					...
					let out = base(data);
					if (out instanceof OpErrors) return out;
					for (const step of steps) {
						if (step.kind === "filter") continue;
						...
						if (step.kind === "narrow") {
							...
							try {
								...
								return OpErrors.single([], `valid (morph threw ${detail})`, out, errorConfig);
							}
							if (out instanceof OpErrors) {
...
function appendPipes(
	...
	for (const candidate of pipes) {
		...
		if (typeof candidate !== "function") throw new OpTypeError("pipe operands must be functions or Types");
		schema = makeType(
			schema.ir,
		...
		for (let leftIndex = 0; leftIndex < ir.members.length; leftIndex++) {
			...
			for (let rightIndex = leftIndex + 1; rightIndex < ir.members.length; rightIndex++) {
				...
				try {
					...
					if (error instanceof OpTypeError) continue;
					throw error;
				}
				throw new OpTypeError("an unordered union with overlapping morph inputs is indeterminate");
			}
		}
	}
	switch (ir.k) {
...
function getPathIR(ir: IR, requestedKey: unknown): IR {
	...
	if ((typeof key === "function" || (typeof key === "object" && key !== null)) && IR_BRAND in key) {
		...
		if (keySchema.ir.k === "symbol") key = Symbol.for("optype.index");
		else {
	...
	if (typeof key !== "string" && typeof key !== "number" && typeof key !== "symbol") {
		throw new OpTypeError(`get keys must be strings, numbers, or symbols`);
	}
	...
	if (ir.k === "array") {
		...
		if (!Number.isSafeInteger(index) || index < 0) throw new OpTypeError(`key ${String(key)} is not declared`);
	...
	if (ir.k === "tuple") {
		...
		if (ir.variadic === undefined) throw new OpTypeError(`key ${String(key)} is not declared`);
		return unionOf([{ k: "undefined" }, ir.variadic, ...ir.postfix]);
	}
	if (ir.k === "undefined") return ir;
	if (ir.k !== "object") throw new OpTypeError("get requires an object schema");
	...
	if (typeof key === "string") {
		...
		for (const index of ir.patternIndexes ?? []) {
			if (!(walk(index.key, key) instanceof OpErrors)) matches.push(index.val);
		}
	} else if (typeof key === "symbol" && ir.symbolIndex !== undefined) {
		matches.push(ir.symbolIndex);
	}
	if (matches.length === 0) throw new OpTypeError(`key ${String(key)} is not declared`);
...
function requireObject(ir: IR, operation: string): ObjectIR {
	if (ir.k !== "object") throw new OpTypeError(`${operation} requires an object schema`);
	return ir;
}
function selectObjectProps(ir: IR, keys: readonly PropertyKey[], keepSelected: boolean, operation: string): ObjectIR {
	...
	for (const key of selected) {
		if (!object.props.some(prop => prop.key === key)) throw new OpTypeError(`key ${String(key)} does not exist`);
	}
...
function propertyToIR(property: TypeProperty): PropIR {
	if (property.kind !== "required" && property.kind !== "optional") {
		throw new OpTypeError(`mapped property ${String(property.key)} has invalid kind`);
	}
	if (!(IR_BRAND in property.value)) {
		throw new OpTypeError(`mapped property ${String(property.key)} must contain a schema value`);
	}
...
function dateRefinement(
	...
	if (!Number.isFinite(timestamp)) throw new OpTypeError("date bound must be valid");
	if (!acceptsDateIR(schema.ir)) throw new OpTypeError("date bounds require a Date type");
...
function withShallowExtras(ir: IR, extras: ObjectIR["extras"]): IR {
	...
	throw new OpTypeError("onUndeclaredKey requires an object schema");
}
...
function intersectTuples(left: TupleIR, right: TupleIR): IR {
	...
	if (minimum > maximum) throw new OpTypeError("tuple length intersection is unsatisfiable");
	...
	for (let index = 0; index < prefixLength; index++) {
		...
		try {
			...
			if (required || !(error instanceof OpTypeError)) throw error;
			break;
		}
	}
	const variadic =
...
const kIntersections = Symbol("optype.intersections");
...
function intersectResolved(a: IR, b: IR): IR {
	if (a.k === "never" || b.k === "never") throw new OpTypeError("intersection with never is unsatisfiable");
	if (a.k === "unknown") return b;
	if (b.k === "unknown") return a;
	if (a === b) return a;
	if (a.k === "morph" && b.k === "morph") {
		if (a.fn !== b.fn || a.out !== b.out) {
			throw new OpTypeError("intersection of distinct morphs is indeterminate");
		}
	...
	if (a.k === "union" || b.k === "union") {
		...
		if (union === undefined) throw new OpTypeError("union intersection invariant failed");
		...
		for (const branch of branches) {
			try {
				...
				if (!(error instanceof OpTypeError)) throw error;
			}
		}
		if (members.length === 0) throw new OpTypeError("intersection has no satisfiable branches");
		return members.length === 1 ? members[0] : { k: "union", members };
	}
	if (a.k === "lit") {
		if (walk(b, a.v) instanceof OpErrors) throw new OpTypeError("literal is excluded by the intersection");
		return a;
	}
	...
	if (a.k === "string" && b.k === "string") {
		...
		if (min !== undefined && max !== undefined && min > max) {
			throw new OpTypeError("string length intersection is unsatisfiable");
		}
	...
	if (a.k === "number" && b.k === "number") {
		...
		if (min !== undefined && max !== undefined && (min > max || (min === max && (xmin || xmax)))) {
			throw new OpTypeError("numeric range intersection is unsatisfiable");
		}
	...
	if (a.k === "array" && b.k === "array") {
		...
		const max = minOf(a.max, b.max);
		if (min !== undefined && max !== undefined && min > max) {
			throw new OpTypeError("array length intersection is unsatisfiable");
		}
	...
	if (a.k === "instance" && b.k === "instance") {
		...
		throw new OpTypeError(`intersection of ${a.expected} and ${b.expected} is unsatisfiable`);
	}
	...
	if (leftDomain !== undefined && rightDomain !== undefined && leftDomain !== rightDomain) {
		throw new OpTypeError(`intersection of ${leftDomain} and ${rightDomain} is unsatisfiable`);
	}
...
function isSubtype(source: IR, target: IR, seen = new WeakMap<IR, Set<IR>>()): boolean {
	...
	if (source.k === "lit") return !(walk(target, source.v) instanceof OpErrors);
...
function withLengthBound(ir: IR, side: "min" | "max", bound: number): IR {
	...
	throw new OpTypeError(`cannot apply length bound to ${ir.k}`);
}

function withNumericBound(ir: IR, side: "min" | "max", bound: number, exclusive = false): IR {
	if (!Number.isFinite(bound)) throw new OpTypeError("numeric bound must be finite");
	if (ir.k === "number") {
	...
	throw new OpTypeError(`cannot apply numeric bound to ${ir.k}`);
}
...
const GENERIC_META = Symbol("optype.generic");
...
function validateGenericParameters(parameters: readonly GenericParameter[]): void {
	...
	for (const parameter of parameters) {
		if (!/^[A-Za-z_$]\w*$/.test(parameter.name)) {
			throw new OpTypeError(`invalid generic parameter "${parameter.name}"`);
		}
		if (names.has(parameter.name)) throw new OpTypeError(`duplicate generic parameter "${parameter.name}"`);
		names.add(parameter.name);
	}
	if (parameters.length === 0) throw new OpTypeError("generic declarations require at least one parameter");
}
...
function parseGenericArgument(definition: unknown, outer?: AliasResolver): IR {
	if (outer === undefined) {
		try {
			...
			if (!(error instanceof OpTypeError) || !error.message.includes('unknown keyword "this"')) throw error;
		}
	}
	...
	const self: IR = {
		...
		name: "this",
		resolve: () => {
			if (root === undefined || root === self) throw new OpTypeError('"this" cannot be used as a root definition');
			return root;
		},
	};
	...
	if (root === self) throw new OpTypeError('"this" cannot be used as a root definition');
	return root;
}
	...
	const meta: GenericMeta = {
		...
		instantiateIR(arguments_) {
			if (arguments_.length !== parameters.length) {
				throw new OpTypeError(`generic expects ${parameters.length} arguments (received ${arguments_.length})`);
			}
			for (let index = 0; index < parameters.length; index++) {
				...
				if (!isSubtype(arguments_[index], constraint)) {
					throw new OpTypeError(`${parameter.name} must be assignable to its constraint`);
				}
			}
...
): FluentType<Exclude<output, OpErrors>, input>;
...
function keywordSchema<output = string, input = output>(name: string): FluentType<output, input> {
	...
	if (ir === undefined) throw new OpTypeError(`missing built-in keyword ${name}`);
...
export type Matcher<input = unknown, output = unknown> = FluentType<output, input> &
	(<const value extends input>(value: value, ...args: readonly unknown[]) => output | OpErrors);
...
function caseResolver(value: unknown): (input: unknown, ...args: readonly unknown[]) => unknown {
	if (typeof value !== "function") throw new OpTypeError("match case values must be functions");
...
function publicMatcher<input, output>(
	...
	if (state.key === undefined || state.branches.length === 0) {
		...
		if (first.k !== "object") throw new OpTypeError("match.at cases must define object schemas");
		const propertyKey = String(state.key);
		const values: IR[] = [];
		for (const branch of state.branches) {
			if (branch.schema.ir.k !== "object") throw new OpTypeError("match.at cases must define object schemas");
			const property = branch.schema.ir.props.find(candidate => candidate.key === propertyKey);
			if (property === undefined) throw new OpTypeError(`match.at case is missing ${propertyKey}`);
	...
	const execute = (input: unknown, args: readonly unknown[]): unknown => {
		...
		if (state.input !== undefined) {
			...
			if (validated instanceof OpErrors) return validated;
			matchedInput = validated;
		}
		for (const branch of state.branches) {
			const matched = branch.schema.run(matchedInput);
			if (!(matched instanceof OpErrors)) return branch.resolve(matched, ...args);
		}
	...
	const callable = ((value: unknown, ...args: readonly unknown[]): unknown => {
		...
		if ((fallback === "assert" || fallback === "never") && result instanceof OpErrors) {
	...
	for (const rawDefinition of Reflect.ownKeys(cases)) {
		...
		if (rawDefinition === "default") {
			if (value !== "assert" && value !== "never" && value !== "reject" && typeof value !== "function") {
				throw new OpTypeError('match default must be "assert", "never", "reject" or a function');
			}
...
function createMatchParser(state: MatchState): MatchParser {
	...
	function at(key: PropertyKey, cases?: Record<PropertyKey, unknown>): unknown {
		if (state.key !== undefined) throw new OpTypeError("match.at may only be specified once");
		const next = createMatchParser({ ...state, key });
		return cases === undefined ? next : next.match(cases);
	}
	parser.at = at;
	parser.strings = cases => {
		if (state.key === undefined) throw new OpTypeError("match.strings requires match.at(key)");
	...
	parser.in = (...args: unknown[]) => {
		if (state.input !== undefined) throw new OpTypeError("match.in may only be specified once");
		return createMatchParser({
			...state,
	...
	function parser(...definitions: readonly unknown[]): unknown {
		...
		if (spreadIndexes.length > 1) {
			...
			if (
				...
				throw new OpTypeError("An optional element may not follow a variadic element");
			}
			throw new OpTypeError("A tuple may have at most one variadic element");
		}
		if (spreadIndexes.length === 1 && spreadIndexes[0] + 2 < (marker === -1 ? definitions.length : marker)) {
			...
			if (
				...
				throw new OpTypeError("A postfix required element cannot follow an optional or defaultable element");
			}
		}
		...
		return (implementation: (...arguments_: readonly unknown[]) => unknown) => {
			if (typeof implementation !== "function") throw new OpTypeError("type.fn requires a function implementation");
...
function requireNaryObject(ir: IR): ObjectIR {
	...
	if (resolved.k !== "object") throw new OpTypeError("merge requires an object schema");
	return resolved;
}
...
export namespace type {
	...
	export const errors = OpErrors;
	export type errors = OpErrors;
	...
	export const keywords = {
		...
		> {
			...
			if (keyIR.k !== "string" && keyIR.k !== "symbol") {
				throw new OpTypeError("Record key must be assignable to string or symbol");
			}
	...
	export function instanceOf<const ctor extends Constructor>(ctor: ctor): FluentType<Constructed<ctor>> {
		if (typeof ctor !== "function" || ctor.prototype === undefined) {
			throw new OpTypeError("instanceof operands must be constructors");
		}
	...
	export function generic(...arguments_: readonly (GenericParameterSpec | unknown)[]): Generic | GenericBuilder {
		...
		const parameters: GenericParameter[] = arguments_.map(parameter => {
			...
			throw new OpTypeError("generic parameters must be names or [name, constraint] pairs");
		});
		validateGenericParameters(parameters);
	...
	export function withJsonSchema<t, i = t>(schema: Type<t, i>, json: Record<string, unknown>): Type<t, i> {
		...
		if (internal.hasDefault || hasMorph(internal.ir) || internal[kSteps].some(step => step.kind === "pipe")) {
			throw new OpTypeError("type.withJsonSchema cannot wrap schemas with defaults or output-changing morphs");
		}
		return makeType<t, i>(
			{
				k: "refine",
				...
				pred: value => {
					...
					return result instanceof OpErrors ? result : true;
				},
...
const MODULE_SCOPE = Symbol("optype.moduleScope");
...
function buildScope(aliases: Record<string, unknown>, options?: ScopeOptions): TypeScope {
	...
	for (const sourceName in aliases) {
		...
		if (entries.has(name)) throw new OpTypeError(`alias "${name}" is declared as both public and private`);
		entries.set(name, {
			name,
			sourceName,
			private: isPrivate,
	...
	const genericFor = (entry: ScopeAlias): RuntimeGeneric => {
		...
		if (parameters === undefined) throw new OpTypeError(`alias "${entry.name}" is not generic`);
	...
	const targetFor = (name: string): IR => {
		...
		if (resolved === undefined) throw new OpTypeError(`unknown alias "${name}"`);
		...
		import(...names: readonly string[]) {
			const selected =
				names.length === 0
					...
					: names.map(name => {
							const entry = entries.get(name);
							if (entry === undefined) throw new OpTypeError(`unknown alias "${name}"`);
							return entry;
						});
...
/**
 * Minimal structural constraint matching any optype schema.
 *
