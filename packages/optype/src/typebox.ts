import { OpTypeError } from "./errors";
import type { Def, EmbeddableSchema } from "./ir";
import { type NarrowContext, type Type as OpType, type ToJsonSchemaOptions, type } from "./type";
...
const OPTIONAL_INNER = Symbol("optype.typebox.optionalInner");
const OBJECT_INFO = Symbol("optype.typebox.objectInfo");
 ...
 * optype schemas included (those carry no legacy compat members).
 *
...
export type TTyped<T> = OpType<T> & LegacyTypeBoxCompat<T>;
...
interface RuntimeType<T> extends OpType<T> {
...
function withLegacyCompat<T>(schema: OpType<T>): CompatRuntime<T> {
...
function checkFiniteOption(name: string, value: number | undefined): void {
	if (value !== undefined && !Number.isFinite(value)) throw new OpTypeError(`${name} must be finite`);
}
...
function tString(opts?: StringOpts): TString {
	...
	if (opts?.pattern !== undefined) {
		...
		try {
			...
			throw new OpTypeError(`invalid regular expression pattern ${JSON.stringify(opts.pattern)}`);
		}
...
function tNumber(opts?: NumberOpts, integer = false): TNumber {
	...
	if (opts?.multipleOf !== undefined && opts.multipleOf <= 0)
		throw new OpTypeError("multipleOf must be greater than zero");
...
function tIntersect<const E extends readonly AnySchema[]>(
	...
	try {
		...
		if (error instanceof OpTypeError) return applyMeta(validateAll(), opts);
		throw error;
	}
...
function tObject<const P extends Record<string, AnySchema>>(properties: P, opts?: ObjectOpts): TObject<P> {
	...
	for (const key in properties) {
		...
		// optype (like ArkType) rejects `key?` with a default, and a default
...
function requireObject(schema: AnySchema, operation: string): ObjectInfo {
	...
	if (!info) throw new OpTypeError(`Type.${operation} requires a schema created by Type.Object`);
	return info;
}
...
function tUnsafe<T = unknown>(_jsonSchema: Record<string, unknown> = {}): TUnsafe<T> {
	...
	// optype cannot honestly implement that contract without importing a second validator.
	return withLegacyCompat(type.unknown as OpType<T>);
}
