 ...
 * - failure returns an `OpErrors` with a single fast-fail entry
 */
import { MISSING, OpErrors } from "./errors";
...
/**
 * Validate `value` against `ir`; returns output value or `OpErrors`.
...
function fail(path: PropertyKey[], expected: string, data: unknown): OpErrors {
	const storedPath = path.length === 0 ? undefined : path.length === 1 ? path[0] : [...path];
	return new OpErrors(storedPath, expected, data);
}
...
function checkNode(ir: IR, v: unknown): boolean {
	switch (ir.k) {
		...
		case "sub":
			return !(ir.schema.run(v) instanceof OpErrors);
	}
}
...
function visitFinish(ir: IR, v: unknown, path: PropertyKey[]): unknown {
	...
	if (!(out instanceof OpErrors) || ir.cfg === undefined) return out;
...
function visitNode(ir: IR, v: unknown, path: PropertyKey[]): unknown {
	switch (ir.k) {
		...
		case "refine": {
			...
			if (base instanceof OpErrors) return base;
			try {
				const result = ir.pred(base);
				if (result instanceof OpErrors) return path.length === 0 ? result : prefixAll(result, path);
		...
		case "morph": {
			...
			if (input instanceof OpErrors) return input;
			const context = {
			...
			if (output instanceof OpErrors) return output;
		...
		case "union": {
			...
			let targeted: OpErrors | undefined;
			let targetCount = 0;
			for (const member of ir.members) {
		...
		case "array": {
			...
			for (let index = 0; index < v.length; index++) {
				...
				path.pop();
				if (element instanceof OpErrors) {
			...
			for (let index = prefixCount; index < ir.prefix.length; index++) {
				...
				if (item.hasDefault && morph) {
					...
					if (item.defFactory && typeof payload === "function") {
						...
						path.pop();
						if (resolved instanceof OpErrors) {
...
function prefixAll(errs: OpErrors, path: PropertyKey[]): OpErrors {
 ...
 * "A or B" expectation.
 */
export function unionFail(ir: IR & { k: "union" }, v: unknown, path: PropertyKey[], expected?: string): OpErrors {
	...
	if (ir.members.every(member => unwrapBase(member).k === "object")) {
		const branches = ir.members.flatMap(member => {
			...
			return result instanceof OpErrors ? [[...result]] : [];
		});
		if (branches.length !== 0) {
			...
			if (common.length !== 0) {
				for (const entry of common) {
					...
					alternatives.push(
						new OpErrors(entry.path, [...expectations].join(" or "), entry.data, { preserveActual: true }),
					);
				}
			} else {
				for (const branch of branches) {
...
function discriminateFailure(members: IR[], value: unknown, path: PropertyKey[]): OpErrors | undefined {
	...
	for (const candidate of candidates) {
		...
		if (exact.length === 1) {
			...
			return result instanceof OpErrors ? result : undefined;
		}
