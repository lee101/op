import { type OpErrors, OpTypeError } from "./errors";
...
interface Decoratable<out Out> extends EmbeddableSchema {
	(value: unknown): Out | OpErrors;
	narrow(predicate: (value: Out, context: NarrowContext) => unknown): Decoratable<Out>;
	pipe<Next>(transform: (value: Out, context: NarrowContext) => Next): Decoratable<Exclude<Next, OpErrors>>;
...
/** A callable optype schema carrying the Zod-v4-style fluent surface. */
...
function lengthBound(kind: "min" | "max", schema: Decoratable<unknown>, bound: number): void {
	...
	if (!Number.isSafeInteger(bound) || bound < 0) {
		throw new OpTypeError(`${kind} length must be a nonnegative safe integer`);
	}
}
...
function decorate<Out>(schema: Decoratable<Out>, optional = false): ZodLikeSchema<Out> {
	...
	const withObjectExtras = (extras: "keep" | "reject" | "delete"): ZodLikeSchema<Out> => {
		if (schema.ir.k !== "object") throw new OpTypeError("object mode requires an object schema");
	...
	return Object.assign(schema, {
		...
		min(bound: number): ZodLikeSchema<Out> {
			...
			if (ir.k === "number") {
				if (Number.isNaN(bound)) throw new OpTypeError("number min must not be NaN");
			...
			throw new OpTypeError(`cannot apply min to ${ir.k}`);
		},
		max(bound: number): ZodLikeSchema<Out> {
			const ir = schema.ir;
			if (ir.k === "string" || ir.k === "array") {
			...
			if (ir.k === "number") {
				if (Number.isNaN(bound)) throw new OpTypeError("number max must not be NaN");
			...
			throw new OpTypeError(`cannot apply max to ${ir.k}`);
		},
		int(): ZodLikeSchema<Out> {
			if (schema.ir.k !== "number") throw new OpTypeError(`cannot apply int to ${schema.ir.k}`);
			return next(restrictBase(schema, { ...schema.ir, int: true }));
		},
		positive(): ZodLikeSchema<Out> {
			if (schema.ir.k !== "number") throw new OpTypeError(`cannot apply positive to ${schema.ir.k}`);
			const ir = schema.ir;
			if (ir.min !== undefined && ir.min > 0) return next(restrictBase(schema, ir));
			return next(restrictBase(schema, { ...ir, min: 0, xmin: true }));
		},
		nonnegative(): ZodLikeSchema<Out> {
			if (schema.ir.k !== "number") throw new OpTypeError(`cannot apply nonnegative to ${schema.ir.k}`);
			return this.min(0);
		},
		regex(expression: RegExp, message?: string): ZodLikeSchema<Out> {
			if (schema.ir.k !== "string") throw new OpTypeError(`cannot apply regex to ${schema.ir.k}`);
		...
		url(): ZodLikeSchema<Out> {
			if (schema.ir.k !== "string") throw new OpTypeError(`cannot apply url to ${schema.ir.k}`);
		...
		partial(): Out extends object ? ZodLikeSchema<Partial<Out>> : ZodLikeSchema<Out> {
			if (schema.ir.k !== "object") throw new OpTypeError(`cannot apply partial to ${schema.ir.k}`);
...
const enumSchema = <const Values extends readonly [string, ...string[]]>(
	...
	if (values.length === 0) throw new OpTypeError("enum requires at least one value");
...
export const record = <Key extends string, Value>(
	...
	if (!isStringKeyIR(keySchema.ir)) throw new OpTypeError("record keys must use a string schema");
