...
```sh
npm install @openpaths/optype
# or
bun add @openpaths/optype
```
...
Failed validation returns `OpErrors`; each entry exposes `code`, `path`,
...
TypeBox-style and Zod-style builders produce native optype schemas:
...
`@openpaths/optype/ark` provides the repository's ArkType compatibility facade
...
```sh
bun packages/optype/bench/bench.ts
```
...
candidate was than optype in this run. Results vary with hardware, runtime,
