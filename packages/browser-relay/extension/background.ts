/**
 * OP Browser Relay — MV3 service worker.
 *
...
/** Title of the op tab group; mirrored to session storage so a restarted service worker can still dissolve it. */
 ...
 * duplicate "op" groups in the same window.
 */
...
/** Move tabs into the per-window op group, creating or reusing it by title. */
...
/** Dissolve every op-titled group (relay disconnected or asked us to release tabs). */
...
chrome.alarms.create("op-relay-keepalive", { periodInMinutes: 0.5 });
chrome.alarms.onAlarm.addListener(alarm => {
	if (alarm.name === "op-relay-keepalive") void connect();
});
