/**
 * Regression for https://github.com/lee101/op/issues/823.
 *
 * On WSL (and any host where the user moves the standalone binary away from the
 * build-time native artifacts), the compiled `op` binary fails to load
...
describe("issue 823: standalone-binary native loader path resolution", () => {
	...
	it("places embedded-extracted candidates ahead of build-host candidates for linux-x64 standalone", () => {
		...
		// these are where the embedded-addon extraction lands (~/.op/natives/<v>) and where
		// `op update` writes the standalone binary on linux (~/.local/bin).
	...
	it("prefers platform leaf package candidates ahead of core nativeDir candidates on npm installs", () => {
		...
		const candidates = resolveLoaderCandidates({
			...
			versionedDir: "/home/u/.op/natives/15.5.15",
	...
	it("keeps Windows staging ahead of leaf package and core nativeDir candidates", () => {
		const versionedDir = "/home/u/.op/natives/15.5.15";
		const leafPackageDir = "/app/node_modules/@openpaths/natives-win32-x64";
		const nativeDir = "/app/node_modules/@openpaths/natives/native";
		const candidates = resolveLoaderCandidates({
