/**
 * Regression for the Windows `bun install -g` update path: when an `op`
 * process is running, bun cannot overwrite a locked
 * `node_modules/@openpaths/natives/native/pi_natives.win32-x64.node` during
 ...
 *      (per-package-version cache under `~/.op/natives/<version>/`) so the
...
const winNodeModulesNativeDir = "C:\\Users\\Admin\\node_modules\\@openpaths\\pi-natives\\native";
const winWorkspaceNativeDir = "C:\\Users\\Admin\\dev\\openpaths\\packages\\natives\\native";
const posixNodeModulesNativeDir = "/home/u/proj/node_modules/@openpaths/natives/native";
...
describe("windows native addon staging", () => {
	...
	it("prepends versionedDir candidates ahead of node_modules when staging on Windows", () => {
		const versionedDir = "C:\\Users\\Admin\\.op\\natives\\15.0.1";
		const userDataDir = "C:\\Users\\Admin\\AppData\\Local\\op";
	...
	it("classifies only Windows node_modules paths case-insensitively", () => {
		const leafPackageDir = "/tmp/node_modules/@openpaths/natives-darwin-arm64";
		const uppercaseNodeModulesNativeDir = "/tmp/NODE_MODULES/@openpaths/natives/native";
		...
		try {
			const workspace = initLoaderContext({
				platform: "linux",
				isCompiledBinary: false,
				nativeDir: "/tmp/openpaths/packages/natives/native",
				leafPackageDir,
			});
			const installed = initLoaderContext({
				platform: "linux",
				isCompiledBinary: false,
				nativeDir: "/tmp/node_modules/@openpaths/natives/native",
				leafPackageDir,
			});
	...
	it("falls back to the node_modules-only candidate list when staging is off", () => {
		...
		const versionedDir = "/home/u/.op/natives/15.0.1";
		const candidates = resolveLoaderCandidates({
	...
	it("removes only older version directories after the current native version loads", async () => {
		const nativesDir = await fs.mkdtemp(path.join(os.tmpdir(), "op-natives-cache-"));
