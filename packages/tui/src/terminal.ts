...
export function emergencyTerminalRestore(): void {
	try {
		...
		if (terminal) {
			...
			// frame before leaving it, then let stop() pop op's main-screen frame.
			if (altScreenActive) {
				const keyboardExit =
...
export class ProcessTerminal implements Terminal {
	...
	start(onInput: (data: string) => void, onResize: () => void, onDisconnect?: () => void): void {
		...
		// `rmkx` = "\x1b[?1l\x1b>"). op decodes both CSI ("\x1b[A") and SS3
		...
		// are disabled while OP owns the TTY so typing in the editor does not
	...
	#queryOsc99Support(): void {
		...
		const id = `op-probe-${nextOsc99ProbeId++}`;
	...
	stop(): void {
		...
		// navigation stays broken after op exits (#6374).
		this.#safeWrite("\x1b[?1l\x1b>");
