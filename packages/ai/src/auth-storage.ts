...
import { $env, $envExact, extractRetryHint, getAgentDbPath, logger } from "@openpaths/utils";
import {
 ...
 * accounts (e.g. an expired Anthropic OAuth grant) stay visible in `op usage`
...
export type AuthStorageOptions = {
	 ...
	 *
	 * Examples:
	 * - `"local ~/.op/agent/agent.db"`
	 * - `"broker http://op.internal:8765"`
	 */
	sourceLabel?: string;
	/**
...
const USAGE_REPORT_CACHE_KEY_VERSION_OVERRIDES: Partial<Record<Provider, number>> = {
	...
	zai: 2,
	// v2: retires cached reports from the OP-observed spend estimator (dollar
...
// `@openpaths/ai` entrypoint and the in-module call sites below.
...
export class AuthStorage {
	...
	/**
	 * Disabled credential tombstones for display surfaces (`op usage`,
	 ...
	 * per-credential data with stored identities (`op usage`) use this so a
