 ...
 * pure protocol translation: foreign wire → op Context → pi-ai stream() →
 * op events → foreign wire.
 *
 * Endpoints:
...
function deriveSessionId(modelId: string, context: Context): string {
	...
	if (first) {
		...
		// of the same conversation (op re-stamps every parsed Message). role +
