...
import { getAgentDbPath, getDbBusyTimeoutMs, logger } from "@openpaths/utils";
import type {
	AuthCredential,
	AuthCredentialStore,
...
export class SqliteAuthCredentialStore implements AuthCredentialStore {
	...
	static async open(dbPath: string = getAgentDbPath()): Promise<SqliteAuthCredentialStore> {
		...
		// Concurrent op startups can race against WAL recovery and the schema
	 ...
	 * the connection: concurrent op startups race WAL recovery and the leases
