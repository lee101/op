 ...
 * gateway translation. Inbound: foreign HTTP body → op Context. Outbound:
 * op AssistantMessage[Stream] → Anthropic-shaped JSON / SSE.
 */
		...
		switch (block.type) {
			...
			case "tool_search_tool_result":
				if (isAnthropicServerToolHistoryBlock(block)) {
					...
					// Other server tools use distinct result block types that op
