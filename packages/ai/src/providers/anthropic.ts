...
import { isAnthropicSigningProxyUrl, isOfficialAnthropicApiUrl } from "@openpaths/catalog/compat/anthropic";
import { hostMatchesUrl, isVertexRawPredictUrl } from "@openpaths/catalog/hosts";
import { mapEffortToAnthropicAdaptiveEffort } from "@openpaths/catalog/model-thinking";
import { calculateCost, getBundledModel } from "@openpaths/catalog/models";
import { isAnthropicOAuthToken } from "@openpaths/catalog/utils";
import { parseGitHubCopilotApiKey } from "@openpaths/catalog/wire/github-copilot";
import {
	$env,
	getInstallId,
	isEnoent,
...
const CLAUDE_DEVICE_ID_INSTALL_HASH_DOMAIN = "op-claude-device-id-v1:";
const CLAUDE_DEVICE_ID_ACCOUNT_HASH_DOMAIN = "op-claude-device-id-v2";
...
export function maybeAddReplayUnsignedThinkingHint(model: Model<"anthropic-messages">, message: string): string {
	...
	const hint = `Provider "${model.provider}" looks like an Anthropic-compatible signing proxy: it rejected a replayed unsigned thinking block. Set \`compat.replayUnsignedThinking: false\` under \`providers.${model.provider}\` in your models.yml and retry. See https://github.com/lee101/op/issues/4297.`;
	return `${hint}\n\n${message}`;
}
...
/**
 * Serialize op {@link Message}s to Anthropic wire messages.
 *
