...
} from "@openpaths/catalog/discovery/gitlab-duo-workflow";
 ...
 * keepalive contract OP can rely on, so a connection silently going half-open
 ...
 * reports its per-workflow step (graph-recursion) limit. Long OP tool-call loops
...
const GITLAB_DUO_WORKFLOW_AVAILABLE_MODELS_QUERY = `query op_gitlabDuoWorkflowAvailableModels($rootNamespaceId: GroupID!) {
...
const GITLAB_DUO_WORKFLOW_INLINE_AGENT_NAME = "op_agent";
const GITLAB_DUO_WORKFLOW_INLINE_PROMPT_ID = "op_inline_prompt";
...
// whose system slot carries OP's own authoritative system prompt (no GitLab jinja
...
function createGitLabDuoWorkflowProviderSessionState(): GitLabDuoWorkflowProviderSessionState {
	const state: GitLabDuoWorkflowProviderSessionState = {
		close: () => {
			...
			// PATCH a workflow the server is still running on OP would be stranded.
			try {
				state.active?.stop?.();
			} catch {
...
async function runGitLabDuoWorkflow(
	...
	const setupForNamespace = async (
		...
		// The inline `ambient` flow fails server-side without a project, and OP has
	...
	try {
		for (let attempt = 0; attempt < 12; attempt++) {
			...
			// A long but healthy OP tool-call loop legitimately overruns it; that is
...
// OP has no GitLab project of its own, but the inline `ambient` flow fails
		...
		void (async () => {
			...
			while (!settled) {
				...
				if (!handleSocketResult(result, data, pending)) {
					...
					// to OP. Clear the pause flag first: the live `onmessage` handler must
...
async function handleGitLabDuoWorkflowSocketMessage(
	...
	if (status === "FAILED" || status === "STOPPED") {
		...
		// RECURSION_LIMIT). A long but healthy OP tool-call loop legitimately hits
...
function buildGitLabMcpToolDefinition(tool: Tool): GitLabMcpToolDefinition {
	...
	// the name the model sees, the toolset key it is matched against, and OP's own
	// tool docs must all be the same bare name. A prefixed wire name only forced the
	// model to learn `mcp__omp__read` while OP docs say `read`, with no upside.
...
// The OP system prompt that rides the inline flow's `prompt_template.system` slot.
// DWS wraps it in its own gateway boilerplate, but the slot content is delivered to
// the model verbatim, so OP's authoritative rules go here directly — no redirect
...
function renderGitLabDuoWorkflowChatMlToolCall(toolCall: GitLabDuoWorkflowReplayToolCall): string {
	...
	// call). The call id is OP-internal wiring the model never reads (call→result pair
...
// The `i` key is OP's per-call intent narration (e.g. "Reading kernel smoke body").
...
// Every workflow definition OP ships is the inline ambient flow (Path B /
