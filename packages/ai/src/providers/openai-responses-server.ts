/**
 * OpenAI Responses HTTP wire-format ↔ op Context bridge for the auth-gateway.
 *
 * Inbound: parses `POST /v1/responses` request bodies into a {@link ParsedRequest}.
 * Outbound: encodes op's {@link AssistantMessage} (and event stream) back into
...
import { logger, structuredCloneJSON } from "@openpaths/utils";
...
export function parseRequest(body: unknown, headers?: Headers): ParsedRequest {
	...
	// `store` is a stateful-storage hint that op's gateway doesn't honour;
					...
					switch (ev.type) {
						...
						case "toolcall_end": {
							...
							if (cur.customWireName) {
								...
								// Standard JSON tool: arguments object on the op side, the
