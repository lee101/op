...
import { toFirepassWireModelId, toFireworksWireModelId } from "@openpaths/catalog/fireworks-model-id";
import { isGlm52ReasoningEffortModelId, isKimiK3ModelId } from "@openpaths/catalog/identity";
...
import { parseAlibabaTokenPlanCredential } from "@openpaths/catalog/wire/alibaba-token-plan";
import {
	COREWEAVE_PROJECT_HEADER,
	coreWeaveProjectHeaders,
	hasCoreWeaveProjectHeader,
	removeBlankCoreWeaveProjectHeaders,
} from "@openpaths/catalog/wire/coreweave";
...
export function resolveOpenAIRequestSetup(
	...
	// Attribute xAI traffic as op unless a User-Agent is already set.
 ...
 * OpenRouter's private `secret/*` and `stealth/*` model ids do not support routing variants.
 */
export function applyOpenRouterRoutingVariant(modelId: string, variant: string | undefined): string {
	...
	if (normalizedModelId.startsWith("secret/") || normalizedModelId.startsWith("stealth/")) return modelId;
 ...
 * Built once per request; `apply_patch` → `edit` is the OP default.
 */
...
export async function processResponsesStream<TApi extends Api>(
	...
	// see https://github.com/lee101/op/issues/1880 — llama.cpp emits parallel
