...
/** Web-search server-tool call whose matching result is replayable by op. */
export type WebSearchServerToolUseBlockParam = ServerToolUseBlockParam & { name: "web_search" };

/** Tool-search server-tool call whose matching result is replayable by op. */
...
/** Anthropic server-tool history variants op can replay atomically. */
...
/** True when a block is complete Anthropic server-tool history op can replay. */
