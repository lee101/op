...
import { getInstallId, logger } from "@openpaths/utils";
import {
	type AuthCredential,
...
export class RemoteAuthCredentialStore implements AuthCredentialStore {
	...
	replaceAuthCredentialsForProvider(_provider: string, _credentials: AuthCredential[]): StoredAuthCredential[] {
		throw new AIError.AuthBrokerError(
			"RemoteAuthCredentialStore is read-only on the client. Use `op auth-broker login <provider>` to mutate credentials.",
		);
	}
	...
	deleteAuthCredentialsForProvider(_provider: string, _disabledCause: string): void {
		throw new AIError.AuthBrokerError(
			"RemoteAuthCredentialStore is read-only on the client. Use `op auth-broker logout <provider>` to mutate credentials.",
		);
	}
