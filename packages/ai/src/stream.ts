...
import { isOfficialAnthropicApiUrl } from "@openpaths/catalog/compat/anthropic";
import type { Effort } from "@openpaths/catalog/effort";
import { isVertexExpressOpenAIUrl, isVertexRawPredictUrl, resolveVertexEndpointHost } from "@openpaths/catalog/hosts";
import {
	...
	resolveWireModelId,
} from "@openpaths/catalog/model-thinking";
import { CATALOG_PROVIDERS, type ProviderCatalogEntry } from "@openpaths/catalog/provider-models";
import { CODEX_BASE_URL } from "@openpaths/catalog/wire/codex";
import { $env, $pickenv, getProviderInFlightRoot, isEnoent, logger, withExtraCaFetch } from "@openpaths/utils";
 ...
 * Used by `op auth-broker migrate --include-env` to discover env-sourced keys
		...
		case "openrouter": {
			...
			if (useResponses) {
				return castApi<"openai-responses">({
					...
					openrouterSort: options?.openrouterSort,
