...
import {
	...
	type ToolCallState,
} from "@openpaths/ai/providers/cursor";
import type { AssistantMessage, CursorExecHandlers, ToolResultMessage } from "@openpaths/ai/types";
import { kCursorExecResolved, setStreamingPartialJson } from "@openpaths/ai/utils/block-symbols";
					...
					args: create(ConnectScmArgsSchema, {
						...
						target: {
							...
							value: create(ConnectScmGithubSchema, {
								repository: create(ConnectScmGithubRepositorySchema, { owner: "can1357", repo: "openpaths" }),
							}),
						},
					}),
				}),
...
describe("Cursor modern exec frames: server-resolved tool calls leave a paired block", () => {
	...
	it("reads the connectScm repository out of the target oneof, not a flat field", async () => {
		const { output, results } = runConnectScm(
			{ toolCallId: "call-scm-1", repository: { owner: "can1357", repo: "openpaths" } },
			{ case: "success", value: {} },
		);
		...
		expect(block).toMatchObject({
			id: "call-scm-1",
			name: "connect_scm",
			arguments: { owner: "can1357", repo: "openpaths" },
		});
