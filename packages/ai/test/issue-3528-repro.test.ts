 ...
 *
 * `op-llm-request-15179edfab4dc557.json` plus the rr-session captures from the
 * reporter showed:
 *
 ...
 * thinking output). The OP-side `Context` preserved those as a
...
import { renderDemotedThinking } from "@openpaths/ai/dialect";
import { convertMessages } from "@openpaths/ai/providers/openai-completions";
import {
	applyChatCompletionsReasoningParams,
	type OpenAICompletionsParams,
} from "@openpaths/ai/providers/openai-shared";
import type { AssistantMessage, Message, Model, ModelSpec, ThinkingContent, UserMessage } from "@openpaths/ai/types";
...
describe("llama.cpp warm-prefix preservation (#3528)", () => {
	...
	it("leaves qwenPreserveThinking off for cloud Qwen hosts", () => {
		...
		// cloud side; OP isn't responsible for KV-cache invalidation there,
