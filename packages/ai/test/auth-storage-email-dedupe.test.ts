...
import { AuthStorage, type FetchImpl, type OAuthCredential, SqliteAuthCredentialStore } from "@openpaths/ai";
...
describe("AuthStorage OAuth login upgrade and multi-account coexistence", () => {
	...
	beforeEach(async () => {
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "op-auth-login-test-"));
	});
...
describe("AuthStorage persistent session stickiness", () => {
	...
	beforeEach(async () => {
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "op-auth-test-"));
