...
import type { Context, FetchImpl, Model, ModelSpec } from "@openpaths/ai/types";
...
describe("resolveOpenAIRequestSetup User-Agent", () => {
	test("sets op User-Agent on xAI when none is provided", () => {
...
describe("xAI stream User-Agent", () => {
	test("xAI Responses POST sends op User-Agent", async () => {
		...
		expect(captured.userAgent).toMatch(/^op\/\d+\.\d+\.\d+$/);
	});

	test("xAI OAuth Responses POST sends op User-Agent", async () => {
		const captured = await captureStreamHeaders(
	...
	test("OpenAI Completions POST does not send op User-Agent", async () => {
		const captured = await captureStreamHeaders(
