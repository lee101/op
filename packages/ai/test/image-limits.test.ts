...
import { complete } from "@openpaths/ai/stream";
import type { Api, Context, ImageContent, Model, OptionsForApi, UserMessage } from "@openpaths/ai/types";
import { getBundledModel } from "@openpaths/catalog/models";
import { $which, removeSyncWithRetries } from "@openpaths/utils";
import { e2eApiKey } from "./oauth";

const TEMP_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "op-temp-images-"));
