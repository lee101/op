...
} from "@openpaths/ai/providers/gitlab-duo-workflow";
import { configureCredentialRedaction } from "@openpaths/ai/providers/transform-messages";
import type {
	AssistantMessage,
	Context,
	FetchImpl,
...
import { extractHttpStatusFromError } from "@openpaths/utils";
...
describe("GitLab Duo Workflow provider protocol", () => {
	...
	it("advertises OP tools under their bare names with the official GitLab MCP schema", () => {
		...
		// bare name OP's own tool docs use.
	...
	it("puts the OP system prompt in the inline flow system slot with reasoning events", () => {
		const systemContext: Context = {
			systemPrompt: ["OP authoritative operating rules. Bridge the local tools."],
		...
		// The system slot carries OP's real system prompt verbatim — no gateway preamble.
		expect(prompt?.prompt_template.system).toContain("OP authoritative operating rules.");
	...
	it("builds startRequest goal as a bare ChatML transcript with tool-run linkage", () => {
		...
		const replayContext: Context = {
			systemPrompt: [`OP system instructions: preserve the local tool bridge. token ${patToken}`],
			messages: [
				{
					role: "user",
		...
		// no <instructions>. The OP system prompt rides the flow config's system slot.
		...
		expect(payload.goal).not.toContain("OP system instructions: preserve the local tool bridge.");
		...
		// rides the very next turn), so the OP-internal call id is omitted from the
		...
		// The OP system prompt lives in the flow config system slot, not the goal.
		const flowPrompt = payload.flowConfig?.prompts[0];
		expect(flowPrompt?.prompt_template.system).toContain("OP system instructions: preserve the local tool bridge.");
	...
	it("strips the OP-internal intent (i) field from replayed tool-call args", () => {
		const replayContext: Context = {
			systemPrompt: ["system"],
			messages: [
...
describe("GitLab Duo Workflow WebSocket state machine", () => {
	...
	it("re-seeds a fresh workflow goal with the entire conversation history including prior tool results", async () => {
		...
		// by its tool-result turn); the OP-internal id is omitted from the transcript.
