...
import { AuthStorage, REMOTE_REFRESH_SENTINEL, SqliteAuthCredentialStore } from "@openpaths/ai";
import {
	AuthBrokerClient,
...
describe("RemoteAuthCredentialStore SSE integration", () => {
	...
	test("loads the account pool once for broker-backed discovery", async () => {
		...
		await withEnv(
			{
				OP_AUTH_BROKER_URL: handle!.url,
				OP_AUTH_BROKER_TOKEN: token,
		...
		await withEnv(
			{
				OP_AUTH_BROKER_URL: handle!.url,
				OP_AUTH_BROKER_TOKEN: token,
				OP_AUTH_BROKER_ACCOUNT_POOL_FILE: path.join(tempDir, "missing-account-pool.json"),
			},
			async () => {
				const discovered = await discoverAuthStorage({
					agentDir: tempDir,
