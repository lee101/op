 ...
 *     → openai-responses parser → op Context
 ...
 * (override via `OP_E2E_GATEWAY_URL`) AND the bearer token file exists at
 * `~/.op/auth-gateway.token`.
 *
 * To run: `bun --cwd packages/ai test test/auth-gateway-cross-protocol-caching.test.ts`
 * with the gateway live (`op auth-gateway serve` or pm2).
 */
...
const MODEL = Bun.env.OP_E2E_ANTHROPIC_MODEL ?? "claude-sonnet-4-5";
...
the op auth-gateway's cross-protocol prompt-caching pipeline. The request
