...
import type { AgentMessage, ResolvedThinkingLevel, ThinkingLevel } from "@openpaths/agent-core";
import type { Model, ToolExample } from "@openpaths/ai";
import { formatSessionDumpText, RpcClient } from "@openpaths/coding-agent";
import { prompt } from "@openpaths/utils";
import { diffLines } from "diff";
import { formatDirectory } from "@openpaths/typescript-edit-benchmark/formatter";
import {
	discoverSharedInfra,
	InProcessClient,
	type SharedInfra,
} from "@openpaths/typescript-edit-benchmark/in-process-client";
...
import type { EditTask } from "@openpaths/typescript-edit-benchmark/tasks";
import {
	verifyExpectedFileSubset,
	verifyExpectedFiles,
} from "@openpaths/typescript-edit-benchmark/verify";

const REPO_ROOT = path.resolve(import.meta.dir, "..", "..", "..", "..");
const RUNS_DIR = path.join(REPO_ROOT, "runs");
const TMP = path.join(RUNS_DIR, `rb-${Math.random().toString(36).slice(2, 10)}`);
const CLI_PATH = Bun.fileURLToPath(import.meta.resolve("@openpaths/coding-agent/cli"));
