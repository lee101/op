 ...
 * terminal-bench-2) using a custom agent (`agent/op_local.py`) that installs
...
/**
 * Resolver injected into Apple Container runs (OP_BENCH_CONTAINER_DNS overrides).
 ...
 */
const CONTAINER_DNS = process.env.OP_BENCH_CONTAINER_DNS || "1.1.1.1";
...
const HELP = `metaharness runner (local op)
...
Model / agent:
  -m, --model <provider/model>   Model (repeatable). Default anthropic/claude-sonnet-4-6
      --agent <name>             op (default) | oracle | nop | any harbor agent
      --install <source|local|published> op install mode (default: source).
      ...
      --version <v>              op version for published install (default: latest)
...
Gateway (auth, no keys in container):
      ...
      --web-search               Enable op web_search (off by default; can't auth via gateway)
...
function parseTrial(dir: string, name: string): Trial | null {
	...
	if (!fs.existsSync(resultPath)) {
		...
		// Realtime cost from the live agent op.txt log, parsed incrementally.
		const probe = probeTrialCost(path.join(dir, "agent", "op.txt"));
	...
	costProbes.delete(path.join(dir, "agent", "op.txt"));
...
function buildTarball(benchDir: string): string {
	process.stdout.write(dim("packing local op (bun pm pack)…\n"));
...
/** Linux deps tree + mount plan for running op straight from the mounted repo. */
...
/**
 * Env vars injected into the in-container op run: every host `PI_*` knob (minus
...
export function buildHarborEnv(
	...
	if (source) {
		env.OP_BENCH_SOURCE_DIR = SOURCE_SRC_MOUNT;
		env.OP_BENCH_SOURCE_BUN = `${SOURCE_BIN_MOUNT}/bun`;
		env.OP_BENCH_SOURCE_ARCH = source.arch;
	}
	if (cfg.binaryArm64) env.OP_BENCH_BINARY_ARM64 = cfg.binaryArm64;
	...
	if (cfg.gateway) {
		env.OP_BENCH_MODELS_YAML = modelsYaml;
		env.OP_BENCH_GATEWAY_URL = cfg.gatewayUrl;
		env.OP_BENCH_GATEWAY_TOKEN = cfg.gatewayToken;
		env.OP_BENCH_GATEWAY_PROVIDERS = deriveProviders(cfg).join(",");
	}
	if (cfg.envType === "apple-container") env.OP_BENCH_CONTAINER_DNS = CONTAINER_DNS;
	const forward = collectForwardEnv(cfg);
	if (Object.keys(forward).length > 0) env.OP_BENCH_FORWARD_ENV = JSON.stringify(forward);
	return env;
}
...
async function runBenchmark(cfg: Config): Promise<BenchmarkRun> {
	...
	if (cfg.agent === "op" && cfg.envType === "docker" && !which("docker")) {
	...
	if (cfg.agent === "op" && cfg.install === "local" && !cfg.binaryArm64 && !cfg.binaryX64) {
		if (tarball) {
	...
	if (cfg.agent === "op" && cfg.install === "source" && !cfg.binaryArm64 && !cfg.binaryX64) {
		source = prepareSourceDeps(cfg);
	}

	// models.yml (gateway)
	let modelsYaml = "";
	if (cfg.agent === "op" && cfg.gateway) {
		modelsYaml = writeModelsYaml(benchDir, cfg);
		if (!gatewayHealthOk(cfg.gatewayUrl)) {
			process.stderr.write(
				yellow(
					`warning: gateway ${cfg.gatewayUrl} health check failed (continuing). Is the pm2 'op-auth-gateway' running?\n`,
				),
			);
		}
	}
	...
	if (cfg.dryRun) {
		...
		process.stdout.write(bold("op env:\n"));
		for (const key in harborEnv) {
			if (key === "OP_BENCH_FORWARD_ENV") continue;
			if (key.startsWith("OP_BENCH_") || key === "PYTHONPATH") process.stdout.write(`  ${key}=${harborEnv[key]}\n`);
		}
		if (harborEnv.OP_BENCH_FORWARD_ENV) {
			const parsedForwardEnv: unknown = JSON.parse(harborEnv.OP_BENCH_FORWARD_ENV);
			if (parsedForwardEnv !== null && typeof parsedForwardEnv === "object" && !Array.isArray(parsedForwardEnv)) {
				...
				process.stdout.write(`  OP_BENCH_FORWARD_ENV=${keys.join(",")} (values hidden)\n`);
			}
		}
		process.stdout.write(`\njob dir: ${jobDir}\nbench dir: ${benchDir}\n`);
