 ...
 *
 * Auth: provider API keys resolve through op's auth storage
 * (~/.op/agent/agent.db: stored key, OAuth, or env var fallback).
 */

import { parseArgs } from "node:util";
import { type Api, AuthStorage, completeSimple, type Model, SqliteAuthCredentialStore } from "@openpaths/ai";
import { type GeneratedProvider, getBundledModel } from "@openpaths/catalog/models";
import { getAgentDbPath } from "@openpaths/utils";
...
async function openModel(modelSpec: string, storage: AuthStorage): Promise<OpenedModel> {
	...
	if (!apiKey) {
		throw new Error(`no credentials for provider "${provider}" (run \`op login\` or set the provider env var)`);
	}
