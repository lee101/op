...
this runs the working tree at `/work/pi`. Install modes (`OP_BENCH_INSTALL`):
  ...
  * `source` (default): the runner bind-mounts the repo read-only plus a
    prebuilt linux `node_modules` tree and a linux `bun` binary; op runs
  ...
  * binary (`--binary`): a self-contained compiled op binary is uploaded.

Auth never enters the container: a generated `~/.op/agent/models.yml` routes the
...
(see `OP_BENCH_*` below); the agent reads them from `os.environ` directly.

Selected via `harbor run --agent-import-path op_local:OpLocal` with the
...
def _patch_apple_container_dns() -> None:
    ...
    sets OP_BENCH_CONTAINER_DNS for apple-container jobs; absent, no-op.
    """
    dns = os.environ.get("OP_BENCH_CONTAINER_DNS")
    if not dns:
        return
...
class OpLocal(BaseInstalledAgent):
    ...
    def __init__(self, *args, **kwargs) -> None:  # noqa: D401 - thin wrapper
        ...
        self._install_mode = _env("OP_BENCH_INSTALL", "source")
        self._tarball = _env("OP_BENCH_TARBALL")
        self._pkg_version = _env("OP_BENCH_VERSION", "latest")
        self._models_yaml_path = _env("OP_BENCH_MODELS_YAML")
        self._gateway_url = _env(
            "OP_BENCH_GATEWAY_URL", "http://host.docker.internal:4000"
        )
        self._gateway_token = _env("OP_BENCH_GATEWAY_TOKEN", "no-auth-dummy")
        self._gateway_providers = [
            p.strip()
            for p in _env(
                "OP_BENCH_GATEWAY_PROVIDERS", "anthropic,openai-codex"
            ).split(",")
            if p.strip()
        ]
        self._thinking = _env("OP_BENCH_THINKING")
        self._auto_approve = _truthy(_env("OP_BENCH_AUTO_APPROVE", "1"))
        # Extra CLI args forwarded verbatim to the in-container op invocation,
        ...
        self._cli = "/root/.op-bench/app/dist/cli.js"
        self._binary_arm64 = _env("OP_BENCH_BINARY_ARM64")
        self._binary_x64 = _env("OP_BENCH_BINARY_X64")
    ...
    def _wrap(self, command: str) -> str:
        ...
        op spawns Bun worker subprocesses at runtime, so `bun` must resolve on
    ...
    async def install(self, environment: BaseEnvironment) -> None:
        ...
        if self._binary:
            ...
            await self.exec_as_root(
                ...
                command=(
                    ...
                    "  echo 'ERROR: Alpine/musl base image; @openpaths/natives ships no musl prebuilt' >&2; exit 3; "
        ...
        # 3) Auth + model config under $HOME/.op/agent.
        if self._gateway_on:
    ...
    async def _install_source(self, environment: BaseEnvironment) -> str:
        """Verify the read-only repo + linux deps mounts and run op from TS source.
    ...
    async def _install_local(self, environment: BaseEnvironment) -> str:
        if not self._tarball:
            raise RuntimeError(
                "OP_BENCH_INSTALL=local requires OP_BENCH_TARBALL (host tarball path)"
            )
        await environment.upload_file(self._tarball, _TARBALL_DST)
        app = f"{self._home}/.op-bench/app"
        await self.exec_as_agent(
            environment,
            command=self._wrap(
                "set -e; "
                ...
                'echo "pinning native @openpaths/natives-linux-$na@$ver"; '
                'bun add --production "@openpaths/natives-linux-$na@$ver"'
            ),
            timeout_sec=900,
        )
    ...
    async def _install_binary(self, environment: BaseEnvironment) -> str:
        """Probe container arch, upload only the matching self-contained op binary."""
        arch = (
            await self.exec_as_agent(environment, command="uname -m")
        ).stdout.strip()
    ...
    async def _install_published(self, environment: BaseEnvironment) -> str:
        app = f"{self._home}/.op-bench/app"
        spec = f"@openpaths/coding-agent@{self._pkg_version}"
        await self.exec_as_agent(
            environment,
            command=self._wrap(
                "set -e; "
        ...
        return f"{app}/node_modules/@openpaths/coding-agent/dist/cli.js"
    ...
    async def _write_models_yaml(self, environment: BaseEnvironment) -> None:
        if self._models_yaml_path and os.path.isfile(self._models_yaml_path):
            ...
            staged = _MODELS_DST
            heredoc = (
                f"cat > {_MODELS_DST} <<'OP_MODELS_EOF'\n{content}\nOMP_MODELS_EOF"
            )
            await self.exec_as_agent(environment, command=heredoc)
        await self.exec_as_agent(
            environment,
            command=(
                f'mkdir -p "$HOME/.op/agent"; '
                f'cp {shlex.quote(staged)} "$HOME/.op/agent/models.yml"'
            ),
        )
    ...
    async def _write_config(self, environment: BaseEnvironment) -> None:
        ...
        await self.exec_as_agent(
            ...
            command=(
                f'mkdir -p "$HOME/.op/agent"; '
                f'cp {shlex.quote(_CONFIG_DST)} "$HOME/.op/agent/config.yml"'
            ),
        )

    @staticmethod
    def _parse_forward_env() -> dict[str, str]:
        """Extra run-time env from the runner (OP_BENCH_FORWARD_ENV = JSON object)."""
        raw = _env("OP_BENCH_FORWARD_ENV")
        if not raw:
            return {}
        try:
    ...
    def _parse_agent_args() -> list[str]:
        """Extra CLI args from the runner (OP_BENCH_AGENT_ARGS = JSON array)."""
        raw = _env("OP_BENCH_AGENT_ARGS")
        if not raw:
            return []
        try:
            parsed = json.loads(raw)
    ...
    def _sum_main(self, path: Path, acc: "_Usage") -> None:
        """Sum assistant `message_end` usage from op's stdout JSONL.
