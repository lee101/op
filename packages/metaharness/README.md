...
1. **Local op, not npm.** By default the runner bind-mounts the repo
   read-only into each task container (`--install source`) and runs op
   ...
   `/opt/op/bin` — so trial setup needs zero outbound network. Alternatives:
   `--install local` (pack a tarball per run) or `--binary` (prebuilt
   `dist/op-linux-*` self-contained binaries).
...
| `--env <KEY[=VALUE]>` | — | Forward env into the op container (repeatable); `KEY` alone forwards the host value |
| `--binary <path>` | — | Prebuilt op binary (repeat for arm64+x64) |
| `--install <source\|local\|published>` | `source` | `source` = repo bind-mount, `local` = tarball pack, `published` = npm `@openpaths/coding-agent` |
...
arc; ~$0.001 per report). API keys resolve through op's auth storage.
