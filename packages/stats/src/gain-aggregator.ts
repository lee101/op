...
import { getStatsDbPath, isEnoent, logger } from "@openpaths/utils";
...
const TEMP_PATH_RE = /(?:^|\/)(?:T|tmp|pi-bash-exec|op-bash-exec|pi-bash-detach)(?:\/|$)|^\/var\/folders(?:\/|$)/;
 ...
 *
 * Rules are generic: op internal wt paths are dropped; conventional worktree
...
export function normalizeProjectPath(p: string): string | null {
	...
	if (/\/\.op\/wt\//u.test(clean)) return null;
