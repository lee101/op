...
import { getStatsDbPath, workerHostEntry } from "@openpaths/utils";
...
// JavaScript entry. Standalone source `op-stats` keeps using this package's
...
function defaultWorkerCount(): number {
	...
	// the compiled `op` binary. Keep macOS on the documented serial path.
 ...
 * self-dispatching CLI entry (op in source, npm-bundle, or compiled form),
 * re-enter that entry with a worker argv selector; otherwise (standalone
 * op-stats, bun test, SDK embedding) load the worker module directly, so this
 * package keeps zero runtime dependency on `@openpaths/coding-agent`.
 */
 ...
 * then terminates. Used by `op --smoke-test` so the install-method CI jobs
