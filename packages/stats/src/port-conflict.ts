...
const STATS_RUNTIME_IMAGES: Record<string, true> = { bun: true, node: true, op: true, "op-stats": true };
...
export const STATS_DASHBOARD_HEADER = "x-op-stats-dashboard";
/** Header recording the server's requested bind host so reuse cannot change its exposure scope. */
export const STATS_DASHBOARD_HOSTNAME_HEADER = "x-op-stats-hostname";
...
async function reclaimStatsPort(port: number): Promise<"retry"> {
	...
	const hasStatsIdentity =
		normalizedImage === "op-stats" ||
		/(?:^|[/"'\s])op-stats(?:\.exe)?(?:["'\s]|$)/.test(normalizedCommand) ||
		/\/packages\/stats\/src\/index\.ts(?:["'\s]|$)/.test(normalizedCommand) ||
		(normalizedImage === "op" && /(?:^|\s)stats(?:\s|$)/.test(normalizedCommand)) ||
		/(?:^|\/)op(?:\.exe)?["'\s]+stats(?:["'\s]|$)/.test(normalizedCommand);
	if (!STATS_RUNTIME_IMAGES[normalizedImage] || !hasStatsIdentity) {
		throw new Error(
			`Port ${port} is in use by ${holder.image} (PID ${holder.pid}), which is not identifiable as an op stats dashboard; refusing to stop it.`,
		);
	}
