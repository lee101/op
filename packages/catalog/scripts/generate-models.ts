...
import { discoverAuthStorage } from "@openpaths/ai/auth-broker/discover";
import type { OAuthAccess } from "@openpaths/ai/auth-storage";
import type { OAuthProvider } from "@openpaths/ai/oauth/types";
import { getGitLabDuoModels } from "@openpaths/ai/providers/gitlab-duo";
...
async function fetchAntigravityModels(): Promise<ModelSpec<"google-gemini-cli">[]> {
	...
	if (!access) {
		...
		console.log("Tip: If you are logged in under a specific profile, run with OP_PROFILE=<name>.");
		return [];
	}
	try {
