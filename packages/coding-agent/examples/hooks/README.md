...
Example hooks for op-coding-agent.

## Usage

```bash
# Load a hook with --hook flag
op --hook examples/hooks/permission-gate.ts

# Or copy to hooks directory for auto-discovery
cp permission-gate.ts ~/.op/agent/hooks/
```
...
```typescript
import type { HookAPI } from "@openpaths/coding-agent/hooks";
