 ...
 * `BlobStore` keep writing to `~/.op/agent/...`. Reach for an object store
 * (S3, R2, GCS) if you need those off-host too.
 */

import { createAgentSession, RedisSessionStorage, SessionManager } from "@openpaths/coding-agent";
...
const storage = await RedisSessionStorage.create({
	client: redis,
	prefix: "op:sessions:", // optional, this is the default
});
