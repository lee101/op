 ...
 * `@openpaths/utils` logger and metric instruments through the agent
 * telemetry hooks, flushes, and exits 0 only if the receiver got a non-empty
 * protobuf POST at both /v1/logs and /v1/metrics.
 */

import type { AgentRunCoverage, AgentRunSummary, ChatUsageEvent } from "@openpaths/agent-core";
import { emptyAgentRunCoverage, emptyAgentRunSummary } from "@openpaths/agent-core";
import {
	createTelemetryExportConfig,
	flushTelemetryExport,
	initTelemetryExport,
	isTelemetryExportEnabled,
} from "@openpaths/coding-agent/telemetry-export";
...
process.env.OTEL_SERVICE_NAME = "openpaths-signals-probe";
...
await Bun.sleep(700);
await flushTelemetryExport();
assertSingleMetricPoint("pi.op.agent.chat.calls");
assertSingleMetricPoint("pi.op.agent.tool.calls");
assertSingleMetricPoint("pi.op.agent.tool.duration");
await server.stop(true);
