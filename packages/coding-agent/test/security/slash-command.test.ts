...
import { refreshDirsFromEnv } from "@openpaths/utils";
...
beforeEach(async () => {
	temporaryRoot = await fs.mkdtemp(path.join(os.tmpdir(), "op-security-slash-"));
...
describe("/security", () => {
	...
	test("validate returns a static OP-native residual prompt", async () => {
