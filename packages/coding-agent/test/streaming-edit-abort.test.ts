...
import type { AssistantMessage, StopReason, ToolCall } from "@openpaths/ai";
...
import * as autoGeneratedGuard from "@openpaths/coding-agent/tools/auto-generated-guard";
...
it("keeps the session alive when replace mode streams an ssh:// path", async () => {
	const checkSpy = vi.spyOn(autoGeneratedGuard, "assertEditableFile");
	const abortSignalRef: { current?: AbortSignal } = {};
	const remotePath = "ssh://test-host/tmp/op-repro.txt";
