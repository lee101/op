...
describe("parseArgs — trusted extension allowlist", () => {
	it("accepts repeatable native absolute paths", () => {
		const parsed = parseArgs(["--trusted-extension", "/opt/op/policy.ts", "--trusted-extension=/opt/op/audit.ts"]);

		expect(parsed.trustedExtensions).toEqual(["/opt/op/policy.ts", "/opt/op/audit.ts"]);
	});
	...
	it("fails closed on missing, relative, swallowed, or mixed values", () => {
		...
		expect(() => parseArgs(["--extension", "--trusted-extension", "/opt/op/policy.ts"])).toThrow(
			/requires a non-empty/,
		);
		expect(() => parseArgs(["--trusted-extension", "/opt/op/policy.ts", "--hook", "/tmp/hook.ts"])).toThrow(
			/cannot be combined/,
		);
	});
});
