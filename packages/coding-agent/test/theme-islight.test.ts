...
import { generateThemeVars } from "@openpaths/coding-agent/export/html";
import { defaultThemes } from "@openpaths/coding-agent/modes/theme/defaults";
import { getResolvedThemeColors, getThemeByName, isLightTheme } from "@openpaths/coding-agent/modes/theme/theme";
import { getAgentDir, getCustomThemesDir, removeWithRetries, setAgentDir } from "@openpaths/utils";
...
describe("getResolvedThemeColors HTML export defaults", () => {
	...
	it("uses light text when a light-status custom theme derives dark export surfaces from userMessageBg", async () => {
		...
		tempAgentDir = await fs.mkdtemp(path.join(os.tmpdir(), "op-theme-export-"));
