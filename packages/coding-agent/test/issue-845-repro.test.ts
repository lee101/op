...
import { resolveUpdateMethodForTest } from "@openpaths/coding-agent/cli/update-cli";
import { removeSyncWithRetries } from "@openpaths/utils";

// Issue #845: on Windows with Bun installed via Scoop, ~/.bun is a junction
// to scoop\persist\Oven-sh.Bun\.bun. `bun pm bin -g` and the op path that
...
// does not follow filesystem links, so it misclassified Bun-installed op
// as "binary" and tried to swap op.exe in place – which fails on Windows
...
describe("issue-845: resolveUpdateMethod follows symlinks/junctions", () => {
	...
	it("classifies op reached through a symlinked bin dir as bun-managed", () => {
	...
	it("classifies op at the real bin dir as bun-managed when bunBinDir is symlinked", () => {
		const ompAtReal = path.join(realBinDir, "op");
