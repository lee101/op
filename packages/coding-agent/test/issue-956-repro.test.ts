...
import { getConfigRootDir, getProjectDir, removeWithRetries, setAgentDir, setProjectDir } from "@openpaths/utils";
...
describe("issue #956: interactive /mcp test", () => {
	...
	beforeEach(async () => {
		projectDir = await fs.mkdtemp(path.join(os.tmpdir(), "op-issue-956-project-"));
		agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "op-issue-956-agent-"));
