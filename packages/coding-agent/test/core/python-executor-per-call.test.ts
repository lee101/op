...
import { executePython } from "@openpaths/coding-agent/eval/py/executor";
import type { KernelExecuteOptions, KernelExecuteResult } from "@openpaths/coding-agent/eval/py/kernel";
import { PythonKernel } from "@openpaths/coding-agent/eval/py/kernel";
...
describe("executePython (per-call)", () => {
	...
	it("returns a cancelled timeout result when kernel startup exceeds the deadline", async () => {
		...
		using tempDir = TempDir.createSync("@op-python-executor-per-call-");
