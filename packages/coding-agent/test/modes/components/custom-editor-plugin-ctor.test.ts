...
import { ProcessTerminal, TUI } from "@openpaths/tui";
 ...
 * `CustomEditor`/`Editor` and forward `super(tui, theme, keybindings)`. op's
...
describe("CustomEditor upstream-pi constructor compatibility (#4766)", () => {
	...
	it("still accepts op's own (theme) constructor", async () => {
		await initTheme();
