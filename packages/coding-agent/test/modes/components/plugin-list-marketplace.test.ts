...
import { PluginManager } from "@openpaths/coding-agent/extensibility/plugins";
import {
...
} from "@openpaths/coding-agent/modes/components/plugin-settings";
...
describe("PluginListComponent", () => {
	...
	it("empty-state mentions both npm and marketplace install commands", () => {
		...
		expect(text).toContain("op plugin install <package>");
		expect(text).toContain("op plugin install <name>@<marketplace>");
	});
...
describe("MarketplacePluginDetailComponent", () => {
	...
	it("shortens home-relative install paths to ~ before rendering", () => {
		...
		const installPath = `${home}/.op/cache/plugins/sample@mkt`;
		...
		expect(text).toContain("~/.op/cache/plugins/sample@mkt");
