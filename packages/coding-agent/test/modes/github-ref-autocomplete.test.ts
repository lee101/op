...
import { KeybindingsManager as AppKeybindingsManager } from "@openpaths/coding-agent/config/keybindings";
import { getGithubRefContext, getGithubRefSuggestions } from "@openpaths/coding-agent/modes/github-ref-autocomplete";
import { createPromptActionAutocompleteProvider } from "@openpaths/coding-agent/modes/prompt-action-autocomplete";
import type { SlashCommand } from "@openpaths/tui";
...
describe("github-ref autocomplete — token detection", () => {
	...
	it("requires a token boundary before # so embedded hashes don't match", () => {
		...
		expect(getGithubRefContext("https://github.com/lee101/op#3164")).toBeNull();
