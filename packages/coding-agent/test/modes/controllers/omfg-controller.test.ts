...
import type { AssistantMessage, Usage } from "@openpaths/ai";
import type { Rule } from "@openpaths/coding-agent/capability/rule";
import { OmfgController } from "@openpaths/coding-agent/modes/controllers/omfg-controller";
...
import { Container, type TUI } from "@openpaths/tui";
import { removeWithRetries } from "@openpaths/utils";

const PROJECT_OPTION = "This project (.op/rules)";
const GLOBAL_OPTION = "Global — all projects (~/.op/agent/rules)";
...
describe("OmfgController", () => {
	it("saves a matching generated rule under project rules and registers it live", async () => {
		...
		const savedPath = path.join(harness.projectDir, ".op", "rules", "ts-no-any.md");
		...
		expect(rendered).toContain(path.join(".op", "rules", "ts-no-any.md"));
	...
	it("reiterates when the first valid rule does not match history", async () => {
		...
		expect(await Bun.file(path.join(harness.projectDir, ".op", "rules", "ts-no-any.md")).exists()).toBe(true);
	});
	...
	it("asks before saving when validation never confirms a match", async () => {
		...
		expect(await Bun.file(path.join(harness.projectDir, ".op", "rules", "no-match.md")).exists()).toBe(false);
	});
	...
	it("lets the user amend from the save selector before writing the rule", async () => {
		...
		expect(await Bun.file(path.join(harness.projectDir, ".op", "rules", "ts-any-broad.md")).exists()).toBe(false);
		expect(await Bun.file(path.join(harness.projectDir, ".op", "rules", "ts-no-explicit-any.md")).exists()).toBe(
			true,
		);
	});
	...
	it("clears the panel and aborts the inner request on Escape", async () => {
		...
		expect(controller.hasActiveRequest()).toBe(false);
		expect(await Bun.file(path.join(harness.projectDir, ".op", "rules", "ts-no-any.md")).exists()).toBe(false);
	});
});
