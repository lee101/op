...
import { LoginDialogComponent } from "@openpaths/coding-agent/modes/components/login-dialog";
...
describe("SelectorController login", () => {
	...
	it("routes enhanced paste into a direct API-key prompt", async () => {
		...
		dialog.pasteText("OP_PASTE_TEST_123");
		dialog.handleInput("\n");

		await expect(prompt).resolves.toBe("OP_PASTE_TEST_123");
	});
});
