 ...
 * `op list` / `op remove` / `op marketplace <sub>` / `op uninstall …` etc.
 * as top-level commands, but only `op install` is registered. Before the fix,
 * `resolveCliArgv(["list"])` rewrote the bare verb to `["launch", "list"]`, so
 * `op list` silently started an interactive agent session with "list" as the
 * initial LLM prompt instead of managing plugins (the real command is
 * `op plugin list`). #4845 extended the same footgun to the multi-word
 * documented grammar: `op marketplace add xyz` leaked the whole argv to the
 * model as a prompt.
 *
 ...
 * `op plugin <action>` command rather than leaking to the model — while
 * genuine prose prompts that merely begin with one of these words still fall
 * through to `launch`.
 *
 * Imported via a relative path (not the `@openpaths/coding-agent` alias) so the
...
describe("documented-but-unregistered plugin verbs do not leak to launch (#2935)", () => {
	test("bare `op list` hints at `op plugin list` instead of launching with 'list' as the prompt", () => {
		...
		expect("error" in result && result.error).toContain("op plugin list");
	});

	test("bare `op remove` hints at `op plugin uninstall` instead of launching with 'remove' as the prompt", () => {
		const result = resolveCliArgv(["remove"]);
		expect(result).not.toEqual({ argv: ["launch", "remove"] });
		expect(result).not.toHaveProperty("argv");
		expect(result).toHaveProperty("error");
		expect("error" in result && result.error).toContain("op plugin uninstall");
	});
	...
	test("multi-word `op marketplace add xyz` hints at `op plugin marketplace` instead of leaking to the prompt (#4845)", () => {
		...
		expect(result).not.toHaveProperty("argv");
		expect(result).toHaveProperty("error");
		expect("error" in result && result.error).toContain("op plugin marketplace");
	});

	test("bare marketplace-family verbs hint at their `op plugin` command (#4845)", () => {
		for (const [verb, hint] of [
			["marketplace", "op plugin marketplace"],
			["discover", "op plugin discover"],
			["upgrade", "op plugin upgrade"],
			["uninstall", "op plugin uninstall"],
			["enable", "op plugin enable"],
			["disable", "op plugin disable"],
		] as const) {
