...
describe("global --profile flag", () => {
	...
	beforeEach(() => {
		...
		configDir = `.op-profile-cli-test-${Snowflake.next()}`;
	...
	it("activates a profile inherited from OP_PROFILE at run time", async () => {
		const writeSpy = vi.spyOn(process.stdout, "write").mockImplementation(() => true);
		setProfile(undefined);
		process.env.OP_PROFILE = "work";
		delete process.env.PI_PROFILE;
	...
	it("loads profile agent .env before command modules import pi-utils env", async () => {
		...
		try {
			...
			await Bun.write(path.join(defaultAgentDir, ".env"), "OP_PROFILE_BOOTSTRAP_SENTINEL=default\n");
			await Bun.write(path.join(profileAgentDir, ".env"), "OP_PROFILE_BOOTSTRAP_SENTINEL=work\n");
			...
			await Bun.write(
				...
				[
					...
					'process.stdout.write("\\nSENTINEL=" + (Bun.env.OP_PROFILE_BOOTSTRAP_SENTINEL ?? ""));',
				].join("\n"),
			);
			...
			delete childEnv.OP_PROFILE;
			delete childEnv.PI_PROFILE;
			delete childEnv.PI_CODING_AGENT_DIR;
			delete childEnv.OP_PROFILE_BOOTSTRAP_SENTINEL;
	...
	it("surfaces an invalid OP_PROFILE env as a clean error, not an import crash", async () => {
		...
		try {
			...
			const childEnv: Record<string, string | undefined> = {
				...process.env,
				HOME: home,
				PI_CONFIG_DIR: ".op-profile-cli-env-bad",
				OP_PROFILE: "..",
				NO_COLOR: "1",
			};
			delete childEnv.PI_PROFILE;
			...
			expect(stderr).toContain("Invalid OP profile");
