...
import { MCPOAuthFlow, refreshMCPOAuthToken } from "@openpaths/coding-agent/mcp/oauth-flow";
...
describe("mcp oauth flow", () => {
	it("uses openpaths client name for dynamic client registration", async () => {
		...
		expect((registrationPayload as { client_name?: string } | null)?.client_name).toBe("openpaths");
			...
			if (url === "https://cropwise.example/oauth/register") {
				return new Response(
					JSON.stringify({
						...
						error_description: "client_name 'openpaths' is not on the approved list.",
					}),
