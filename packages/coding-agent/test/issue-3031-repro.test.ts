/**
 * Regression for https://github.com/lee101/op/issues/3031
 *
...
} from "@openpaths/coding-agent/mnemopi/embed-client";
import type {
...
describe("issue #3031 — mnemopi embeddings live in an isolated subprocess", () => {
	it("ping/pongs through the spawned worker subprocess and tears it down cleanly", async () => {
		...
		// `op --smoke-test`. Run it in a child Bun process instead of this
		...
		const script =
			'const { smokeTestMnemopiEmbedWorker } = await import("@openpaths/coding-agent/mnemopi/embed-client"); await smokeTestMnemopiEmbedWorker({ timeoutMs: 15000 });';
