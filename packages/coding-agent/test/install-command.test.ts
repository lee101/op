/**
 * Regression test for #1496 (bug 2): `op install ./my-extension` used to be
...
import { commands, isSubcommand, resolveCliArgv } from "@openpaths/coding-agent/cli-commands";
import { looksLikeLocalPath } from "@openpaths/coding-agent/commands/install";
...
describe("install command is registered as a top-level subcommand", () => {
	...
	test("CLI runner rejects only bare reserved management words", () => {
		expect(resolveCliArgv(["extensions"])).toEqual({
			error: '`op extensions` is not a management command. Use `op plugin list` / `op plugin install`, or run `op launch extensions` if you meant to send "extensions" as a prompt.',
		});
...
describe("looksLikeLocalPath", () => {
	...
	test("npm specs and marketplace refs are remote", () => {
		expect(looksLikeLocalPath("@openpaths/exa")).toBe(false);
	...
	test("bare names that exist as a local directory are treated as local", () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "op-install-test-"));
		try {
