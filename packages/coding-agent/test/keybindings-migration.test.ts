...
import { matchesAppFollowUp } from "@openpaths/coding-agent/modes/utils/keybinding-matchers";
import { type KeybindingsConfig, setKeybindings } from "@openpaths/tui";
import {
...
`,
	...
	it("merges default user keybindings when create uses the active profile with no arguments (#4867)", async () => {
		...
		try {
			...
			restoreEnvValue("OP_PROFILE", originalOmpProfile);
