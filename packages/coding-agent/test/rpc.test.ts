...
import type { AgentEvent, AgentMessage } from "@openpaths/agent-core";
import { type AssistantMessage, Effort, type TextContent } from "@openpaths/ai";
import {
...
import { RpcClient } from "@openpaths/coding-agent/modes/rpc/rpc-client";
import type { BashExecutionMessage } from "@openpaths/coding-agent/session/messages";
...
describe.skipIf(!e2eApiKey("ANTHROPIC_API_KEY"))("RPC mode", () => {
	...
	beforeEach(() => {
		sessionDir = path.join(os.tmpdir(), `op-rpc-test-${Snowflake.next()}`);
...
describe("RPC fast mode with unsupported Fireworks model and priority tier", () => {
	...
	beforeEach(async () => {
		sessionDir = path.join(os.tmpdir(), `op-rpc-fast-mode-test-${Snowflake.next()}`);
