...
import * as connectionManager from "@openpaths/coding-agent/ssh/connection-manager";
import { removeWithRetries } from "@openpaths/utils";

async function withLooseKey<T>(run: (keyPath: string) => Promise<T>): Promise<T> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "op-ssh-key-"));
...
describe("buildRemoteCommand", () => {
	...
	it("still rejects directory identity paths on Windows args", async () => {
		const keyPath = await fs.mkdtemp(path.join(os.tmpdir(), "op-ssh-key-"));
		try {
			await expect(
				connectionManager.buildRemoteCommand(
					{
