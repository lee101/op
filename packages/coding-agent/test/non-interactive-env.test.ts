...
import { buildNonInteractiveEnv, NON_INTERACTIVE_ENV } from "@openpaths/coding-agent/exec/non-interactive-env";
...
it("filters expanded dotenv values while preserving matching and empty launcher values", async () => {
	const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "op-env-"));
	try {
		await Bun.write(
			path.join(tmp, ".env"),
			[
		...
		const script = [
			...
			"	inherited: env.OP_TEST_INHERITED_MARKER ?? null,",
		...
		for (const bunArgs of bunArgSets) {
			const proc = Bun.spawn([process.execPath, ...bunArgs, "--no-install", "--eval", script], {
				...
				env: {
					...
					EMPTY_PARENT_VAR: "",
					OP_TEST_INHERITED_MARKER: "keep-me",
					NODE_ENV: "development",
