/**
 * Routing tests for `op plugin install <local-path>` (#1945).
 *
...
import type { InstalledPlugin } from "@openpaths/coding-agent/extensibility/plugins/types";
...
async function createLocalPlugin(root: string): Promise<string> {
	...
	await Bun.write(
		...
		JSON.stringify({
			name: "kimi-datasource",
			version: "1.0.0",
			op: { extensions: ["./src/extension.ts"] },
		}),
	);
	return localPlugin;
}
...
describe("runPluginCommand({ action: 'install', args: [<local>] })", () => {
	...
	beforeEach(async () => {
		tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "op-plugin-install-local-"));
		...
		spyOn(piUtils, "getPluginsLockfile").mockReturnValue(path.join(tmpRoot, "op-plugins.lock.json"));
	...
	test("real local plugin directory: install symlinks it like link would", async () => {
		...
		// symlink rather than copy-install, matching `op plugin link`.
	...
	test("doctor --fix preserves linked local plugin state without package dependencies", async () => {
		await Bun.write(
			...
			JSON.stringify({ name: "op-plugins", private: true, dependencies: {} }),
		);
		const localPlugin = await createLocalPlugin(tmpRoot);
