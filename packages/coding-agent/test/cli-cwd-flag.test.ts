...
import { applyStartupCwd } from "@openpaths/coding-agent/cli/startup-cwd";
...
describe("parseArgs — --cwd flag", () => {
	...
	it("applies --cwd before session lookup callers read the project directory", async () => {
		const launchDir = fs.mkdtempSync(path.join(os.tmpdir(), "op-cwd-launch-"));
		const targetDir = fs.mkdtempSync(path.join(os.tmpdir(), "op-cwd-target-"));
	...
	it("normalizes a relative --cwd target to the resolved absolute path", async () => {
		const launchDir = fs.mkdtempSync(path.join(os.tmpdir(), "op-cwd-rel-"));
