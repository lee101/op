...
import { type Skill as CapabilitySkill, skillCapability } from "@openpaths/coding-agent/capability/skill";
import { getCapability } from "@openpaths/coding-agent/discovery";
import { getWslWindowsHomeCandidate, runHostProbe } from "@openpaths/coding-agent/discovery/agents";
import {
	...
	type Skill,
} from "@openpaths/coding-agent/extensibility/skills";
...
describe("skills", () => {
	...
	describe("loadSkills with options", () => {
		...
		// canonical OP-native `~/.agent[s]/skills` (the `agents` provider).
		...
		// MUST NOT consider the OP-native `enableAgentsUser`/`...Project`
		...
		it("should skip skills disabled via frontmatter", async () => {
			const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "op-disabled-skill-"));
...
`,
		...
		it("should hide skills with disable-model-invocation frontmatter (Agent Skills spec)", async () => {
			const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "op-dmi-skill-"));
