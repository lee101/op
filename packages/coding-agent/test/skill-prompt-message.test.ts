...
import { buildSkillPromptMessage, type Skill } from "@openpaths/coding-agent/extensibility/skills";
...
async function createSkill(body: string): Promise<{ dir: string; skill: Skill }> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), `op-skill-prompt-${Snowflake.next()}-`));
