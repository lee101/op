/**
 * Real package-manager seam for `op.rename` migrations.
 *
...
beforeAll(async () => {
	...
	await initTheme();
	fixtureDir = await TempDir.create("@op-rename-itest-");
...
/** Two shared, read-only packages that expose the same `op` bin. */
async function makeFixtures(root: string): Promise<{ oldDir: string; newDir: string }> {
	const mkpkg = async (name: string, version: string): Promise<string> => {
		...
		await Bun.write(path.join(dir, "package.json"), JSON.stringify({ name, version, bin: { op: "cli.js" } }));
...
async function verifyLauncher(binDir: string, expectedVersion: string): Promise<InstalledVersionVerification> {
	const launcher = path.join(binDir, "op");
