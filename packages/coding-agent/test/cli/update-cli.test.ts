...
describe("getLatestRelease rename pointers", () => {
	...
	it("follows op.rename to the new package and resolves version, dist, and names from its manifest", async () => {
		const urls = stubRegistry({
			"@new/op": { version: "999.1.0", op: { dist: "npm" } },
			"@openpaths/coding-agent": {
				version: "999.0.0",
				op: { dist: "binary", rename: { package: "@new/op", natives: "@new/natives" } },
			},
		});
		...
		expect(release.packages).toEqual({ pkg: "@new/op", natives: "@new/natives" });
		expect(urls).toEqual([
			"https://registry.npmjs.org/@openpaths/coding-agent/latest",
			"https://registry.npmjs.org/@new/op/latest",
		]);
	});
	...
	it("ignores a rename pointer that cycles back to an already-visited package", async () => {
		const urls = stubRegistry({
			"@openpaths/coding-agent": {
				version: "999.0.0",
				op: { rename: { package: "@openpaths/coding-agent" } },
			},
		});
