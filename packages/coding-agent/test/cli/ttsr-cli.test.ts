...
import {
	...
	type TtsrScanArgs,
	type TtsrTestArgs,
} from "@openpaths/coding-agent/cli/ttsr-cli";
import { resetSettingsForTest } from "@openpaths/coding-agent/config/settings";
import { getProjectAgentDir, getProjectDir, removeSyncWithRetries, setProjectDir } from "@openpaths/utils";

let testTmpDir: string;

beforeAll(() => {
	testTmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "op-ttsr-tests-"));
});
...
describe("op ttsr", () => {
	...
	describe("test — context inference and matching", () => {
		it("infers tool/edit context when a positional resolves to a .ts file and --source is omitted", async () => {
			...
			// Simulate `op ttsr test --rule <rule> src/foo.ts`: the command layer
