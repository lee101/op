...
import { StatusLineComponent, type StatusLineSettings } from "@openpaths/coding-agent/modes/components/status-line";
import { STATUS_LINE_PRESETS } from "@openpaths/coding-agent/modes/components/status-line/presets";
import { initTheme } from "@openpaths/coding-agent/modes/theme/theme";
import * as git from "@openpaths/coding-agent/utils/git";
import { removeSyncWithRetries, setProjectDir } from "@openpaths/utils";
...
beforeEach(async () => {
	...
	projectDir = fs.mkdtempSync(path.join(os.tmpdir(), "op-status-line-settings-cache-"));
