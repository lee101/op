...
import * as asrClient from "@openpaths/coding-agent/stt/asr-client";
import * as downloader from "@openpaths/coding-agent/stt/downloader";
import { STTController } from "@openpaths/coding-agent/stt/stt-controller";
import { getTinyModelsCacheDir, removeWithRetries, setAgentDir } from "@openpaths/utils";
...
describe("isSttModelCached completeness", () => {
	...
	beforeEach(async () => {
		...
		tmp = await fs.mkdtemp(path.join(os.tmpdir(), "op-stt-cache-"));
		setAgentDir(tmp);
