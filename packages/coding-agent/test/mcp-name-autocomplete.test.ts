...
import { collectMcpServerNames } from "@openpaths/coding-agent/modes/controllers/mcp-command-controller";
import type { InteractiveModeContext } from "@openpaths/coding-agent/modes/types";
import { buildTuiBuiltinSlashCommands } from "@openpaths/coding-agent/slash-commands/builtin-registry";
...
describe("MCP server-name autocomplete", () => {
	...
	beforeEach(async () => {
		projectDir = await fs.mkdtemp(path.join(os.tmpdir(), "op-mcp-autocomplete-project-"));
		agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "op-mcp-autocomplete-agent-"));
