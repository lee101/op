...
import type { Context } from "@openpaths/ai";
...
import { runPrintMode } from "@openpaths/coding-agent/modes/print-mode";
...
import { Snowflake } from "@openpaths/utils";

// Regression for #8272: with plan.defaultOnStartup:true, a headless `op -p`
...
describe("print mode + plan.defaultOnStartup (#8272)", () => {
	...
	beforeEach(async () => {
		tempDir = path.join(os.tmpdir(), `op-8272-${Snowflake.next()}`);
