...
import { computeFileHash } from "@openpaths/hashline";
...
} from "@openpaths/coding-agent/edit";
import { HashlineFilesystem } from "@openpaths/coding-agent/edit/hashline/filesystem";
import { resolveLocalUrlToPath } from "@openpaths/coding-agent/internal-urls";
import type { WritethroughCallback } from "@openpaths/coding-agent/lsp";
import type { PlanModeState } from "@openpaths/coding-agent/plan-mode/state";
...
describe("HashlineFilesystem ACP fs routing", () => {
	...
	beforeEach(async () => {
		...
		tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "op-acp-hashline-"));
	...
	it("keeps a local sandbox artifact addressed by absolute path off the ACP bridge", async () => {
		...
		// the OP-owned artifact must be written to disk, never pushed to the editor.
		const { bridge, spy: bridgeSpy } = makeBridge();
		const session = createSession(tmpDir, {
			bridge,
...
describe("executeHashlineSingle model-visible payload under write-time drift", () => {
	...
	beforeEach(async () => {
		resetSettingsForTest();
		tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "op-acp-hashline-e2e-"));
		await Settings.init({ inMemory: true, cwd: tmpDir });
	});
...
describe("executeReplace ACP fs routing", () => {
	...
	beforeEach(async () => {
		resetSettingsForTest();
		tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "op-acp-replace-"));
		await Settings.init({ inMemory: true, cwd: tmpDir });
	});
...
describe("executePatchSingle ACP fs routing", () => {
	...
	beforeEach(async () => {
		resetSettingsForTest();
		tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "op-acp-patch-"));
		await Settings.init({ inMemory: true, cwd: tmpDir });
	});
