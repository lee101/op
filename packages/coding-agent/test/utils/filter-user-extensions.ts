...
import { getAgentDir, getConfigRootDir, getPluginsDir } from "@openpaths/utils";
...
//    or `../`-relative entry pointing somewhere else under `~/.op/`), plus
//    the legacy non-XDG `~/.op/plugins` tree on hosts without XDG dirs.
// 2. `getAgentDir()` (`~/.op/agent` or `$PI_CODING_AGENT_DIR`)
...
// 3. `getPluginsDir()` (XDG-aware: `$XDG_DATA_HOME/op/plugins` or legacy)
//    Handles installed plugin extensions that live outside `~/.op` when
...
// symlinks `~/.op/agent/extensions/foo.ts` to a dotfiles repo would see the
