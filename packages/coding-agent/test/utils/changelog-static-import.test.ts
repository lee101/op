...
/**
 * Swap `@openpaths/utils` and the changelog module's `../config` import for a
...
function changelogUtilsStubPlugin(): BunPlugin {
	return {
		...
		setup(build) {
			build.onResolve({ filter: /^@openpaths\/pi-utils$/ }, () => ({ path: utilsStubPath }));
...
describe("bundled changelog asset path resolution", () => {
	const moduleUrl = new URL("file:///opt/op/dist/cli.js");

	test.each([
		["Windows drive-letter", String.raw`C:\op\dist\CHANGELOG.md`],
		["Windows UNC", String.raw`\\server\share\op\CHANGELOG.md`],
		["POSIX", "/opt/op/dist/CHANGELOG.md"],
...
describe("changelog static import resources", () => {
	...
	test("reads the emitted changelog asset when run outside the bundle directory", async () => {
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "op-changelog-bundle-"));
		try {
	...
	test("reads the emitted changelog asset from a compiled binary", async () => {
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "op-changelog-compiled-"));
		try {
