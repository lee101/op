...
import { QrCode, type QrEcLevel, renderQrHalfBlocks } from "@openpaths/coding-agent/utils/qrcode";
...
describe("QR encoder", () => {
	...
	const vectors: ReadonlyArray<{
		...
		{
			text: "https://my.openpaths.io/#mgAYTZwEnpRQtca0CTgn-Q.gdJU",
			ecl: "M",
			mask: 4,
			version: 4,
			size: 33,
	...
	it("deterministically selects a penalty-minimizing mask when none is forced", () => {
		...
		const qr = QrCode.encodeText("https://my.openpaths.io/#demo", "M");
...
describe("renderQrHalfBlocks", () => {
	it("frames the symbol in a light quiet zone wide enough for the margin", () => {
		const qr = QrCode.encodeText("https://openpaths.io/#demo", "M");
		const margin = 3;
