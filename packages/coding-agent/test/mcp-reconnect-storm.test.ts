...
import type { MCPStdioServerConfig } from "@openpaths/coding-agent/mcp/types";
...
describe("MCP reconnect storm (issue #1592)", () => {
	...
	beforeEach(() => {
		workDir = fs.mkdtempSync(path.join(os.tmpdir(), "op-mcp-storm-"));
	...
	it("stops respawning after a burst of immediate exits", async () => {
		...
		const config: MCPStdioServerConfig = {
			...
			command: BUN_EXEC,
			args: [FIXTURE_PATH],
			env: { OP_TEST_SPAWN_LOG: spawnLog },
		};
