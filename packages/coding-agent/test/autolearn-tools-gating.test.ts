...
import { resetActiveSkillsForTests, type Skill, setActiveSkills } from "@openpaths/coding-agent/extensibility/skills";
import type { HindsightSessionState } from "@openpaths/coding-agent/hindsight/state";
import type { MnemopiSessionState } from "@openpaths/coding-agent/mnemopi/state";
import { createTools, type ToolSession } from "@openpaths/coding-agent/tools";
import { LearnTool } from "@openpaths/coding-agent/tools/learn";
import { ManageSkillTool } from "@openpaths/coding-agent/tools/manage-skill";
...
describe("manage_skill execute", () => {
	...
	beforeEach(async () => {
		...
		tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "op-manage-skill-"));
...
describe("learn execute", () => {
	...
	beforeEach(async () => {
		originalAgentDir = getAgentDir();
		tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "op-learn-"));
		spyOn(os, "homedir").mockReturnValue(tempHome);
		setAgentDir(path.join(tempHome, ".op", "agent"));
		remembered = [];
	});
