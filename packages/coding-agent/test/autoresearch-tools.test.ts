...
import type { ImageContent, TextContent } from "@openpaths/ai";
import { createSessionRuntime } from "@openpaths/coding-agent/autoresearch/state";
import {
	type AutoresearchStorage,
	closeAllAutoresearchStorages,
	openAutoresearchStorage,
	type SessionRow,
} from "@openpaths/coding-agent/autoresearch/storage";
import { createInitExperimentTool } from "@openpaths/coding-agent/autoresearch/tools/init-experiment";
import { createLogExperimentTool } from "@openpaths/coding-agent/autoresearch/tools/log-experiment";
import { createRunExperimentTool } from "@openpaths/coding-agent/autoresearch/tools/run-experiment";
import { createUpdateNotesTool } from "@openpaths/coding-agent/autoresearch/tools/update-notes";
import type { ASIData, LogDetails, NumericMetricMap, RunDetails } from "@openpaths/coding-agent/autoresearch/types";
import type { ExtensionAPI, ExtensionContext } from "@openpaths/coding-agent/extensibility/extensions";
...
describe("init_experiment", () => {
	...
	beforeEach(() => {
		...
		process.env.OP_AUTORESEARCH_DB_DIR = dbOverride.path();
	});

	afterEach(async () => {
		delete process.env.OP_AUTORESEARCH_DB_DIR;
		closeAllAutoresearchStorages();
		await Bun.sleep(0);
