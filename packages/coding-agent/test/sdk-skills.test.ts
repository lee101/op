...
import { getActiveSkills } from "@openpaths/coding-agent/extensibility/skills";
import type { Skill } from "@openpaths/coding-agent/sdk";
...
function createExtensionSkill(packageDir: string, skillName: string): void {
	...
	fs.writeFileSync(
		...
		JSON.stringify({ name: path.basename(packageDir), op: { extensions: ["./extension.ts"] } }),
	);
...
describe("createAgentSession skills option", () => {
	...
	beforeEach(() => {
		...
		// Create skill in .op/skills/ for native project-level discovery
		skillsDir = path.join(tempDir, ".op", "skills", "test-skill");
		fs.mkdirSync(skillsDir, { recursive: true });
		originalHome = process.env.HOME;
		tempHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-sdk-home-"));
		process.env.HOME = tempHomeDir;
		const nativeUserSkillsDir = path.join(tempHomeDir, ".op", "agent", "skills");
...
`,
	...
	it("SDK invocation root scope isolates disabled discovery and merges normal discovery", async () => {
		...
		const installedPackage = path.join(tempHomeDir, ".op", "plugins", "node_modules", "sdk-installed-extension");
		...
		fs.writeFileSync(path.join(tempDir, ".op", "settings.json"), JSON.stringify({ extensions: [settingsPackage] }));
		fs.mkdirSync(path.join(tempHomeDir, ".op", "plugins"), { recursive: true });
		fs.writeFileSync(
			path.join(tempHomeDir, ".op", "plugins", "package.json"),
			JSON.stringify({ name: "op-plugins", dependencies: { "sdk-installed-extension": "1.0.0" } }),
		);

		const previousAgentDir = getAgentDir();
		setAgentDir(path.join(tempHomeDir, ".op", "agent"));
		const baseSessionOptions = {
			cwd: tempDir,
			agentDir: path.join(tempHomeDir, ".op", "agent"),
			modelRegistry: sharedModelRegistry,
	...
	it("should still discover project skills when user skills directory is missing", async () => {
		const userAgentDir = path.join(tempHomeDir, ".op", "agent");
	...
	it("refreshSkills reloads project skills on an existing session", async () => {
		...
		const runtimeSkillDir = path.join(tempDir, ".op", "skills", "runtime-added-skill");
...
`,
	...
	it("manage_skill hot-registers managed skills in the active session", async () => {
		...
		const managedAgentDir = path.join(tempHomeDir, ".op", "agent");
