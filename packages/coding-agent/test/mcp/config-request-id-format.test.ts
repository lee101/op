 ...
 *
 * Both OP-native loaders are covered: `.op/mcp.json` (native provider) and a
...
beforeEach(async () => {
	...
	tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "op-mcp-reqid-home-"));
	tempAgentDir = await fs.mkdtemp(path.join(os.tmpdir(), "op-mcp-reqid-agent-"));
	tempCwd = await fs.mkdtemp(path.join(os.tmpdir(), "op-mcp-reqid-cwd-"));
...
test("requestIdFormat from .op/mcp.json reaches the transport config", async () => {
	const configs = await loadFrom(path.join(".op", "mcp.json"), {
