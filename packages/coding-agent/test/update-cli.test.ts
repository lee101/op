...
import * as pluginCli from "@openpaths/coding-agent/cli/plugin-cli";
import * as updateCli from "@openpaths/coding-agent/cli/update-cli";
import {
	buildBunInstallArgs,
...
describe("update-cli install target detection", () => {
	it("leaves Nix store installations under Nix management", () => {
		const method = resolveUpdateMethodForTest(
			"/nix/store/0123456789-op-17.2.15/bin/op",
	...
	it("uses bun update when prioritized op is inside bun global bin", () => {
		const method = resolveUpdateMethodForTest("/Users/test/.bun/bin/op", "/Users/test/.bun/bin");

		expect(method).toBe("bun");
	});

	it("uses npm update when prioritized op is inside an npm global bin", () => {
		const method = resolveUpdateMethodForTest("/Users/test/.npm-global/bin/op", undefined, {
	...
	it("uses binary update when a plain file in the npm global bin dir is the standalone binary, not an npm symlink", () => {
		...
		const method = resolveUpdateMethodForTest("/home/u/.local/bin/op", undefined, {
	...
	it("uses binary update when a plain file in the bun global bin dir is the standalone binary", () => {
		const method = resolveUpdateMethodForTest("/home/u/.local/bin/op", "/home/u/.local/bin", {
			ompIsRegularFile: true,
		});
	...
	it("updates the standalone binary behind a foreign npm-bin alias without replacing the alias", async () => {
		...
		const standalonePath = path.join(dir, ".local", "bin", "op");
	...
	it("treats a Bun-bin alias into ~/.bun/custom as foreign", async () => {
		...
		const standalonePath = path.join(bunDir, "custom", "op");
	...
	it("uses binary update when prioritized op is outside bun global bin", () => {
		const method = resolveUpdateMethodForTest("/Users/test/.local/bin/op", "/Users/test/.bun/bin");

		expect(method).toBe("binary");
	});

	it("uses binary update when bun global bin cannot be resolved", () => {
		const method = resolveUpdateMethodForTest("/Users/test/.local/bin/op", undefined);

		expect(method).toBe("binary");
	});

	it("uses Homebrew update when prioritized op resolves into the Homebrew formula", async () => {
		const dir = await makeTempDir();
		const prefix = path.join(dir, "opt", "op");
		const linkedBin = path.join(dir, "bin");
		await fs.mkdir(path.join(prefix, "bin"), { recursive: true });
		await fs.mkdir(linkedBin, { recursive: true });
		await Bun.write(path.join(prefix, "bin", "op"), "binary");
		await fs.symlink(path.join(prefix, "bin", "op"), path.join(linkedBin, "op"));

		const method = resolveUpdateMethodForTest(path.join(linkedBin, "op"), "/Users/test/.bun/bin", {
	...
	it("uses mise update when prioritized op is in an active mise bin path", () => {
		const method = resolveUpdateMethodForTest(
			"/Users/test/.local/share/mise/installs/github-can1357-openpaths/latest/bin/op",
			undefined,
			{
				miseBinDirs: ["/Users/test/.local/share/mise/installs/github-can1357-openpaths/latest/bin"],
			},
		);
	...
	it("uses mise update when prioritized op is a mise shim", () => {
		const method = resolveUpdateMethodForTest("/Users/test/.local/share/mise/shims/op", undefined, {
...
describe("update-cli package manager commands", () => {
	it("targets the Homebrew tap formula and switches to reinstall for forced updates", () => {
		expect(buildHomebrewUpdateArgs(false)).toEqual(["upgrade", "can1357/tap/op"]);
	...
	it("pins npm package installs to the official registry and the checked native package versions", () => {
		...
		expect(args).toContain("@openpaths/coding-agent@16.3.15");
		expect(args).toContain("@openpaths/natives@16.3.15");
		expect(args).toContain("@openpaths/natives-win32-x64@16.3.15");
	});
});

describe("update-cli npm rename contract", () => {
	it("parses a well-formed op.rename pointer and rejects malformed ones", () => {
		expect(resolveReleaseRename({ op: { rename: { package: "@new/op", natives: "@new/natives" } } })).toEqual({
			pkg: "@new/op",
			natives: "@new/natives",
		});
		expect(resolveReleaseRename({ op: { rename: { package: "@new/op" } } })).toEqual({
			pkg: "@new/op",
			natives: undefined,
		});
		expect(resolveReleaseRename({ op: { rename: { package: "" } } })).toBeUndefined();
		expect(resolveReleaseRename({ op: { rename: "@new/op" } })).toBeUndefined();
		expect(resolveReleaseRename({ op: {} })).toBeUndefined();
	...
	it("installs renamed package names in lock-step, with no old-name leftovers in the argv", () => {
		const packages = { pkg: "@new/op", natives: "@new/natives" };

		const bunArgs = buildBunInstallArgs("17.0.0", "linux-x64", packages);
		expect(bunArgs).toContain("@new/op@17.0.0");
		expect(bunArgs).toContain("@new/natives@17.0.0");
		expect(bunArgs).toContain("@new/natives-linux-x64@17.0.0");
		expect(bunArgs.some(arg => arg.startsWith("@openpaths/"))).toBe(false);

		expect(buildNpmInstallArgs("17.0.0", "linux-x64", packages)).toContain("@new/op@17.0.0");
	});
	...
	it("removes the old agent package and its natives companions when both names moved", () => {
		...
		expect(buildRenameCleanupPackages(packages, "darwin-arm64")).toEqual([
			...
			"@openpaths/natives",
			"@openpaths/natives-darwin-arm64",
		]);
	...
	it("keeps the natives packages on an agent-only rename so cleanup cannot strip the addon the new install pinned", () => {
		const packages = { pkg: "@new/op", natives: "@openpaths/natives" };
		expect(buildRenameCleanupPackages(packages, "darwin-arm64")).toEqual(["@openpaths/coding-agent"]);
		expect(buildRenameCleanupPackages(packages, "linux-arm")).toEqual(["@openpaths/coding-agent"]);
	});
});
			...
			steps: {
				...
				async verify() {
					...
					return script.verify[verifies++]
						? { ok: true, actual: "999.1.0", path: "/bin/op" }
						: { ok: false, path: "/bin/op" };
				},
			},
		};
	}
...
describe("update-cli bun install command", () => {
	it("pins the official npm registry and bypasses the manifest cache so a stale mirror or snapshot cannot mask a freshly published version", () => {
		// Regression: op queries https://registry.npmjs.org/<pkg>/latest directly.
		...
		//     the release.
		// See https://github.com/lee101/op/issues/1686.
	...
	it("pins the native addon core and the platform-specific leaf to the same version so the loader sentinel cannot drift on supported tags", () => {
		...
		// package, leaving @openpaths/natives and @openpaths/natives-<tag>
	...
	it("omits the leaf on unsupported platform tags so an EBADPLATFORM swap does not mask the underlying `no matching version` error", () => {
		...
		expect(args).toContain("@openpaths/natives@15.9.0");
		expect(args.some(arg => arg.startsWith("@openpaths/natives-"))).toBe(false);
	});
...
describe("update-cli bun cache pruning", () => {
	it("keeps only the newest cached version for filtered global install packages", async () => {
		...
		const result = await pruneBunInstallCache(dir, new Set(["react", "@openpaths/utils"]));
...
describe("update-cli release binary integrity", () => {
	...
	it("rejects release metadata that does not identify one exact stable asset", () => {
		...
		expect(() =>
			resolveReleaseBinaryAsset(
				releaseAsset({ browser_download_url: "https://example.com/op-linux-x64" }),
				tag,
				binaryName,
			),
...
describe("update-cli binary replacement", () => {
	it("restores the previous binary when the replacement fails verification", async () => {
		...
		).rejects.toThrow("restored previous op binary");
...
describe("update-cli script-shim takeover", () => {
	...
	it("installs op.exe beside the shims and retires them", async () => {
		const dir = await makeTempDir();
	...
	it("restores the shims and removes the exe when the exe reports the wrong version", async () => {
		...
		).rejects.toThrow(/still reports 17\.2\.12 \(expected 18\.0\.0\); restored previous op launcher/);
	...
	function renameLockingPs1(): Mock<typeof nodeFs.promises.rename> {
		...
		return spyOn(nodeFs.promises, "rename").mockImplementation(async (from, to) => {
			if (path.basename(String(from)) === "op.ps1") {
	...
	it("rewrites an immovable precedence-winning shim as a forwarder to the exe", async () => {
		...
		expect(await Bun.file(path.join(dir, "op.exe")).text()).toBe(exe);
		expect(await Bun.file(path.join(dir, "op")).exists()).toBe(false);
		expect(await Bun.file(path.join(dir, "op.cmd")).exists()).toBe(false);
		// PowerShell resolves .ps1 before .exe: the locked shim must now exec
		// the new binary instead of keeping its old body.
		expect(await Bun.file(path.join(dir, "op.ps1")).text()).toContain('& "$PSScriptRoot\\op.exe" @args');
	});
	...
	it("restores a forwarded shim's original body when verification fails", async () => {
		...
		try {
			...
			).rejects.toThrow("restored previous op launcher");
		} finally {
			renameSpy.mockRestore();
		}
...
describe("update-cli concurrent binary updates", () => {
	...
	// Regression for #8434: two overlapping `op update` runs must not share a
