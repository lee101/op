...
} from "@openpaths/coding-agent/task/isolation-runner";
...
async function seedFooRepo(finalContent: string): Promise<{ repoRoot: string; patchPath: string }> {
	const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "op-isolation-merge-"));
...
describe("runIsolatedSubprocess", () => {
	...
	it("preserves branch-mode output as a patch when branch transfer fails", async () => {
		const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "op-isolation-run-"));
		tempRoots.push(repoRoot);
		...
		expect(deleteSpy).toHaveBeenCalledWith(repoRoot, "op/task/PreserveBranchFailure");
