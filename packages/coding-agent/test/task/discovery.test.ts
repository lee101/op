...
import { disableProvider, enableProvider } from "@openpaths/coding-agent/capability";
...
const OP_AGENT_MD = [
	"---",
	"name: op-test-agent",
	"description: OP-native test agent.",
	"---",
	"You are an OP task agent.",
].join("\n");

const OP_PLUGIN_AGENT_MD = [
	"---",
...
async function writeOmpPluginAgent(home: string): Promise<void> {
	const userPluginsRoot = path.join(home, ".op", "plugins");
	...
	await fs.writeFile(
		...
		JSON.stringify({ name: "loom", version: "1.0.0", op: { version: "1.0.0" } }),
	);
	await fs.writeFile(
		path.join(userPluginsRoot, "package.json"),
		JSON.stringify({
			name: "op-plugins-root",
			version: "0.0.0",
			dependencies: { loom: "1.0.0" },
		}),
	);
	await fs.writeFile(path.join(pluginRoot, "agents", "loom-verify-spec.md"), OP_PLUGIN_AGENT_MD);
}
...
describe("discoverAgents", () => {
	...
	beforeEach(async () => {
		tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "op-task-agent-discovery-"));
		projectDir = path.join(tempHome, "project");
		await fs.mkdir(projectDir, { recursive: true });
	});

	afterEach(async () => {
		enableProvider("op-plugins");
		clearOmpExtensionCliRoots();
		clearFsCache();
		await removeWithRetries(tempHome);
	});

	test("loads OP agents but skips Claude Code custom agents", async () => {
		await fs.mkdir(path.join(projectDir, ".op", "agents"), { recursive: true });
		await fs.writeFile(path.join(projectDir, ".op", "agents", "op-test-agent.md"), OP_AGENT_MD);
		...
		expect(names).toContain("op-test-agent");
		expect(names).not.toContain("cc-test-agent");
		expect(projectAgentsDir).toBe(path.join(projectDir, ".op", "agents"));
	});

	test("loads agents from OP npm plugins under <home>/.op/plugins/node_modules", async () => {
	...
	test("excludes OP npm plugin agents when op-plugins is disabled", async () => {
		await writeOmpPluginAgent(tempHome);
		disableProvider("op-plugins");
	...
	test("CLI extension agents win over project `extensions:` settings on dedup", async () => {
		...
		// the skills/hooks/tools surface in discovery/op-plugins.ts.
		...
		await fs.mkdir(path.join(projectDir, ".op"), { recursive: true });
		await fs.writeFile(path.join(projectDir, ".op", "settings.json"), JSON.stringify({ extensions: [projectExt] }));
	...
	test("explicit-only CLI roots expose only explicitly named package agents", async () => {
		...
		await fs.mkdir(path.join(projectDir, ".op"), { recursive: true });
		await fs.writeFile(path.join(projectDir, ".op", "settings.json"), JSON.stringify({ extensions: [settingsExt] }));
		await writeOmpPluginAgent(tempHome);
