...
import * as planHandoff from "@openpaths/coding-agent/plan-mode/plan-handoff";
...
import * as isolationRunner from "@openpaths/coding-agent/task/isolation-runner";
import {
...
} from "@openpaths/coding-agent/task/structured-subagent";
import type { AgentDefinition, SingleResult } from "@openpaths/coding-agent/task/types";
...
describe("structured subagent primitive", () => {
	...
	it("leases temporary artifacts for a retained invocation and registers them for agent URLs", async () => {
		...
		expect(path.basename(settled.artifactsDir)).toStartWith("op-task-");
		await fs.rm(settled.artifactsDir, { recursive: true, force: true });
	});
	...
	it("persists nested patch text with the compatible recovery path and wording", async () => {
		const artifactsDir = await fs.mkdtemp(path.join(os.tmpdir(), "op-structured-subagent-"));
		const completed = result();
		completed.patchPath = "/recovery/Worker.patch";
		completed.branchName = "op/task/Worker";
		...
		expect(hint).toContain("Captured branch preserved as op/task/Worker.");
