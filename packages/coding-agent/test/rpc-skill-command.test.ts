...
import { tryRunRpcSkillCommand } from "@openpaths/coding-agent/modes/rpc/rpc-mode";
import { type CustomMessage, SKILL_PROMPT_MESSAGE_TYPE } from "@openpaths/coding-agent/session/messages";
import { removeWithRetries, Snowflake } from "@openpaths/utils";

describe("tryRunRpcSkillCommand", () => {
	test("dispatches registered /skill commands as skill prompt messages", async () => {
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), `op-rpc-skill-${Snowflake.next()}-`));
