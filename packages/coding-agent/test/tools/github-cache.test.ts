 ...
 *
 * Each test isolates `OP_GITHUB_CACHE_DB` to a temp file and clears
...
import { getOrFetchIssue, getOrFetchPr } from "@openpaths/coding-agent/tools/gh";
import {
	clearAll,
	getCached,
	getOrFetchView,
...
} from "@openpaths/coding-agent/tools/github-cache";
import { ToolAbortError, throwIfAborted } from "@openpaths/coding-agent/tools/tool-errors";
...
beforeEach(async () => {
	...
	originalEnv = process.env.OP_GITHUB_CACHE_DB;
	process.env.OP_GITHUB_CACHE_DB = path.join(tempDir, "github-cache.db");
	resetCacheForTests();
});
...
afterEach(async () => {
	...
	if (originalEnv === undefined) {
		delete process.env.OP_GITHUB_CACHE_DB;
	} else {
		process.env.OP_GITHUB_CACHE_DB = originalEnv;
	}
	vi.restoreAllMocks();
...
describe("github-cache db layer", () => {
	...
	it("does not chmod an existing cache parent directory", async () => {
		...
		process.env.OP_GITHUB_CACHE_DB = path.join(parent, "github-cache.db");
		resetCacheForTests();
