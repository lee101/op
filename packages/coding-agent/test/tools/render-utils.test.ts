...
import { KeybindingsManager, setKeyHintPlatform } from "@openpaths/coding-agent/config/keybindings";
import { getThemeByName, initTheme, type Theme, theme } from "@openpaths/coding-agent/modes/theme/theme";
import {
	dedupeParseErrors,
	expandKeyHint,
	formatCodeFrameLine,
	...
	shortenPath,
	truncateDiffByHunk,
} from "@openpaths/coding-agent/tools/render-utils";
...
describe("formatScreenshot", () => {
	...
	it("formats non-home path without tilde", () => {
		const filePath = path.join(path.parse(os.homedir()).root, "op-render-utils", "capture.png");
		const resized = fakeResized({ mimeType: "image/webp", buffer: new Uint8Array(1024) });
	...
	it("formats temp-only screenshot without save line", () => {
		...
		expect(
			formatScreenshot({
				...
				dest: path.join(os.tmpdir(), "op-sshots-123.png"),
				resized,
			}),
