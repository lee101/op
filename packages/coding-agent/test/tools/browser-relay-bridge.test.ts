...
import { RelayBridge, type RelaySocket } from "@openpaths/coding-agent/tools/browser/relay/bridge";
import type {
	RelayRpcRequest,
	RelayToExtMessage,
	TabSnapshot,
} from "@openpaths/coding-agent/tools/browser/relay/protocol";
...
/**
 * Emulate the op tab worker adopting a tab: attach to its page target, then
...
async function claimTab(
	...
	bridge.cdpMessage(connId, JSON.stringify({ id: ++msgSeq, sessionId, method: "OP.claimTarget" }));
	await flush();
}

describe("RelayBridge tab grouping", () => {
	it("groups nothing on hello or tab lifecycle events — only claimed tabs join the op group", () => {
		const bridge = new RelayBridge({ group: { title: "op", color: "cyan" } });
	...
	it("never re-groups a tab the user pulled out of the op group", async () => {
		const bridge = new RelayBridge({ group: { title: "op", color: "cyan" } });
		const ext = new FakeExtSocket();
		connect(bridge, ext, [tab({ tabId: 1 })]);
		const cdp = new FakeCdpSocket();
	...
	it("never overlaps group RPCs: a tab claimed mid-flight waits for the pending group", async () => {
		...
		// and mint duplicate "op" groups; the second request must queue.
	...
	it("regroups claimed tabs after an extension reconnect instead of treating the dissolve as user opt-out", async () => {
		...
		ack(bridge, ext, "group", { grouped: { "1": 42 } });
		await flush();
		// Relay/extension link drops: the extension dissolves the op group on
