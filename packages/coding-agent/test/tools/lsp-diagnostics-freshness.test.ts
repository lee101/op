...
import { createLspWritethrough, type FileDiagnosticsResult, FileFormatResult } from "@openpaths/coding-agent/lsp";
import * as lspClient from "@openpaths/coding-agent/lsp/client";
import * as lspConfig from "@openpaths/coding-agent/lsp/config";
import type { Diagnostic, LinterClient, LspClient, ServerConfig } from "@openpaths/coding-agent/lsp/types";
import { EquivalentUriMap, fileToUri } from "@openpaths/coding-agent/lsp/utils";
import type { DeferredDiagnosticsEntry, ToolSession } from "@openpaths/coding-agent/tools";
import { WriteTool } from "@openpaths/coding-agent/tools/write";
import { type ptree, TempDir } from "@openpaths/utils";
...
describe("LSP diagnostics freshness", () => {
	...
	beforeEach(() => {
		tempDir = TempDir.createSync("@op-lsp-freshness-");
	});
	...
	it("suppresses TypeScript project diagnostics for orphan files but keeps syntax errors", async () => {
		...
		const orphanDir = TempDir.createSync("@op-lsp-orphan-");
		try {
