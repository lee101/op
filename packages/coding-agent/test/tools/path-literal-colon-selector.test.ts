...
} from "@openpaths/coding-agent/tools/path-utils";
import { ReadTool } from "@openpaths/coding-agent/tools/read";
import { GrepOutputMode } from "@openpaths/natives";
...
describe("leading-colon path recovery (issue #5508)", () => {
	...
	it("strips a leading colon before an absolute path in resolveToCwd", () => {
		expect(resolveToCwd(":/tmp/op-colon-test.txt", tmpDir)).toBe("/tmp/op-colon-test.txt");
	});
...
// Regression: the `op grep` CLI subcommand resolved its path argument with a
