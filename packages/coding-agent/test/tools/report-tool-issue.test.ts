...
import * as reportIssue from "@openpaths/coding-agent/tools/report-tool-issue";
import {
...
} from "@openpaths/coding-agent/tools/report-tool-issue";
...
describe("flushGrievances", () => {
	...
	it("posts pending rows with bearer header and marks them pushed=1 on 200", async () => {
		...
		expect(body.agent?.name).toBe("op");
		...
		// stays so users can browse what they've shipped via `op grievances`.
