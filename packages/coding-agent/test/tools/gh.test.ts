...
import type { ToolCall } from "@openpaths/ai";
...
} from "@openpaths/coding-agent/tools/gh";
import * as git from "@openpaths/coding-agent/utils/git";
import * as piUtils from "@openpaths/utils";
import { $which, getAgentDir, hashPath, removeWithRetries, setAgentDir, WhichCachePolicy } from "@openpaths/utils";
 ...
 * user's real `~/.op/wt`. Returns the temp home and a cleanup hook.
 */
interface TempHome {
	home: string;
...
async function setupTempHome(): Promise<{ home: string; cleanup: () => Promise<void> }> {
	...
	// through the spied homedir instead of `$XDG_DATA_HOME/op/wt` (CI sets these).
...
async function expectedWorktreePath(home: string, primaryRoot: string, localBranch: string): Promise<string> {
	...
	return fs.realpath(path.join(home, ".op", "wt", segment));
}
...
describe("github tool", () => {
	...
	it("reads repository files through the GitHub contents API", async () => {
		...
		expect(textSpy).toHaveBeenCalledWith(
			...
			[
				"api",
				"/repos/lee101/op/contents/packages/coding-agent/package.json",
				"--method",
				"GET",
				"-H",
	...
	describe("pr_checkout (single, cross-repository) and git remote handling", () => {
		...
		describe("git.remote.add idempotency", () => {
			...
			it("does not depend on localized git remote-add stderr for existing remotes", async () => {
				...
				const fakeBin = await fs.mkdtemp(path.join(os.tmpdir(), "op-fake-git-"));
...
`,
			...
			it("pins Git messages while preserving UTF-8 character locale", async () => {
				...
				const fakeBin = await fs.mkdtemp(path.join(os.tmpdir(), "op-fake-git-locale-"));
...
`,
	...
	it("pins gh messages while preserving UTF-8 character locale", async () => {
		...
		const fakeBin = await fs.mkdtemp(path.join(os.tmpdir(), "op-fake-gh-locale-"));
