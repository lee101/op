...
import { resetActiveSkillsForTests, setActiveSkills } from "@openpaths/coding-agent/extensibility/skills";
import {
...
} from "@openpaths/coding-agent/internal-urls";
import { AgentRegistry } from "@openpaths/coding-agent/registry/agent-registry";
import * as sshFileTransfer from "@openpaths/coding-agent/ssh/file-transfer";
...
describe("GrepTool internal URL resolution", () => {
	...
	it("expands op:// root to grep embedded documentation files", async () => {
		const session = createSession();
		const tool = new GrepTool(session);
		...
		const text = getResultText(result);
		expect(text).toContain("# op://tools/grep.md");
		expect(text).toContain("Grep file contents with a regex across files");
	});

	it("expands op://docs to grep embedded documentation files", async () => {
		const session = createSession();
		const tool = new GrepTool(session);
		...
		const text = getResultText(result);
		expect(text).toContain("# op://tools/read.md");
