 ...
 * crashes OP. OP now owns the profile directory and removes it itself with a
...
import { removeUserDataDir } from "@openpaths/coding-agent/tools/browser/launch";
import { type BrowserHandle, releaseBrowser } from "@openpaths/coding-agent/tools/browser/registry";
import * as piUtils from "@openpaths/utils";

async function makeProfileDir(): Promise<string> {
	const dir = await fs.promises.mkdtemp(path.join(os.tmpdir(), "op-chrome-profile-test-"));
