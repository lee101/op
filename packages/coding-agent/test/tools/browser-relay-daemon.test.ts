...
const url = Bun.env.OP_TEST_RELAY_URL;
...
process.stdout.write(String(await probeRelayServer(url)));`,
			],
			{
				env: {
					...
					NO_PROXY: "",
					no_proxy: "",
					OP_TEST_RELAY_URL: `http://127.0.0.1:${relay.port}`,
				},
				stdout: "pipe",
				stderr: "pipe",
			},
	...
	it("stays alive while a consumer in another project holds the global broker lease", async () => {
		const home = await fs.mkdtemp(path.join(os.tmpdir(), "op-relay-global-"));
		...
		const globalRuntimeDir = path.join(home, ".op", "run", "daemons", "global", "browser-relay");
...
const cdpUrl = process.env.OP_TEST_RELAY_URL;
const marker = process.env.OP_TEST_READY_MARKER;
		...
		const spawnConsumer = (cwd: string, profile: string, marker: string) =>
			Bun.spawn([process.execPath, scriptPath], {
				...
				env: {
					...
					PI_CONFIG_DIR: ".op",
					OP_PROFILE: profile,
					OP_DAEMON_IDLE_GRACE_MS: "200",
					OP_TEST_RELAY_URL: cdpUrl,
					OP_TEST_READY_MARKER: marker,
				},
				stdin: "pipe",
				stdout: "ignore",
				stderr: "pipe",
