...
import { acquireBrowser, getBrowsersMapForTest } from "@openpaths/coding-agent/tools/browser/registry";
import {
	acquireTab,
	getTabsMapForTest,
	releaseTab,
	releaseTabsForOwner,
} from "@openpaths/coding-agent/tools/browser/tab-supervisor";
import { ToolAbortError } from "@openpaths/coding-agent/tools/tool-errors";

function makeKind(socketSuffix: string): CmuxKind {
	return { kind: "cmux", socketPath: `/tmp/op-test-${socketSuffix}.sock`, surface: `surface-${socketSuffix}` };
}
...
describe("browser lifecycle — close deadlines", () => {
	...
	it("rejects a stuck close with the backend, tab, and pending resource", async () => {
		...
		const kind: CmuxKind = { kind: "cmux", socketPath: "/tmp/op-close-deadline.sock" };
		const browser = await acquireBrowser(kind, { cwd: "/tmp" });
