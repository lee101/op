...
import { DEFAULT_FUZZY_THRESHOLD, executePatchSingle } from "@openpaths/coding-agent/edit";
import type { FileDiagnosticsResult } from "@openpaths/coding-agent/lsp";
...
beforeEach(async () => {
	...
	tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "op-patch-unchanged-"));
