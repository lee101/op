...
import { BUILTIN_SLASH_COMMANDS } from "@openpaths/coding-agent/slash-commands/builtin-registry";
...
describe("/move directory completion", () => {
	...
	beforeEach(async () => {
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "op-move-completion-"));
	...
	it("completes parent directory paths", async () => {
		...
		const siblingName = `op-move-sibling-${path.basename(tempDir)}`;
