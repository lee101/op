...
import { AuthBrokerError } from "@openpaths/ai/auth-broker";
import { MissingApiKeyError } from "@openpaths/ai/error";
import { parseArgs } from "@openpaths/coding-agent/cli/args";
import { runRootCommand } from "@openpaths/coding-agent/main";
import { describeAuthBrokerStartupError } from "@openpaths/coding-agent/session/auth-broker-config";
import { setInteractiveHost } from "@openpaths/utils";
...
describe("describeAuthBrokerStartupError", () => {
	it("turns a broker connection failure into recovery guidance", async () => {
		...
		expect(message).toContain("op auth-broker serve");
		expect(message).toContain("op config reset auth.broker.url");
		expect(message).toContain("OP_AUTH_BROKER_URL");
	});

	it("names the configured broker URL when it can be resolved", async () => {
		const prevUrl = process.env.OP_AUTH_BROKER_URL;
		const prevToken = process.env.OP_AUTH_BROKER_TOKEN;
		process.env.OP_AUTH_BROKER_URL = "http://127.0.0.1:8765";
		process.env.OP_AUTH_BROKER_TOKEN = "test-token";
		try {
			...
			if (prevUrl === undefined) delete process.env.OP_AUTH_BROKER_URL;
			else process.env.OP_AUTH_BROKER_URL = prevUrl;
			if (prevToken === undefined) delete process.env.OP_AUTH_BROKER_TOKEN;
			else process.env.OP_AUTH_BROKER_TOKEN = prevToken;
		}
	});

	it("passes through a missing-token message unchanged", async () => {
		const err = new MissingApiKeyError(undefined, "OP_AUTH_BROKER_URL is set but no bearer token is available.");
...
describe("runRootCommand — unreachable auth broker at startup", () => {
	it("exits 1 with an actionable message instead of an uncaught AuthBrokerError", async () => {
		...
		expect(stderr).toContain("op auth-broker serve");
	}, 15_000);
});
