...
describe("profile alias installer", () => {
	it("writes a bash-compatible function that forwards subcommands through op", async () => {
		...
		expect(result.command).toBe("op --profile=work");
		expect(files.get("/home/me/.bashrc")).toContain("op-work() {");
		expect(files.get("/home/me/.bashrc")).toContain('command op --profile=work "$@"');
	});
	...
	it("can target the current source invocation instead of the installed op binary", async () => {
		const files = new Map<string, string>();
		...
		expect(files.get("/Users/me/.zshrc")).toContain("op-work() {");
	...
	it("installs the zsh alias under ZDOTDIR when set", async () => {
		...
		expect(files.get(result.configPath)).toContain("op-work() {");
	});
	...
	it("writes a fish function that forwards argv", async () => {
		...
		const content = files.get("/Users/me/.config/fish/conf.d/op-profiles.fish") ?? "";
		expect(content).toContain("function op-work --wraps op");
		expect(content).toContain("command op --profile=work $argv");
	});
	...
	it("installs the fish alias under XDG_CONFIG_HOME when set", async () => {
		...
		expect(result.configPath).toBe("/home/me/.dotfiles/config/fish/conf.d/op-profiles.fish");
		expect(files.get(result.configPath)).toContain("function op-work --wraps op");
	});
	...
	it("writes a PowerShell function because aliases cannot carry arguments", async () => {
		...
		expect(content).toContain("function op-work");
		expect(content).toContain("& op --profile=work @args");
	});
	...
	it("detects pwsh from PSModulePath when SHELL is unset on Windows", async () => {
		...
		expect(files.get(result.configPath)).toContain("& op --profile=work @args");
	});
	...
	it("replaces a previous block for the same alias", async () => {
		const files = new Map<string, string>([
			[
				...
				[
					"before",
					"# >>> op profile alias: op-work >>>",
					"alias op-work='command op --profile=old'",
					"# <<< op profile alias: op-work <<<",
					"after",
				].join("\n"),
			],
		...
		expect(content).toContain('command op --profile=work "$@"');
	...
	it("refuses to rewrite a malformed managed block missing its end marker", async () => {
		...
		const original = ["# >>> op profile alias: op-work >>>", "op-work() {", "export SECRET=keepme"].join("\n");
	...
	it("refuses to shadow the base op command case-insensitively", async () => {
		for (const aliasName of ["op", "OP"]) {
			await expect(
				installProfileAlias({
					profile: "work",
					aliasName,
	...
	it("validates profile names before rendering shell code", async () => {
		...
		).rejects.toThrow("Invalid OP profile");
	...
	it("normalizes backslashes in XDG_CONFIG_HOME for fish config paths on Windows", async () => {
		...
		expect(result.configPath).toBe("D:/xdg/fish/conf.d/op-profiles.fish");
		expect(result.reloadedWith).toBe("source 'D:/xdg/fish/conf.d/op-profiles.fish'");
	});
