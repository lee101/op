...
import { parseArgs, reportUnrecognizedFlags } from "@openpaths/coding-agent/cli/args";
import { applyExtensionFlags } from "@openpaths/coding-agent/cli/extension-flags";

// Regression coverage for issue #2459: `op --list-models` (a stale flag) was
...
describe("parseArgs — unrecognized flag tracking (#2459)", () => {
	...
	it("treats every token after `--` as a positional, even flag-shaped ones (#2461 review)", () => {
		// `op -p -- --explain-this` must forward `--explain-this` as the
