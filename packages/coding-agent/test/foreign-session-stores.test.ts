...
beforeEach(async () => {
	tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "op-foreign-sessions-"));
...
describe("foreign session persistence", () => {
	it("writes a fresh OP identity with source provenance", async () => {
		const { info, store } = await createClaudeFixture();
		const sessionDir = path.join(tempRoot, "op-sessions");
