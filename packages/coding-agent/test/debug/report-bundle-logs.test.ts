...
import { getConfigRootDir, getLogsDir, removeWithRetries, setAgentDir } from "@openpaths/utils";
...
describe("report bundle logs", () => {
	it("collects every same-day PID log, not only the current process", async () => {
		cleanupRoot = await fs.mkdtemp(path.join(os.tmpdir(), "op-report-logs-"));
		...
		const crashedName = `op.${today}.4242.log`;
		const rotatedName = `${crashedName}.1`;
		const currentName = `op.${today}.${process.pid}.log`;
