...
import { applyDirenvPreflight, executeBash } from "@openpaths/coding-agent/exec/bash-executor";
import { findEnvrc, loadDirenvEnv, parseDirenvExport } from "@openpaths/coding-agent/exec/direnv";
...
describe.skipIf(!hasDirenv)("bash executor direnv wiring (end-to-end)", () => {
	...
	it("removes variables the .envrc unsets from the command environment", async () => {
		...
		await allowEnvrc(root);
		// Inherited from the process env (as an OP-provided var would be); the
