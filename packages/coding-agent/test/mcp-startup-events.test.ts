...
describe("mcp/startup-events — connection-status cross-module contract", () => {
	...
	it("sanitizes failure errors before rendering them in status text", () => {
		const homePath = `${os.homedir()}/.op/mcp.log`;
		...
		expect(message).not.toContain("\t");
		expect(message).toContain("broken: failed at   ~/.op/mcp.log");
	});
	...
	it("sanitizes server names before rendering them in status text", () => {
		...
		expect(message).not.toContain("\n");
		expect(message).not.toContain("\t");
		expect(message).toContain("Connected: ~/.op/connected   server.");
		expect(message).toContain("Failed: ~/.op/broken server: missing command.");
		expect(message).toContain("Still connecting: ~/.op/pending");
	});
