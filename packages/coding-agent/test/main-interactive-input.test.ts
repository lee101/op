...
} from "@openpaths/coding-agent/main";
import type { SubmittedUserInput } from "@openpaths/coding-agent/modes/types";
import type { CreateAgentSessionOptions } from "@openpaths/coding-agent/sdk";
import { discoverTitleSystemPromptFile } from "@openpaths/coding-agent/system-prompt";
...
describe("discoverTitleSystemPromptFile", () => {
	it("discovers TITLE_SYSTEM.md from the project op config directory", async () => {
		const projectDir = await fs.mkdtemp(path.join(os.tmpdir(), "op-title-system-"));
		cleanupDirs.push(projectDir);
		const configDir = path.join(projectDir, ".op");
