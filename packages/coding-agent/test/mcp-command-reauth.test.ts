...
import * as mcpClient from "@openpaths/coding-agent/mcp/client";
import * as oauthFlow from "@openpaths/coding-agent/mcp/oauth-flow";
import type { MCPServerConfig } from "@openpaths/coding-agent/mcp/types";
import { MCPCommandController } from "@openpaths/coding-agent/modes/controllers/mcp-command-controller";
import { OAuthManualInputManager } from "@openpaths/coding-agent/modes/oauth-manual-input";
...
describe("/mcp auth commands", () => {
	...
	beforeEach(async () => {
		projectDir = await fs.mkdtemp(path.join(os.tmpdir(), "op-mcp-reauth-project-"));
		agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "op-mcp-reauth-agent-"));
