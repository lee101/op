...
import type { Api, Context, Message, Model, ModelSpec } from "@openpaths/ai";
import { clearCustomApis, registerCustomApi } from "@openpaths/ai";
...
import {
	...
	withDateCwdReminder,
} from "@openpaths/coding-agent/session/date-cwd-reminder";
import { SessionManager } from "@openpaths/coding-agent/session/session-manager";
import { formatLocalCalendarDate } from "@openpaths/coding-agent/utils/local-date";
import { normalizePromptPath } from "@openpaths/coding-agent/utils/prompt-path";
...
describe("date-cwd-reminder", () => {
	...
	describe("renderDateCwdReminder", () => {
		it("renders a system-reminder block carrying the date and cwd with a do-not-repeat instruction", () => {
			const reminder = renderDateCwdReminder("2026-08-14", "C:/work/op");
			...
			expect(reminder).toContain("C:/work/op");
	...
	describe("withDateCwdReminder", () => {
		...
		it("injects the reminder into the first user message and keeps the system prompt bytes", () => {
			...
			const out = withDateCwdReminder(context, "2026-08-14", "/work/op");

			expect(out).not.toBe(context);
			expect(out.systemPrompt).toBe(systemPrompt);
			expect(out.messages[0]).toEqual({
				role: "user",
				content: `${renderDateCwdReminder("2026-08-14", "/work/op")}\n\ndo the thing`,
				timestamp: 1,
			});
		});
	});
