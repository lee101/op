...
import { type Args, parseArgs } from "@openpaths/coding-agent/cli/args";
import * as modelResolverModule from "@openpaths/coding-agent/config/model-resolver";
import { Settings } from "@openpaths/coding-agent/config/settings";
import * as pluginHelpers from "@openpaths/coding-agent/discovery/helpers";
import { createSessionManager, runRootCommand } from "@openpaths/coding-agent/main";
import { AuthStorage } from "@openpaths/coding-agent/session/auth-storage";
import type { SessionHeader } from "@openpaths/coding-agent/session/session-entries";
import type { SessionInfo } from "@openpaths/coding-agent/session/session-listing";
import * as sessionListingModule from "@openpaths/coding-agent/session/session-listing";
...
import { getProjectDir, normalizePathForComparison, setProjectDir } from "@openpaths/utils";
...
describe("createSessionManager — cross-project --resume", () => {
	...
	beforeEach(async () => {
		existingProject = await fsp.mkdtemp(path.join(os.tmpdir(), "op-xproj-"));
...
describe("runRootCommand — cross-project --resume", () => {
	...
	beforeEach(async () => {
		...
		root = await fsp.mkdtemp(path.join(os.tmpdir(), "op-xproj-root-"));
...
describe("createSessionManager — cross-project --resume relocation (moved worktree)", () => {
	...
	beforeEach(async () => {
		missingRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "op-moved-xproj-"));
