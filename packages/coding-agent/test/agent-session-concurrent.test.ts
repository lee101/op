...
import { Agent, AgentBusyError, type AgentMessage, type AgentTool } from "@openpaths/agent-core";
import type { AssistantMessage, Message, ToolCall } from "@openpaths/ai";
...
import { TtsrManager } from "@openpaths/coding-agent/export/ttsr";
...
import { GoalRuntime } from "@openpaths/coding-agent/goals/runtime";
...
describe("AgentSession concurrent prompt guard", () => {
	...
	// already processing` from op-rpc clients (robomp triage reminder path).
...
describe("AgentSession TTSR resume gate", () => {
	...
	it("relativizes the rule file path in the TTSR interrupt injection (no absolute leak)", async () => {
		...
		const ruleAbsPath = path.join(cwd, ".op", "rules", "no-unwrap.md");
