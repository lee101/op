...
import { type ExtensionModule, extensionModuleCapability } from "@openpaths/coding-agent/capability/extension-module";
import { resetSettingsForTest, Settings } from "@openpaths/coding-agent/config/settings";
import { getCapability, initializeWithSettings } from "@openpaths/coding-agent/discovery";
import {
	discoverAndLoadExtensions,
	discoverExtensionPaths,
	loadExtensions,
} from "@openpaths/coding-agent/extensibility/extensions/loader";
import { discoverSessionExtensionPaths } from "@openpaths/coding-agent/sdk";
...
describe("extensions discovery", () => {
	...
	it("discovers a symlinked extension directory with a package.json manifest", async () => {
		...
		fs.writeFileSync(
			...
			JSON.stringify({ name: "ctk", op: { extensions: ["./index.ts"] } }),
		);
