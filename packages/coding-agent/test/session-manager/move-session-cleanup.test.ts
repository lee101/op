...
import { cleanupEmptyMoveSession, SessionManager } from "@openpaths/coding-agent/session/session-manager";
...
describe("move-session cleanup tracking", () => {
	...
	beforeEach(async () => {
		testAgentDir = await fsp.mkdtemp(path.join(os.tmpdir(), "op-move-cleanup-"));
