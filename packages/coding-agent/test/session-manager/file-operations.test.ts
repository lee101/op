...
import type { FileEntry, SessionHeader } from "@openpaths/coding-agent/session/session-entries";
import { findMostRecentSession, resolveResumableSession } from "@openpaths/coding-agent/session/session-listing";
...
describe("SessionManager temp cwd session dirs", () => {
	...
	beforeEach(() => {
		testAgentDir = fs.mkdtempSync(path.join(os.tmpdir(), "op-session-dir-test-"));
...
describe("SessionManager legacy session migration persistence", () => {
	...
	beforeEach(() => {
		...
		testAgentDir = fs.mkdtempSync(path.join(os.tmpdir(), "op-session-manager-legacy-agent-"));
		setAgentDir(testAgentDir);
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "op-session-manager-legacy-"));
	});
