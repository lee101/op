...
import { FileSessionStorage, type WriteTextAtomicOptions } from "@openpaths/coding-agent/session/session-storage";
import type { SessionTitleUpdate } from "@openpaths/coding-agent/session/session-title-slot";
...
describe("session title source persistence", () => {
	...
	beforeEach(() => {
		testAgentDir = fs.mkdtempSync(path.join(os.tmpdir(), "op-title-source-"));
