...
import { recoverOrphanedBackups } from "@openpaths/coding-agent/session/session-listing";
import { SessionManager } from "@openpaths/coding-agent/session/session-manager";
import { FileSessionStorage, MemorySessionStorage } from "@openpaths/coding-agent/session/session-storage";
...
describe("SessionManager rewrite EPERM replacement fallback", () => {
	...
	beforeEach(async () => {
		sessionDir = await fsp.mkdtemp(path.join(os.tmpdir(), "op-eperm-"));
	});
...
describe("FileSessionStorage.writeTextAtomic commitGuard cleanup", () => {
	...
	beforeEach(async () => {
		sessionDir = await fsp.mkdtemp(path.join(os.tmpdir(), "op-guard-cleanup-"));
	});
