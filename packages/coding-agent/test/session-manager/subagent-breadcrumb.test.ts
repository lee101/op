...
import { readTerminalBreadcrumbEntry } from "@openpaths/coding-agent/session/session-paths";
import { getTerminalId } from "@openpaths/tui";
import { getConfigRootDir, getTerminalSessionsDir, setAgentDir } from "@openpaths/utils";
...
describe("SessionManager subagent breadcrumb isolation", () => {
	...
	beforeEach(async () => {
		...
		testAgentDir = await fsp.mkdtemp(path.join(os.tmpdir(), "op-subagent-crumb-"));
