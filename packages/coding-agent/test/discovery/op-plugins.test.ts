 ...
 *
 * The native `op` discovery provider only walks `.op/` and `~/.op/agent/`.
 * Extension packages registered via `extensions:` in settings or
 * `--extension` on the CLI ship their own `skills/`, `hooks/`, `tools/`,
 * `commands/`, `rules/`, `prompts/`, and `.mcp.json`. The `op-plugins`
 * provider (`src/discovery/op-plugins.ts`) is what wires those sub-trees
...
import { clearCache } from "@openpaths/coding-agent/capability/fs";
import { hookCapability } from "@openpaths/coding-agent/capability/hook";
import { mcpCapability } from "@openpaths/coding-agent/capability/mcp";
import { promptCapability } from "@openpaths/coding-agent/capability/prompt";
import { ruleCapability } from "@openpaths/coding-agent/capability/rule";
import { skillCapability } from "@openpaths/coding-agent/capability/skill";
import { slashCommandCapability } from "@openpaths/coding-agent/capability/slash-command";
import { toolCapability } from "@openpaths/coding-agent/capability/tool";
import type { LoadContext, Provider } from "@openpaths/coding-agent/capability/types";
...
} from "@openpaths/coding-agent/discovery/op-extension-roots";
import { discoverExtensionPaths } from "@openpaths/coding-agent/extensibility/extensions/loader";
import { getConfigRootDir, removeSyncWithRetries, setAgentDir } from "@openpaths/utils";

const PROVIDER_ID = "op-plugins";
...
function buildExtensionPackage(packageDir: string, skillName = "my-skill"): void {
	writeFile(
		...
		JSON.stringify({ name: path.basename(packageDir), op: { extensions: ["./src/main.ts"] } }),
	);
...
beforeEach(() => {
	clearCache();
	clearOmpExtensionCliRoots();
	tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "op-plugins-"));
	...
	setAgentDir(path.join(home, ".op", "agent"));
});
...
test("project settings.json#extensions surfaces every sub-directory", async () => {
	writeFile(path.join(project, ".op", "settings.json"), JSON.stringify({ extensions: [ext] }));
...
test("user settings.json#extensions also feeds sub-discovery", async () => {
	writeFile(path.join(home, ".op", "agent", "settings.json"), JSON.stringify({ extensions: [ext] }));
...
test("explicit-only CLI roots replace stale state and exclude every ambient package source", async () => {
	...
	const installed = path.join(home, ".op", "plugins", "node_modules", "installed-extension");
	...
	writeFile(path.join(project, ".op", "settings.json"), JSON.stringify({ extensions: [projectExt] }));
	writeFile(path.join(home, ".op", "agent", "settings.json"), JSON.stringify({ extensions: [userExt] }));
	writeFile(
		path.join(home, ".op", "plugins", "package.json"),
		JSON.stringify({ name: "op-plugins", dependencies: { "installed-extension": "1.0.0" } }),
	);
...
test("file-extension entrypoints contribute zero sub-surface (the file has no siblings to scan)", async () => {
	...
	writeFile(path.join(project, ".op", "settings.json"), JSON.stringify({ extensions: [standaloneFile] }));
...
test("relative paths in settings resolve against the project cwd", async () => {
	...
	writeFile(path.join(project, ".op", "settings.json"), JSON.stringify({ extensions: [`./${relative}`] }));
...
test("installed plugins under `<plugins>/node_modules/` are surfaced (e.g. via `op plugin link`/`install`)", async () => {
	...
	const pluginsDir = path.join(home, ".op", "plugins");
	...
	writeFile(
		...
		JSON.stringify({ name: "op-plugins", dependencies: { "my-installed-ext": "1.0.0" } }),
	);
	// Plugin's own package.json must carry an `op`/`pi` manifest for the
	// loader to recognise it; the buildExtensionPackage fixture already wrote
	// one with `op.extensions`, which is sufficient.
...
test("project-scoped installed plugins surface project-level sub-discovery", async () => {
	const pluginsDir = path.join(project, ".op", "plugins");
	const installed = path.join(pluginsDir, "node_modules", "my-project-ext");
	fs.mkdirSync(installed, { recursive: true });
	fs.cpSync(ext, installed, { recursive: true });
	writeFile(
		path.join(pluginsDir, "op-plugins.lock.json"),
		JSON.stringify({
...
test("disabled installed plugins do not contribute sub-discovery", async () => {
	...
	writeFile(
		path.join(pluginsDir, "package.json"),
		JSON.stringify({ name: "op-plugins", dependencies: { "my-disabled-ext": "1.0.0" } }),
	);
	writeFile(
		path.join(pluginsDir, "op-plugins.lock.json"),
...
test("linked plugins (only in lockfile, not in package.json#dependencies) are surfaced", async () => {
	// `op plugin link ./local-ext` creates a symlink under
	...
	// still find the package — otherwise the documented `op install
