 ...
 * from both `~/.op/agent/RULES.md` (user) and the nearest `.op/RULES.md`
...
import { type Rule, ruleCapability } from "@openpaths/coding-agent/capability/rule";
...
import { loadCapability } from "@openpaths/coding-agent/discovery";
...
test("user ~/.op/agent/RULES.md becomes an alwaysApply rule", async () => {
	writeFile(
		path.join(home, ".op", "agent", "RULES.md"),
...
test("project .op/RULES.md becomes an alwaysApply rule", async () => {
	writeFile(path.join(project, ".op", "RULES.md"), "# Project rule\nAlways say hi.\n");
...
test("project RULES.md is found walking up from a sub-package cwd", async () => {
	...
	writeFile(path.join(project, ".op", "RULES.md"), "# Repo-wide sticky rule\n");
	...
	expect(projectRule).toBeDefined();
	expect(projectRule?.alwaysApply).toBe(true);
	expect(projectRule?.path).toBe(path.join(project, ".op", "RULES.md"));
});

test("user and project sticky RULES.md both survive public capability dedup", async () => {
	const userRulesPath = path.join(home, ".op", "agent", "RULES.md");
	const projectRulesPath = path.join(project, ".op", "RULES.md");
...
test("alwaysApply is forced even when frontmatter says false", async () => {
	writeFile(path.join(home, ".op", "agent", "RULES.md"), "---\nalwaysApply: false\n---\nStick around anyway.\n");
...
test("absent RULES.md does not produce a rule", async () => {
	// No RULES.md anywhere — only a sibling .op/rules/ to make sure the directory exists.
	writeFile(path.join(home, ".op", "agent", "rules", "other.md"), "# Unrelated rule\n");
