/**
 * Regression: OP-native user-level config discovery must follow the active
 * profile. A profile relocates the agent directory to ~/.op/profiles/<name>/agent;
 * the native provider used to read user config (commands, skills, rules, etc.)
 * from the literal home (~/.op/agent) via `ctx.home`, leaking the default
 ...
 * (ctx.home + ".op/agent") points at the tempdir decoys below; without the fix
...
import { type Skill, skillCapability } from "@openpaths/coding-agent/capability/skill";
import { type SlashCommand, slashCommandCapability } from "@openpaths/coding-agent/capability/slash-command";
import { loadCapability } from "@openpaths/coding-agent/discovery";
import { getConfigRootDir, removeWithRetries, setAgentDir } from "@openpaths/utils";
...
describe("native user-level config discovery follows the active profile", () => {
	...
	beforeEach(async () => {
		...
		tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "op-profile-iso-home-"));
		projectDir = await fs.mkdtemp(path.join(os.tmpdir(), "op-profile-iso-project-"));
		profileAgentDir = await fs.mkdtemp(path.join(os.tmpdir(), "op-profile-iso-agent-"));
		...
		const defaultAgentDir = path.join(tempHome, ".op", "agent");
