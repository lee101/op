...
import { getConfigDirs } from "@openpaths/coding-agent/config";
import { resolveClaudePaths } from "@openpaths/coding-agent/config/claude-paths";
import { getUserPath } from "@openpaths/coding-agent/discovery/helpers";
...
describe("PI_CONFIG_DIR", () => {
	...
	test("getUserPath resolves the native user scope via getAgentDir (profile-aware)", () => {
		...
		// The old behavior joined ctx.home + ".op/agent" and leaked the default
	...
	test("getConfigDirs respects PI_CONFIG_DIR for user base", () => {
		process.env.PI_CONFIG_DIR = ".config/op";
		const result = getConfigDirs("commands", { project: false });
		const expected = path.resolve(path.join(os.homedir(), ".config/op", "agent", "commands"));
		expect(result[0]).toEqual({ path: expected, source: ".op", level: "user" });
	});
});
