 ...
 * resolved those relative values against the OP session cwd, so the bundled Codex
 ...
 * `resolvePluginStdioPaths`, matching the claude-plugins/op-plugins fix in #5481.
 */
...
beforeEach(async () => {
	...
	tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "op-codex-mcp-home-"));
	tempCwd = await fs.mkdtemp(path.join(os.tmpdir(), "op-codex-mcp-cwd-"));
