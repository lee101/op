...
import * as secrets from "@openpaths/coding-agent/secrets";
...
import { getSessionsDir, removeSyncWithRetries, Snowflake } from "@openpaths/utils";
import { getActiveProfile, getConfigRootDir, setProfile } from "@openpaths/utils/dirs";
...
async function withTempConfigRoot<T>(run: () => Promise<T>): Promise<T> {
	...
	const configDirName = `.op-sdk-session-${Snowflake.next()}`;
...
describe("createAgentSession session storage isolation", () => {
	...
	it("loads configured secrets per session alongside built-in credential redaction", async () => {
		await withClearedSecretEnv(async () => {
			...
			fs.mkdirSync(path.join(cwd, ".op"), { recursive: true });
			fs.writeFileSync(path.join(cwd, ".op", "secrets.yml"), `- type: plain\n  content: ${configuredSecret}\n`);
