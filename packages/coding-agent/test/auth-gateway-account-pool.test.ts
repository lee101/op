...
import { runAuthGatewayCommand } from "@openpaths/coding-agent/cli/auth-gateway-cli";
import { removeWithRetries } from "@openpaths/utils";

const BROKER_TOKEN = "gateway-account-pool-token";
const ENV_KEYS = ["OP_AUTH_BROKER_URL", "OP_AUTH_BROKER_TOKEN", "OP_AUTH_BROKER_ACCOUNT_POOL_FILE"] as const;
...
describe("auth-gateway account pool", () => {
	...
	beforeEach(async () => {
		...
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "op-auth-gateway-pool-"));
		...
		process.env.OP_AUTH_BROKER_TOKEN = BROKER_TOKEN;
		process.env.OP_AUTH_BROKER_ACCOUNT_POOL_FILE = poolPath;
	});
