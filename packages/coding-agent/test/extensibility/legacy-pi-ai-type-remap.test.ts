...
import {
	...
	getBundledProviders,
	modelsAreEqual,
} from "@openpaths/catalog/models";
import { Type as TypeBoxShimType } from "@openpaths/coding-agent/extensibility/legacy-typebox";
import {
...
// pi-ai 15.1.0 removed the runtime `Type` export from `@openpaths/ai`'s
...
async function writeFixtureExtension(source: string): Promise<string> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "op-pi-ai-type-remap-"));
	tempRoots.push(dir);
...
describe("legacy-pi @(scope)/pi-ai root `Type` remap (issue #1437)", () => {
	...
	it('redirects `import { Type } from "@openpaths/ai"` for plugins published against the canonical scope', async () => {
		const entry = await writeFixtureExtension(
			['import { Type } from "@openpaths/ai";', "export const probe = Type;"].join("\n"),
		);
	...
	it("does not redirect subpath imports such as @openpaths/ai/utils/schema", async () => {
		const entry = await writeFixtureExtension(
			[
	...
	it("re-exports the legacy model catalog and schema helpers from the root", async () => {
		const loaded = (await loadLegacyPiModule(
			await writeFixtureExtension(
				[
					'import { calculateCost, clampThinkingLevel, getBundledModel, getBundledModels, getBundledProviders, getModel, getModels, modelsAreEqual, StringEnum } from "@openpaths/ai";',
...
describe("legacy pi package root remaps (issue #1474)", () => {
	it("loads @earendil-works/pi-coding-agent root imports when host package resolution is unavailable", async () => {
		...
		vi.spyOn(Bun, "resolveSync").mockImplementation((specifier: string, from: string) => {
			if (specifier === "@openpaths/coding-agent" && from.endsWith(path.join("src", "extensibility", "plugins"))) {
	...
	it("preserves legacy defineTool root imports and usable coding tools", async () => {
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "op-legacy-coding-tools-"));
		tempRoots.push(dir);
	...
	it("falls back to legacy-scoped subpath peers for direct plugin imports", async () => {
		const realResolveSync = Bun.resolveSync.bind(Bun);
		vi.spyOn(Bun, "resolveSync").mockImplementation((specifier: string, from: string) => {
			if (specifier === "@openpaths/ai/oauth") {
				throw new Error(`canonical peer unavailable from ${from}`);
			}
			return realResolveSync(specifier, from);
		});

		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "op-legacy-direct-subpath-"));
		tempRoots.push(dir);
	...
	it("routes @earendil-works/pi-utils through canonical Bun.resolveSync in non-compiled mode", async () => {
		// Regression: when op runs from a node_modules install (not the monorepo
		// and not a compiled binary), the bundled packages live at
		// `node_modules/@openpaths/*`, not next to the source tree. Hardcoding
