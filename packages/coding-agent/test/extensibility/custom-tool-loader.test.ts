...
async function writeTool(name: string, source: string): Promise<string> {
	tempRoot ??= await fs.mkdtemp(path.join(os.tmpdir(), "op-custom-tool-loader-"));
...
describe("custom tool loader", () => {
	it("injects callable optype-backed zod schemas through validation and wire emission", async () => {
