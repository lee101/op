...
import { isValidJsonSchema, toolWireSchema } from "@openpaths/ai/utils/schema";
import { validateToolArguments } from "@openpaths/ai/utils/validation";
import { type TSchema, Type } from "@openpaths/coding-agent/extensibility/legacy-typebox";
...
describe("pi.typebox compatibility shim", () => {
	...
	// Callable optype schemas intentionally have no enumerable JSON Schema
	...
	// Issue #8420: legacy extensions build raw documents that embed optype
	...
	describe("lowers embedded optype schemas inside Type.Unsafe documents", () => {
		...
		it("recovers the wire schema when a builder is spread into a new document", () => {
			...
			// `{ ...base, description }` copies optype's internal fields, not JSON
