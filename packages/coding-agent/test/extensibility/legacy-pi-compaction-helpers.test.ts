...
// `./core/compaction/index.ts`); in op it lives in
// `@openpaths/agent-core/compaction` and the coding-agent barrel does not
...
// during plugin validation (e.g. `op plugin install pi-blackhole`). This pins
...
describe("legacy shim compaction helpers", () => {
	...
	// Issue #7174: `compact` (same `@openpaths/agent-core/compaction` module as
	// `estimateTokens`) was likewise absent from the shim surface, so
	// `op plugin install npm:pi-claude-bridge` failed with "Export named
