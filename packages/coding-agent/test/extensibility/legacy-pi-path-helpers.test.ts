...
import * as configModule from "@openpaths/coding-agent/config";
import * as shim from "@openpaths/coding-agent/extensibility/legacy-pi-coding-agent-shim";
import * as utils from "@openpaths/utils";
...
describe("legacy shim path helpers", () => {
	...
	it("getPackageDir resolves the coding-agent package root in source mode", () => {
		// op's canonical helper returns the package root containing package.json
	...
	// `path.join(getPackageDir(), ...)`. op's canonical helper returns
