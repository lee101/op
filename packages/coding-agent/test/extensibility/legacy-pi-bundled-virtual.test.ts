...
// `@openpaths/*` imports through virtual modules backed by live host module
...
describe("legacy-pi bundled virtual module synthesizer (issue #3423)", () => {
	...
	it("emits one ES named export per enumerable namespace key", () => {
		const src = __synthesizeLegacyPiBundledSourceWithModules("@openpaths/coding-agent", modules);
		expect(src).toContain(
			`const __omp_bundled = globalThis[${JSON.stringify(globalKey)}]["@openpaths/coding-agent"];`,
		);
	...
	it("forwards `default` through `export default` so default imports survive", () => {
		const src = __synthesizeLegacyPiBundledSourceWithModules("@openpaths/utils", modules);
	...
	it("throws when asked to synthesize a key the bundled modules do not cover", () => {
		expect(() => __synthesizeLegacyPiBundledSourceWithModules("@openpaths/not-bundled", modules)).toThrow(
			/no bundled module registered for @openpaths\/pi-not-bundled/,
		);
	});
	...
	it("routes Bun plugin resolution through the bundled namespace so onLoad can serve extension imports", async () => {
		using tempDir = TempDir.createSync("@op-legacy-pi-bundled-virtual-");
		const entryPath = tempDir.join("extension-entry.ts");
		const bundlePath = tempDir.join("extension-entry.bundle.mjs");

		await Bun.write(
			entryPath,
			['export { legacyAnswer } from "op-legacy-pi-bundled:@openpaths/utils";', ""].join("\n"),
		);

		expect(resolveBundledVirtualSpecifier("@openpaths/utils")).toEqual({
			namespace: "op-legacy-pi-bundled",
			path: "@openpaths/utils",
		});
		expect(resolveBundledVirtualSpecifier("op-legacy-pi-bundled:@openpaths/utils")).toEqual({
			namespace: "op-legacy-pi-bundled",
			path: "@openpaths/utils",
		});
		...
		const plugin: BunPlugin = {
			name: "op-legacy-pi-bundled-virtual-regression",
			setup(build) {
				build.onResolve({ filter: /^op-legacy-pi-bundled:.+$/, namespace: "file" }, args =>
					resolveBundledVirtualSpecifier(args.path),
				);
				build.onResolve({ filter: /.*/, namespace: "op-legacy-pi-bundled" }, args =>
					resolveBundledVirtualSpecifier(args.path),
				);
				build.onLoad({ filter: /.*/, namespace: "op-legacy-pi-bundled" }, args => {
		...
		expect(onLoadPaths).toEqual(["@openpaths/utils"]);
		...
		expect(bundledModule.legacyAnswer).toBe("served:@openpaths/utils");
	});
});
