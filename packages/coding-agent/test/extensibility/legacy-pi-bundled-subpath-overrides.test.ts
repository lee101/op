...
describe("legacy pi compat compiled-mode subpath overrides (issue #3442)", () => {
	it("does not evaluate unrelated host modules while loading the registry", async () => {
		using tempDir = TempDir.createSync("@op-legacy-pi-loaders-");
...
`,
	...
	it("expands wildcard exports for concrete on-disk targets (issue #3442 follow-up)", () => {
		...
		expect(bundledModuleKeys.has("@openpaths/ai/oauth/anthropic")).toBe(true);
		expect(bundledModuleKeys.has("@openpaths/ai/oauth/openai-codex")).toBe(true);
	});
	...
	it("actually loads the shim's shared Pi translation through the bundled registry", async () => {
		...
		const key = "@openpaths/ai/providers/cursor-pi-args";
...
`,
	...
	it("expands web search provider wildcard exports for compiled plugin imports", () => {
		const overrides = __buildLegacyPiPackageRootOverrides(true, bundledModuleKeys);
		const providerKeys = [
			"@openpaths/coding-agent/web/search/providers/xai",
			"@openpaths/coding-agent/web/search/providers/tinyfish",
			"@openpaths/coding-agent/web/search/providers/firecrawl",
			"@openpaths/coding-agent/web/search/providers/duckduckgo",
		] as const;
	...
	it("does not enumerate root catch-all wildcards (./* / ./*.js)", () => {
		...
		// Concrete check: `@openpaths/coding-agent/cli` is NOT bundled.
		expect(bundledModuleKeys.has("@openpaths/coding-agent/cli")).toBe(false);
		expect(bundledModuleKeys.has("@openpaths/coding-agent/main")).toBe(false);
	});
	...
	it("does not bundle main-thread-unsafe worker entrypoints", () => {
		...
		expect(bundledModuleKeys.has("@openpaths/coding-agent/eval/js/worker-entry")).toBe(false);
	});
	...
	it("does not register subpath overrides in dev/install mode", () => {
		...
		expect(overrides).not.toHaveProperty("@openpaths/ai/oauth");
		expect(overrides).not.toHaveProperty("@openpaths/coding-agent/tools");
