...
import {
	...
	type SessionEvent,
} from "@openpaths/coding-agent/extensibility/hooks";
import { theme } from "@openpaths/coding-agent/modes/theme/theme";
...
describe.skipIf(!e2eApiKey("ANTHROPIC_API_KEY"))("Compaction hooks", () => {
	...
	beforeEach(() => {
		tempDir = path.join(os.tmpdir(), `op-compaction-hooks-test-${Snowflake.next()}`);
