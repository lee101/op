...
import { type AuthBrokerServerHandle, startAuthBroker } from "@openpaths/ai/auth-broker";
import { runAuthBrokerCommand } from "@openpaths/coding-agent/cli/auth-broker-cli";
import { getAgentDbPath, removeWithRetries, setAgentDir } from "@openpaths/utils";
...
describe("auth-broker import (CLIProxyAPI)", () => {
	...
	beforeEach(async () => {
		originalAgentDir = process.env.OP_AGENT_DIR;
		savedEnv.OP_AUTH_BROKER_URL = process.env.OP_AUTH_BROKER_URL;
		savedEnv.OP_AUTH_BROKER_TOKEN = process.env.OP_AUTH_BROKER_TOKEN;
		delete process.env.OP_AUTH_BROKER_URL;
		delete process.env.OP_AUTH_BROKER_TOKEN;
		agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "op-import-agent-"));
		cliproxyDir = await fs.mkdtemp(path.join(os.tmpdir(), "op-import-cliproxy-"));
		setAgentDir(agentDir);
	});

	afterEach(async () => {
		process.stdout.write = ORIGINAL_STDOUT_WRITE;
		if (originalAgentDir === undefined) delete process.env.OP_AGENT_DIR;
		else process.env.OP_AGENT_DIR = originalAgentDir;
		await removeWithRetries(agentDir);
		await removeWithRetries(cliproxyDir);
		for (const key of ["OP_AUTH_BROKER_URL", "OP_AUTH_BROKER_TOKEN"] as const) {
	...
	test("imports a directory of CLIProxyAPI JSONs and maps types to op providers", async () => {
...
describe("auth-broker import (broker-routed)", () => {
	...
	beforeEach(async () => {
		savedEnv.OP_AUTH_BROKER_URL = process.env.OP_AUTH_BROKER_URL;
		savedEnv.OP_AUTH_BROKER_TOKEN = process.env.OP_AUTH_BROKER_TOKEN;
		agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "op-import-client-"));
		brokerAgentDir = await fs.mkdtemp(path.join(os.tmpdir(), "op-import-broker-"));
		cliproxyDir = await fs.mkdtemp(path.join(os.tmpdir(), "op-import-cliproxy-broker-"));
		setAgentDir(agentDir);
		...
		process.env.OP_AUTH_BROKER_URL = handle.url;
		process.env.OP_AUTH_BROKER_TOKEN = token;
	});
