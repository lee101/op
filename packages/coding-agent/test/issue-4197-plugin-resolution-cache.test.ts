...
import { clearClaudePluginRootsCache } from "@openpaths/coding-agent/discovery/helpers";
import {
...
test("getEnabledPlugins caches repeated discovery for the same cwd and home until plugin caches clear", async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "op-plugin-cache-"));
	tempRoots.push(root);
	const home = path.join(root, "home");
	const cwd = path.join(root, "project");
	const pluginsDir = path.join(home, ".op", "plugins");
	const pluginPackageJson = path.join(pluginsDir, "node_modules", "op-cache-repro", "package.json");
	await fs.mkdir(path.dirname(pluginPackageJson), { recursive: true });
	await fs.mkdir(cwd, { recursive: true });
	await writeJson(path.join(pluginsDir, "package.json"), { dependencies: { "op-cache-repro": "1.0.0" } });
	await writeJson(path.join(pluginsDir, "op-plugins.lock.json"), {
		plugins: { "op-cache-repro": { version: "1.0.0", enabled: true, enabledFeatures: null } },
		settings: {},
	});
...
test("legacy bare dependency rewrites cache fallback package resolution until plugin caches clear", async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "op-legacy-cache-"));
	tempRoots.push(root);
