 ...
 *
 * One-shot CLIs (`op bench`, dry-balance) build a bare `ModelRegistry` that
 ...
 * `~/.op/agent/extensions/`) was invisible to model resolution and
 * `op bench <provider>/<model>` failed with "Model not found".
 *
 ...
 * passed explicitly so the test never touches the developer's real `~/.op`.
 */
...
import { getModelMatchPreferences, resolveCliModel } from "@openpaths/coding-agent/config/model-resolver";
import { resetSettingsForTest, Settings } from "@openpaths/coding-agent/config/settings";
import { loadCliExtensionProviders } from "@openpaths/coding-agent/sdk";
