...
import { __providerInFlightForTesting, streamSimple } from "@openpaths/ai/stream";
...
import {
	...
	type SettingPath,
	Settings,
} from "@openpaths/coding-agent/config/settings";
import { AgentStorage } from "@openpaths/coding-agent/session/agent-storage";
import { AUTO_IMAGE_PROVIDER_ORDER } from "@openpaths/coding-agent/tools/image-providers";
import { SEARCH_PROVIDER_ORDER } from "@openpaths/coding-agent/web/search/types";
import { getProjectAgentDir, TempDir } from "@openpaths/utils";
import * as fileLock from "@openpaths/utils/file-lock";
...
describe("Settings", () => {
	...
	describe("config file failure safety", () => {
		...
		it("backs up a corrupted project config and retains the pending project role for retry", async () => {
			await writeSettings({});
			const projectConfigPath = path.join(projectDir, ".op", "config.yml");
			await Bun.write(
				projectConfigPath,
		...
		it("observes both failures when global and project configs are malformed", async () => {
			...
			await Promise.all([
				...
				Bun.write(path.join(projectDir, ".op", "config.yml"), malformed),
			]);
			...
			try {
				...
				expect(
					fs.readdirSync(path.join(projectDir, ".op")).some(name => name.startsWith("config.yml.broken-")),
				).toBe(true);
			} finally {
	...
	describe("model role overrides", () => {
		...
		it("preserves concurrent external per-role edits when saving one global role", async () => {
			...
			// External edit (another op instance / manual edit): changes advisor,
