...
import { substitutePluginRoot } from "@openpaths/coding-agent/discovery/substitute-plugin-root";
...
describe("substitutePluginRoot", () => {
	...
	it("replaces OP_PLUGIN_ROOT in strings", () => {
		expect(substitutePluginRoot(`${OP_VAR}/bin/server`, ROOT)).toBe("/plugins/my-plugin/bin/server");
	});

	it("replaces both variables in same string", () => {
		expect(substitutePluginRoot(`${CLAUDE_VAR}:${OP_VAR}`, ROOT)).toBe("/plugins/my-plugin:/plugins/my-plugin");
	});
