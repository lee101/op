 ...
 *
 * Note: helpers.ts imports @openpaths/natives (Rust addon via glob).
...
import type { InstalledPluginEntry } from "@openpaths/coding-agent/extensibility/plugins/marketplace";
import {
	addInstalledPlugin,
	buildPluginId,
...
describe("resolveActiveProjectRegistryPath", () => {
	...
	beforeEach(() => {
		tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "op-proj-scope-"));
	});
	...
	it("walk-up finds nearest .op/ directory", async () => {
		// Layout: tmpDir/.op/   +   tmpDir/sub/nested/  (cwd)
		// Resolver must climb from cwd → sub → tmpDir and find .op/ there.
		fs.mkdirSync(path.join(tmpDir, ".op"), { recursive: true });
		const cwd = path.join(tmpDir, "sub", "nested");
		fs.mkdirSync(cwd, { recursive: true });

		const result = await resolveActiveProjectRegistryPath(cwd);

		expect(result).toBe(path.join(tmpDir, ".op", "plugins", "installed_plugins.json"));
	});

	it("walk-up stops at the nearest .op/ — does not skip to a more distant one", async () => {
		// Layout: tmpDir/.op/   +   tmpDir/sub/.op/   +   tmpDir/sub/nested/  (cwd)
		// Resolver must stop at tmpDir/sub/.op/, not climb further to tmpDir/.op/.
		fs.mkdirSync(path.join(tmpDir, ".op"), { recursive: true });
		fs.mkdirSync(path.join(tmpDir, "sub", ".op"), { recursive: true });
		const cwd = path.join(tmpDir, "sub", "nested");
		fs.mkdirSync(cwd, { recursive: true });

		const result = await resolveActiveProjectRegistryPath(cwd);

		expect(result).toBe(path.join(tmpDir, "sub", ".op", "plugins", "installed_plugins.json"));
	});

	it("falls back to .git root when no .op/ exists", async () => {
		// Layout: tmpDir/.git/   +   tmpDir/sub/  (cwd)
		// No .op/ anywhere → second pass finds .git/ at tmpDir.
	...
	it("returns null when neither .op/ nor .git/ found anywhere in the tree", async () => {
		// Start at the filesystem root — guaranteed to have no .op/ or .git/ ancestors.
	...
	it("does not treat ~/.git as a project root (pass-2 home-dir guard)", async () => {
		...
		// resolveActiveProjectRegistryPath must NOT return ~/.op/.../installed_plugins.json.
		const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), "op-proj-scope-home-"));
		...
		try {
			const result = await resolveActiveProjectRegistryPath(cwd);
			const homeOmpPath = path.join(homeDir, ".op", "plugins", "installed_plugins.json");
...
describe("listClaudePluginRoots — project shadows user", () => {
	...
	/** Path where listClaudePluginRoots reads the user OP registry. */
	...
	beforeEach(() => {
		tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "op-shadow-home-"));
		tmpProject = fs.mkdtempSync(path.join(os.tmpdir(), "op-shadow-proj-"));

		// Create .op/ in project so resolveActiveProjectRegistryPath finds it.
		fs.mkdirSync(path.join(tmpProject, ".op", "plugins"), { recursive: true });

		userRegPath = path.join(tmpHome, ".op", "plugins", "installed_plugins.json");
		fs.mkdirSync(path.dirname(userRegPath), { recursive: true });

		projectRegPath = path.join(tmpProject, ".op", "plugins", "installed_plugins.json");
	});
