/**
 * Discovery integration tests for OP plugin registry reading.
 *
 * NOTE: listClaudePluginRoots() lives in discovery/helpers.ts which imports
 * @openpaths/natives (native Rust addon via glob). We cannot call it here.
 *
 * Instead these tests validate the structural contract that listClaudePluginRoots
 * depends on:
 *   1. OP registry lives at path.join(home, ".op", "plugins", "installed_plugins.json")
 *      (matches getConfigDirName() == ".op")
...
// Kept here to avoid importing helpers.ts (which pulls in @openpaths/natives).
...
// Matches getConfigDirName() — single source of truth is in @openpaths/utils,
// but we know the value is ".op" and hardcoding it here keeps tests free of
// native-addon transitive imports.
const OP_CONFIG_DIR = ".op";
...
let tmpHome: string;
/** ~/.op/plugins/installed_plugins.json inside tmpHome */
let ompRegistryPath: string;

beforeEach(() => {
	tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "op-discovery-test-"));
	ompRegistryPath = path.join(tmpHome, OP_CONFIG_DIR, "plugins", "installed_plugins.json");
...
describe("OP registry path contract", () => {
	it("OP registry lives at home/.op/plugins/installed_plugins.json", () => {
		// This is the path that listClaudePluginRoots reads.
		// Any change to this path must be reflected in helpers.ts.
		const expected = path.join(tmpHome, ".op", "plugins", "installed_plugins.json");
...
describe("OP registry format compatibility with Claude parser", () => {
...
describe("OP registry round-trip", () => {
...
//
// listClaudePluginRoots must replace Claude entries with OP entries when the same
...
describe("OP precedence contract (registry structure)", () => {
	it("same plugin ID in both registries — OP entry has required fields for deduplication", () => {
		// The replacement logic: roots.filter(r => r.id !== pluginId) keyed by id.
		// OP entries must have installPath so they can be added to roots[].
		const id = buildPluginId("shared-plugin", "common-mkt");
		const ompEntry = makeEntry("/op/cached/path");

		// OP registry entry has installPath (required by listClaudePluginRoots)
