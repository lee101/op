...
import { listOmpExtensionRoots } from "@openpaths/coding-agent/discovery/op-extension-roots";
import { getEnabledPlugins } from "@openpaths/coding-agent/extensibility/plugins/loader";
...
function buildMinimalFixture(): string {
	const root = fs.mkdtempSync(path.join(os.tmpdir(), "op-mgr-fixture-"));
...
function createTestContext(): TestContext {
	const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "op-mgr-test-"));
...
function mockPluginManagerPaths(root: string) {
	return [
		...
		spyOn(piUtils, "getPluginsLockfile").mockReturnValue(path.join(root, "op-plugins.lock.json")),
...
describe("MarketplaceManager", () => {
	...
	it("updateMarketplace expands a home-relative catalog path", async () => {
		...
		try {
			const added = await ctx.manager.addMarketplace(FIXTURE_DIR);
			const catalogPath = "~/.op/plugins/cache/marketplaces/test-marketplace/marketplace.json";
	...
	it("installPlugin → plugin in cache + in registry", async () => {
		...
		const runtimeConfig = await Bun.file(path.join(ctx.tmpDir, "op-plugins.lock.json")).json();
	...
	it("installPlugin exposes marketplace package to the runtime loader", async () => {
		const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "op-mgr-home-"));
		try {
			const pluginsDir = path.join(tmpHome, ".op", "plugins");
			const manager = new MarketplaceManager({
				marketplacesRegistryPath: path.join(tmpHome, ".op", "marketplaces.json"),
	...
	it("installPlugin keeps marketplace packages out of OP extension roots", async () => {
		const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "op-mgr-home-"));
		try {
			const pluginsDir = path.join(tmpHome, ".op", "plugins");
			const manager = new MarketplaceManager({
	...
	it("installPlugin with scope:project exposes the marketplace package to the runtime loader", async () => {
		const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "op-mgr-home-"));
		const projectAnchor = fs.mkdtempSync(path.join(os.tmpdir(), "op-mgr-project-"));
		try {
			const userPluginsDir = path.join(tmpHome, ".op", "plugins");
			const projectPluginsDir = path.join(projectAnchor, ".op", "plugins");
			...
			const projectLock = await Bun.file(path.join(projectPluginsDir, "op-plugins.lock.json")).json();
			...
			expect(fs.existsSync(path.join(userPluginsDir, "op-plugins.lock.json"))).toBe(false);
		} finally {
			fs.rmSync(tmpHome, { recursive: true, force: true });
	...
	it("setPluginEnabled → persisted in registry", async () => {
		...
		let runtimeConfig = await Bun.file(path.join(ctx.tmpDir, "op-plugins.lock.json")).json();
		expect(runtimeConfig.plugins["hello-plugin"].enabled).toBe(false);
		...
		runtimeConfig = await Bun.file(path.join(ctx.tmpDir, "op-plugins.lock.json")).json();
	...
	it("installPlugin scope:project when no projectInstalledRegistryPath → throws", async () => {
		const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "op-mgr-noproj-"));
		try {
