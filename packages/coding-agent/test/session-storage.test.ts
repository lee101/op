...
} from "@openpaths/coding-agent/session/indexed-session-storage";
import { FileSessionStorage } from "@openpaths/coding-agent/session/session-storage";
import { type SessionTitleUpdate, serializeTitleSlot } from "@openpaths/coding-agent/session/session-title-slot";
...
describe("FileSessionStorage writer", () => {
	...
	beforeEach(async () => {
		tempDir = await fsp.mkdtemp(path.join(os.tmpdir(), "op-session-writer-"));
...
describe("FileSessionStorage.deleteSessionWithArtifacts", () => {
	...
	beforeEach(async () => {
		tempDir = await fsp.mkdtemp(path.join(os.tmpdir(), "op-session-storage-"));
		storage = new FileSessionStorage();
	});
