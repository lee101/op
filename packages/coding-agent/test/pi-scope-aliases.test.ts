 ...
 * `@mariozechner` (original), `@earendil-works` (fork), and `@openpaths`
 ...
 *   - `@plannotator/pi-extension`         ⇒ `@openpaths/agent-core`
 *   - `@runfusion/fusion`                 ⇒ `@openpaths/coding-agent/...`
 *
...
const canonicalCodingAgent = Bun.resolveSync("@openpaths/coding-agent", import.meta.dir);
const canonicalCodingAgentExtensions = Bun.resolveSync(
	"@openpaths/coding-agent/extensibility/extensions",
	import.meta.dir,
);
const canonicalUtils = Bun.resolveSync("@openpaths/utils", import.meta.dir);
const canonicalTui = Bun.resolveSync("@openpaths/tui", import.meta.dir);
// Subpath: upstream `pi-ai/oauth` re-exported `utils/oauth/index`; our pi-ai now
// exposes the same surface at the real `@openpaths/ai/oauth` export, so the
// legacy `@mariozechner/pi-ai/oauth` specifier canonicalizes straight to it.
const canonicalAiOauth = Bun.resolveSync("@openpaths/ai/oauth", import.meta.dir);
...
const CASES: readonly AliasCase[] = [
	...
	// @openpaths self-import — canonical scope must still flow through the shim
	// so a duplicate copy is never dragged in from a plugin's own node_modules.
	{ id: "ohmypi-utils", aliasSpecifier: "@openpaths/utils", canonicalPath: canonicalUtils, symbol: "logger" },
	{
		id: "ohmypi-coding-agent",
		aliasSpecifier: "@openpaths/coding-agent",
	...
	// Subpath: legacy `pi-ai/oauth` resolves to the real `@openpaths/ai/oauth`.
	{
