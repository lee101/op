...
import type { AgentEvent } from "@openpaths/agent-core";
import { defineRpcClientTool, RpcClient } from "@openpaths/coding-agent/modes";
import { RpcHostToolBridge } from "@openpaths/coding-agent/modes/rpc/host-tools";
import type {
	RpcHostToolCallRequest,
	RpcHostToolCancelRequest,
	RpcHostToolUpdate,
} from "@openpaths/coding-agent/modes/rpc/rpc-types";
...
describe("RpcClient custom tools", () => {
	it("registers host custom tools and serves tool calls over the RPC transport", async () => {
		const scriptPath = path.join(os.tmpdir(), `op-rpc-host-tools-${Date.now()}.js`);
