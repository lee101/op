...
import { discoverAuthStorage } from "@openpaths/coding-agent/sdk";
import { removeWithRetries } from "@openpaths/utils";

const ENV_KEYS = [
	"OP_AUTH_BROKER_URL",
	"OP_AUTH_BROKER_TOKEN",
	"OP_AUTH_BROKER_SNAPSHOT_CACHE",
	"OP_AUTH_BROKER_SNAPSHOT_TTL_MS",
] as const;
...
describe("discoverAuthStorage auth-broker snapshot cache", () => {
	...
	test("boots from a fresh encrypted cache when the broker is down", async () => {
		...
		process.env.OP_AUTH_BROKER_URL = downUrl;
		process.env.OP_AUTH_BROKER_TOKEN = TOKEN;
		process.env.OP_AUTH_BROKER_SNAPSHOT_CACHE = cachePath;
		process.env.OP_AUTH_BROKER_SNAPSHOT_TTL_MS = "3600000";
	...
	test("boots from a fresh cache when revalidation returns a server error", async () => {
		...
		let storage: AuthStorage | undefined;
		try {
			process.env.OP_AUTH_BROKER_URL = url;
			process.env.OP_AUTH_BROKER_TOKEN = TOKEN;
			process.env.OP_AUTH_BROKER_SNAPSHOT_CACHE = cachePath;
			process.env.OP_AUTH_BROKER_SNAPSHOT_TTL_MS = "3600000";
			await writeAuthBrokerSnapshotCache({
