...
import { Effort, type FetchImpl, type Model } from "@openpaths/ai";
import type { OAuthCredentials } from "@openpaths/ai/oauth/types";
...
import { resolveModelCacheProviderId, resolveOllamaModelCacheProviderId } from "@openpaths/catalog/provider-models";
import type { ModelSpec, OpenAICompat } from "@openpaths/catalog/types";
import {
	applyLlamaCppQwenThinking,
	discoverOllamaModels,
	discoveryProbeTimeoutMs,
} from "@openpaths/coding-agent/config/model-discovery";
import { kNoAuth, ModelRegistry } from "@openpaths/coding-agent/config/model-registry";
import { ProviderDiscoverySchema } from "@openpaths/coding-agent/config/models-config-schema";
import { resetSettingsForTest } from "@openpaths/coding-agent/config/settings";
...
describe("ModelRegistry runtime discovery", () => {
	...
	test("keeps OLLAMA_BASE_URL precedence over OLLAMA_HOST", async () => {
		using _baseUrl = withEnv("OLLAMA_BASE_URL", "http://op-ollama.example:2222");
		using _host = withEnv("OLLAMA_HOST", "ollama-host.example:3333");
		const fetchMock = mockOllamaDiscovery(["phi4-mini"], "http://op-ollama.example:2222");
		const registry = new ModelRegistry(authStorage, modelsJsonPath, { fetch: fetchMock });
		await registry.refresh();

		const model = registry.find("ollama", "phi4-mini");
		expect(model?.baseUrl).toBe("http://op-ollama.example:2222/v1");
	});
