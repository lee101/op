/**
 * Regression for https://github.com/lee101/op/issues/4324
 *
 ...
 * shows up in `~/.op/logs/op.log` without regressing idle-worker shutdown.
 */
import { describe, expect, it } from "bun:test";
import * as path from "node:path";
import { createWorkerSubprocess, type SpawnedSubprocess } from "@openpaths/coding-agent/subprocess/worker-client";
...
describe("issue #4324 — worker subprocess stderr survives to the exit error", () => {
	...
	it("does not keep the parent alive while an unref'd worker stays idle", async () => {
		...
		const wrapperScript = `
			const { createWorkerSubprocess } = await import("@openpaths/coding-agent/subprocess/worker-client");
