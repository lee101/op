 ...
 *
 * Contract under test: the `op models` listing entry point loads extensions
...
import { runModelsListing } from "@openpaths/coding-agent/cli/models-cli";
import { ModelRegistry } from "@openpaths/coding-agent/config/model-registry";
import { getProjectAgentDir, TempDir } from "@openpaths/utils";
...
`,
	...
	await fs.writeFile(
		...
		JSON.stringify({ name: "explicit-package", op: { extensions: ["./src/main.ts"] } }),
	);
	await fs.writeFile(
...
test("op models surfaces extension-registered providers (issue #905)", async () => {
...
test("op models does not execute ambient hooks while retaining explicit providers", async () => {
...
test("op models emits extension shutdown after listing (issue #6297)", async () => {
	const authStorage = await AuthStorage.create(":memory:");
	try {
		const modelRegistry = new ModelRegistry(authStorage);
		await runModelsListing({
...
test("op models explicit-only mode resolves a package and excludes settings providers", async () => {
	const authStorage = await AuthStorage.create(":memory:");
	try {
		const modelRegistry = new ModelRegistry(authStorage);
		const captured: string[] = [];
...
test("op models prints invalid models.yml schema errors before listing output", async () => {
