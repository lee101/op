...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "op-html-template-"));
...
const bundledDependencyStubs: Record<string, string> = {
	"@openpaths/utils": 'export const APP_NAME = "op"; export const isEnoent = () => false;',
	"../../modes/theme/theme":
...
describe("HTML export template", () => {
	...
	test("composes the exact source template once and returns the cached bytes", async () => {
		...
		expect(first).toContain("const THEME_STORAGE_KEY = 'op-export-theme';");
