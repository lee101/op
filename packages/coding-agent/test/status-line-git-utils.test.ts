...
import {
	...
	parseDefaultBranch,
	parseGitHubRepo,
} from "@openpaths/coding-agent/modes/components/status-line/git-utils";

describe("parseGitHubRepo", () => {
	test("parses HTTPS URL", () => {
		expect(parseGitHubRepo("https://github.com/lee101/op.git")).toBe("lee101/op");
	});

	test("parses HTTPS URL without .git suffix", () => {
		expect(parseGitHubRepo("https://github.com/lee101/op")).toBe("lee101/op");
	});

	test("parses SSH scp-style URL", () => {
		expect(parseGitHubRepo("git@github.com:loftiskg/openpaths.git")).toBe("loftiskg/openpaths");
	});

	test("parses SSH scp-style URL without .git suffix", () => {
		expect(parseGitHubRepo("git@github.com:loftiskg/openpaths")).toBe("loftiskg/openpaths");
	});
