...
import { ACP_TERMINAL_AUTH_FLAG, prepareAcpTerminalAuthArgs } from "@openpaths/coding-agent/modes/acp/terminal-auth";
...
import { getConfigRootDir, setAgentDir, VERSION } from "@openpaths/utils";
import type { AgentSideConnection, InitializeRequest } from "@openpaths/utils/acp";
...
async function createAgent(): Promise<AcpAgent> {
	const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "op-acp-init-"));
