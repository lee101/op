...
import {
	...
	sharedSpinnerFrame,
} from "@openpaths/coding-agent/modes/components/tool-execution";
import { getThemeByName } from "@openpaths/coding-agent/modes/theme/theme";
import { getConfigRootDir, getCustomThemesDir, removeWithRetries, setAgentDir } from "@openpaths/utils";
...
describe("theme symbols.spinnerFrames", () => {
	beforeEach(async () => {
		tmpAgentDir = await fs.mkdtemp(path.join(os.tmpdir(), "op-spinner-frames-"));
