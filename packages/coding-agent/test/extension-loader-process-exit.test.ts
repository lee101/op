 ...
 * `process.exit()` at the top level must not terminate the host OP process.
 *
 * The harness intercepts the load via `withHostGuard`; this test pins that the
 * intercepted error surfaces as a per-module load failure (so OP keeps going)
...
import { ExtensionExitError, withHostGuard } from "@openpaths/coding-agent/extensibility/utils";
...
describe("extension/hook loader process.exit guard (#3680)", () => {
	...
	beforeEach(() => {
		project = TempDir.createSync("@op-exit-guard-");
	});
...
import { withHostGuard } from "@openpaths/coding-agent/extensibility/utils";
