...
import { withStatsSyncLock } from "@openpaths/stats/aggregator";
import { type GcResult, runGcCommand } from "@openpaths/coding-agent/cli/gc-cli";
...
beforeEach(async () => {
	...
	root = await fs.mkdtemp(path.join(os.tmpdir(), "op-gc-"));
	writes = [];
	stderrWrites = [];
	process.exitCode = 0;
