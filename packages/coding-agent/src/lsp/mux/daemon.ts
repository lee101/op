 ...
 * first use, stopped when the last op process in the project exits), dials
 * its socket, performs the `op/muxConnect` handshake, and returns an
...
import { logger, ptree } from "@openpaths/utils";
...
export async function smokeTestLspMux(): Promise<void> {
	const endpoint =
		process.platform === "win32"
			? `\\\\.\\pipe\\op-lsp-mux-smoke-${process.pid.toString(16)}`
			: path.join(os.tmpdir(), `op-lsp-mux-smoke-${process.pid.toString(36)}.sock`);
	const spawn = resolveWorkerSpawnCmd(LSP_MUX_WORKER_ARG);
