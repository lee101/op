...
import { $which, isRecord, logger, pathIsWithin, type WhichOptions } from "@openpaths/utils";
...
function getConfigSources(cwd: string): ConfigSource[] {
	...
	// Project config directories (.op/, .pi/, .claude/)
	...
	// User config directories (~/.op/agent/, ~/.pi/agent/, ~/.claude/)
 ...
 * 2. Project config dirs: .op/lsp.*, .pi/lsp.*, .claude/lsp.* (+ hidden variants)
 * 3. User config dirs: ~/.op/agent/lsp.*, ~/.pi/agent/lsp.*, ~/.claude/lsp.* (+ hidden variants)
