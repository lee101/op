...
import { formatHashlineHeader } from "@openpaths/hashline";
...
import { type GrepMatch, GrepOutputMode, type GrepResult, grep } from "@openpaths/natives";
...
async function resolveArchiveSearchPaths(
	...
	for (let idx = 0; idx < pathSpecs.length; idx++) {
		...
		if (!tempDir) {
			tempDir = await mkdtemp(path.join(tmpdir(), "op-search-archive-"));
		}
...
const OP_ROOT_URL_RE = /^op:\/\/(?:\/?|docs\/?)$/i;
...
async function searchVirtualResources(
	...
	const dir = await mkdtemp(path.join(tmpdir(), "op-search-virtual-"));
	try {
...
async function expandVirtualInternalResource(
	...
	if (OP_ROOT_URL_RE.test(rawPath)) {
		const completions = await internalRouter.complete("op", "");
		if (completions && completions.length > 0) {
			...
			for (const completion of completions) {
				...
				const docUrl = `op://${completion.value}`;
