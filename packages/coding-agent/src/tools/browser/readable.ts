import type * as DomNs from "@openpaths/utils/dom";
import type * as ReadabilityNs from "@openpaths/utils/readability";
...
async function loadReadability(): Promise<typeof ReadabilityNs> {
	if (!readabilityModule) {
		readabilityModule = await import("@openpaths/utils/readability");
	}
...
async function loadDom(): Promise<typeof DomNs> {
	if (!domModule) {
		domModule = await import("@openpaths/utils/dom");
	}
	return domModule;
}
