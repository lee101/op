 ...
 *
 * Instead of every op process launching (and sometimes orphaning) a private
 ...
 * it. The broker stops the daemon when the last op client in the project
 * exits, so Chrome can never outlive op, and concurrent acquisitions across
...
/** Broker-owned browser endpoint one op process can attach to. */
...
export function sharedBrowserDaemonName(headless: boolean): string {
	return headless ? "op.browser.headless" : "op.browser.headed";
}
