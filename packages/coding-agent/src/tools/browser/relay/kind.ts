 ...
 * relay served by `op browser-relay` (sibling `server.ts`/`bridge.ts`) plus
 * its companion extension (`packages/browser-relay`, installed via
 * `op browser-relay install`). The relay impersonates Chrome's CDP discovery
 ...
 */
import { parseFlag } from "@openpaths/utils";

/** Browser kind selecting the op browser relay. */
export interface RelayKind {
	kind: "relay";
	cdpUrl: string;
}

/** Default endpoint of the `op-browser-relay` CLI. */
