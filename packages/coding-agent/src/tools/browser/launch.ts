...
import { $which, getPuppeteerDir, logger, removeWithRetries } from "@openpaths/utils";
import type * as BrowsersNs from "@openpaths/utils/browsers";
...
async function loadBrowsers(): Promise<typeof BrowsersNs> {
	if (!browsersModule) {
		browsersModule = await import("@openpaths/utils/browsers");
	}
 ...
 * cached under ~/.op/puppeteer (getPuppeteerDir). Returns undefined when
...
export interface LaunchHeadlessResult {
	browser: Browser;
	/**
	 * OP-owned temporary Chromium profile directory to remove after the browser
	 * process tree exits, or `undefined` when the caller supplied its own
	 * `--user-data-dir` (which OP must not delete).
	 */
	userDataDir?: string;
}
...
export async function launchHeadlessBrowser(opts: LaunchHeadlessOptions): Promise<LaunchHeadlessResult> {
	...
	if (!launchArgs.some(arg => arg.startsWith("--user-data-dir"))) {
		userDataDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), "op-chrome-profile-"));
...
/**
 * Remove an OP-owned headless Chromium profile directory, tolerating the brief
