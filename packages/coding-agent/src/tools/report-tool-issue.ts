 ...
 * bundled endpoint (`dev.autoqaPush.endpoint`, default `qa.openpaths.io`). Each
...
import { $env, $flag, getAutoQaDbPath, getInstallId, logger, VERSION } from "@openpaths/utils";
 ...
 * `~/.op/autoqa.db` (XDG: `$XDG_DATA_HOME/op/autoqa.db`), creating the
...
/**
 * Optional per-flush controls. Used by `op grievances push` to surface
...
function resolvePushConfig(settings: Settings | undefined, bypassConsent: boolean): PushConfig | null {
	...
	// covers explicit user-driven pushes (`op grievances push`) where the
