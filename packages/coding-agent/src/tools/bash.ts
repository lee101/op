...
import { getProjectDir, isEnoent, logger, prompt } from "@openpaths/utils";
 ...
 * deployment shape of an editor spawning op as a co-hosted subprocess.
 */
...
export class BashTool implements AgentTool<typeof bashSchemaBase | typeof bashSchemaWithAsync, BashToolDetails> {
	...
	async execute(
		...
		// must still cancel the call or job, but OP does not impose a deadline.
