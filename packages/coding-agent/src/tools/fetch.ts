...
import type { FetchImpl, ImageContent, TextContent } from "@openpaths/ai";
import { htmlToMarkdown } from "@openpaths/natives";
import { type Component, Text } from "@openpaths/tui";
import { $which, ptree, truncate } from "@openpaths/utils";
...
async function parseFeedToMarkdown(content: string, maxItems = 10): Promise<string> {
	const { parseHTML } = await import("@openpaths/utils/dom");
	try {
...
async function renderNotebookPayload(bytes: Uint8Array, displayUrl: string): Promise<string> {
	return withTempBinaryFile("op-url-notebook-", ".ipynb", bytes, tempPath =>
		readEditableNotebookText(tempPath, displayUrl),
	);
}

async function renderSqlitePayload(bytes: Uint8Array): Promise<string> {
	return withTempBinaryFile("op-url-sqlite-", ".sqlite", bytes, async tempPath => {
