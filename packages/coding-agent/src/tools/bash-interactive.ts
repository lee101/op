...
import { type PtyRunResult, PtySession } from "@openpaths/natives";
import {
	type Component,
...
import type * as XtermModule from "@openpaths/utils/vterm";
import type { Terminal as XtermTerminalType } from "@openpaths/utils/vterm";
...
async function loadXtermTerminal(): Promise<typeof XtermModule.Terminal> {
	if (!xtermTerminalCtor) {
		const mod = (await import("@openpaths/utils/vterm")) as typeof XtermModule & { default?: typeof XtermModule };
