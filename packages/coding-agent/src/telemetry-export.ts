 ...
 *
 * openpaths's agent core (`@openpaths/agent-core`) emits OpenTelemetry GenAI
 ...
 * vars are set so `op` can be observed by any OTLP collector without vendor
 * coupling.
 *
...
import { logger, postmortem } from "@openpaths/utils";
import {
	type Attributes,
	type AttributeValue,
	type Counter,
...
/**
 * Periodic flush interval. A long-lived `op` process (the ACP server is
...
async function registerProviders(signalConfig: SignalConfig): Promise<void> {
	...
	if (signalConfig.metric) {
		...
		metricRecorder = new AgentMetricRecorder(metrics.getMeter("@openpaths/coding-agent"));
	}
	...
	if (signalConfig.log) {
		...
		otelLogger = logProvider.getLogger("@openpaths/coding-agent");
...
class AgentMetricRecorder {
	...
	constructor(meter: Meter) {
		...
		this.#chatCostUsd = meter.createCounter("pi.op.agent.chat.cost.estimated_usd", {
			description: "Estimated USD cost for completed chat calls.",
			unit: "USD",
		});
		this.#runs = meter.createCounter("pi.op.agent.runs", {
			description: "Completed agent runs.",
			unit: "{run}",
		});
		this.#steps = meter.createCounter("pi.op.agent.steps", {
			description: "Agent loop steps completed inside a run.",
			unit: "{step}",
		});
		this.#chatCalls = meter.createCounter("pi.op.agent.chat.calls", {
			description: "Chat calls completed inside agent runs.",
			unit: "{call}",
		});
		this.#chatDurationMs = meter.createHistogram("pi.op.agent.chat.duration", {
			description: "Total chat latency observed in an agent run.",
			unit: "ms",
		});
		this.#toolCalls = meter.createCounter("pi.op.agent.tool.calls", {
			description: "Tool calls completed inside agent runs.",
			unit: "{call}",
		});
		this.#toolDurationMs = meter.createHistogram("pi.op.agent.tool.duration", {
			description: "Total tool latency observed in an agent run.",
			unit: "ms",
		});
		this.#errors = meter.createCounter("pi.op.agent.errors", {
	...
	recordRun(summary: AgentRunSummary, coverage: AgentRunCoverage): void {
		const runAttrs = metricAttributes({
			"pi.op.agent.models_used.count": coverage.modelsUsed.length,
			"pi.op.agent.providers_used.count": coverage.providersUsed.length,
			"pi.op.agent.tools_available.count": coverage.toolsAvailable.length,
			"pi.op.agent.tools_invoked.count": coverage.toolsInvoked.length,
			"pi.op.agent.tools_unused.count": coverage.toolsUnused.length,
		});
		...
		for (const toolName in summary.tools.byName) {
			...
			for (const status of TOOL_STATUSES) {
				...
				if (count > 0) this.#toolCalls.add(count, metricAttributes({ ...toolAttrs, "pi.op.tool.status": status }));
			}
		}
...
function emitRunSummaryLog(summary: AgentRunSummary, coverage: AgentRunCoverage): void {
	emitOtelLog(
		...
		"agent run completed",
		{
			"pi.op.agent.step_count": summary.stepCount,
			"pi.op.agent.chats.total": summary.chats.total,
			"pi.op.agent.chats.total_latency_ms": summary.chats.totalLatencyMs,
			"pi.op.agent.tools.total": summary.tools.total,
			"pi.op.agent.tools.ok": summary.tools.ok,
			"pi.op.agent.tools.error": summary.tools.error,
			"pi.op.agent.tools.skipped": summary.tools.skipped,
			"pi.op.agent.tools.blocked": summary.tools.blocked,
			"pi.op.agent.tools.timeout": summary.tools.timeout,
			"pi.op.agent.tools.aborted": summary.tools.aborted,
			"pi.op.agent.tools.total_latency_ms": summary.tools.totalLatencyMs,
			"pi.op.agent.usage.input_tokens": summary.usage.inputTokens,
			"pi.op.agent.usage.output_tokens": summary.usage.outputTokens,
			"pi.op.agent.usage.cached_input_tokens": summary.usage.cachedInputTokens,
			"pi.op.agent.usage.cache_write_tokens": summary.usage.cacheWriteTokens,
			"pi.op.agent.usage.reasoning_output_tokens": summary.usage.reasoningOutputTokens,
			"pi.op.agent.usage.total_tokens": summary.usage.totalTokens,
			"pi.op.agent.cost.estimated_usd": summary.cost.estimatedUsd,
			"pi.op.agent.cost.unavailable_reasons": summary.cost.unavailableReasons.join(","),
			"pi.op.agent.errors.total": summary.errors.total,
			"pi.op.agent.coverage.tools_available": coverage.toolsAvailable.join(","),
			"pi.op.agent.coverage.tools_invoked": coverage.toolsInvoked.join(","),
			"pi.op.agent.coverage.tools_unused": coverage.toolsUnused.join(","),
			"pi.op.agent.coverage.models_used": coverage.modelsUsed.join(","),
			"pi.op.agent.coverage.providers_used": coverage.providersUsed.join(","),
		},
		"pi.op.agent.run.completed",
	);
}
...
function emitTelemetryWarningLog(warning: AgentTelemetryWarning): void {
	...
	emitOtelLog("warn", warning.message, attrs, "pi.op.telemetry.warning");
}
