...
import { SENSITIVE_TOKEN_RE } from "@openpaths/ai/providers/transform-messages";
import { getSecretPlaceholderKeyPath, isEnoent, logger } from "@openpaths/utils";
 ...
 * — `$XDG_STATE_HOME/op/secret-placeholder.key` (or `~/.op/agent/secret-placeholder.key`
...
export async function loadSecrets(cwd: string, agentDir: string): Promise<SecretEntry[]> {
	const projectPath = path.join(cwd, ".op", "secrets.yml");
