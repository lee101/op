 ...
 * isolated directory (`~/.op/agent/managed-skills`) separate from
 * user-authored skills (`~/.op/agent/skills`). They are discovered and
...
import { getAgentDir, isEnoent } from "@openpaths/utils";
import { YAML } from "bun";

/** Provider id stamped on discovered managed skills (distinguishes them from authored). */
export const MANAGED_SKILLS_PROVIDER_ID = "op-managed";
...
/** Resolve the isolated managed-skills directory (`~/.op/agent/managed-skills`). */
