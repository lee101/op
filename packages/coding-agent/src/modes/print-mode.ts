 ...
 *
 * Used for:
 * - `op -p "prompt"` - text output
 * - `op --mode json "prompt"` - JSON event stream
 */
import type { AgentMessage } from "@openpaths/agent-core";
import type { ImageContent } from "@openpaths/ai";
import { logger, sanitizeText } from "@openpaths/utils";
	...
	if (mode === "text") {
		...
		if (assistantMsg) {
			...
			if (
				...
				// OP-owned Chromium survives this exit (issue #5643). `dispose()`
