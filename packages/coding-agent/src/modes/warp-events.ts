...
import { isInsideTmux, wrapTmuxPassthrough } from "@openpaths/tui/terminal-capabilities";
import { VERSION } from "@openpaths/utils/dirs";
	...
	return {
		emit(event): void {
			...
			const body = {
				...
				// Warp resolves this via CLIAgent.command_prefix(); OpenPaths is "op".
				agent: "op",
	...
	return api => {
		...
		api.on("tool_approval_requested", event => {
			emitter?.emit({
				...
				summary: `op wants to run ${event.toolName}`,
			});
		});
