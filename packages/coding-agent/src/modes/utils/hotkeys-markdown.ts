...
export function buildHotkeysMarkdown(bindings: HotkeysMarkdownBindings): string {
	...
	return [
		...
		"|-----|--------|",
		"| `Enter` | Send message |",
		`| \`${appKey(bindings, "app.message.followUp")}\` | Queue follow-up message (Tab queues when no autocomplete is open) |`,
		...
		"| Key | Action |",
		"|-----|--------|",
		"| `Tab` | Accept autocomplete (queues a follow-up when nothing is open) |",
