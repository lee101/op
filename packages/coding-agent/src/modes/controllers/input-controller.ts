...
import { type AutocompleteProvider, matchesKey, type SlashCommand } from "@openpaths/tui";
import { isEnoent, logger, sanitizeText } from "@openpaths/utils";
...
const OP_STATUS_LINE_RE = /^\s*in:\s+\d+\s+out:\s+\d+(?:\s+cache\s+\S+)?\s+t:\s+\S+\s+tok\/s:\s+\S+/m;
...
export class InputController {
	...
	handleCtrlZ(): void {
		...
		try {
			...
			// longer stops op: tokio swallows it and the TUI ends up torn down
	...
	async openExternalEditor(): Promise<void> {
		...
		try {
			...
			const result = await openInEditor(editorCmd, currentText, { extension: ".op.md", stdio });
			if (result !== null) {
