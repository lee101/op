...
import { CompactionCancelledError, type CompactionOutcome } from "@openpaths/agent-core/compaction";
import {
	getEnvApiKey,
	getProviderDetails,
...
import { Loader, Markdown, padding, Spacer, Text, visibleWidth } from "@openpaths/tui";
import { formatDuration, logger, Snowflake, sanitizeText } from "@openpaths/utils";
	...
	async handleBashCommand(command: string, excludeFromContext = false): Promise<void> {
		...
		try {
			...
			try {
				...
				this.ctx.showError(
					`Bash command completed, but OP failed to update its working directory: ${
...
export function renderUsageReports(
	...
	for (const { provider, providerReports } of providerEntries) {
		...
		// Provider-wide disclaimers (e.g. "OP-observed spend only") render once
