import { THINKING_EFFORTS } from "@openpaths/ai";
import { DEFAULT_SHARE_URL } from "@openpaths/wire";
import { SHAPE_VARIANT_NAMES } from "@openpaths/snapcompact";
...
export const DEFAULT_BASH_INTERCEPTOR_RULES: BashInterceptorRule[] = [
	...
	},
	{
		...
		tool: "hub",
		message:
			'Use the `hub` tool (`op:"start"`) for services, watchers, and debuggers so other op instances can observe and control them.',
	},
	{
		pattern:
...
export const SETTINGS_SCHEMA = {
	...
	// Auth broker — credentials proxied through a remote `op auth-broker serve`
	// host. Hidden from the UI; populate via env vars or hand-edited config.yml.
	// Env (`OP_AUTH_BROKER_URL` / `OP_AUTH_BROKER_TOKEN`) takes precedence so
	...
	"providers.maxInFlightRequests": {
		...
		ui: {
			...
			description:
				'Maximum concurrent LLM requests per provider id (for example "openai" or "anthropic"), shared across local OP processes with this config root. Omitted providers are unlimited.',
		},
	},
		...
		ui: {
			...
			options: [
				...
				},
				{
					value: "project",
					label: "Per-project",
					description: "Save project role models in .op/config.yml; missing project roles use global defaults",
				},
			],
		},
	},
	...
	"startup.checkUpdate": {
		...
		ui: {
			...
			description: "Check for op updates on startup",
		},
	},
	...
	// and mint/enhance isolated managed skills under ~/.op/agent/managed-skills.
	...
	"hindsight.retainContext": { type: "string", default: "op" },
	...
	"lsp.shared": {
		...
		ui: {
			...
			description:
				"Share one language server per project across op instances via the daemon broker (falls back to private servers when unavailable)",
		},
	},
	...
	"github.cache.enabled": {
		...
		ui: {
			...
			description: "Cache rendered issue/PR view output in ~/.op/cache/github-cache.db so repeated reads are free",
		},
	},
	...
	"security.enabled": {
		...
		ui: {
			...
			label: "Security",
			description:
				"Enable OP-native security scan planning, execution, and the read-only security:// resource namespace",
		},
	},
	...
	"browser.relay": {
		...
		ui: {
			...
			label: "Browser Relay",
			description:
				"Drive your own Chrome tabs through the op browser relay. Install the extension once (`op browser-relay install`); the relay server auto-starts when the browser tool needs it. Takes precedence over Browser CDP URL; set PI_BROWSER_RELAY=0 or PI_BROWSER_RELAY=1 to override.",
		},
	},
	...
	"browser.relayUrl": {
		...
		ui: {
			...
			description: "op browser relay endpoint (default http://127.0.0.1:9224).",
		},
	},
	...
	"worktree.base": {
		...
		ui: {
			...
			description:
				"Base directory for agent-managed worktrees — task-isolation copies, `github` PR checkouts, and `op worktree` cleanup all live here. Unset uses ~/.op/wt. Must be an absolute or ~-relative path; relative paths are ignored. The OP_WORKTREE_DIR env var overrides this.",
		},
	},
	...
	"extensionHandlers.toolCallTimeoutMs": {
		...
		ui: {
			...
			description:
				"Positive finite active-work timeout for extension tool_call handlers; invalid values use 30000ms, and time awaiting OP-owned dialogs does not count",
		},
	},
	...
	"dev.autoqaPush.endpoint": {
		type: "string",
		default: "https://qa.openpaths.io/v1/grievances" as const,
		ui: {
			tab: "tools",
			group: "Developer",
			label: "Auto QA Push Endpoint",
			description: "Full URL receiving Auto QA JSON reports (default https://qa.openpaths.io/v1/grievances)",
		},
	},
