...
import { getWorktreeDir, logger, Snowflake } from "@openpaths/utils";
...
async function writeSyntheticTree(
	...
	const tempIndex = path.join(os.tmpdir(), `op-task-index-${Snowflake.next()}`);
	try {
	...
	for (const [relativePath, repoPatches] of byRepo) {
		...
		const stashed =
			(await git.status(nestedDir)).trim().length > 0
				? await git.stash.push(nestedDir, `op-isolation-${Snowflake.next()}`)
				: false;
		try {
...
export async function ensureIsolation(
	...
	for (const candidate of candidates) {
		...
		// `op worktree clear` never sees this sandbox without a live owner,
...
async function replayFilteredAgentCommits(opts: FilteredAgentReplayOptions): Promise<void> {
	...
	const tmpDir = path.join(os.tmpdir(), `op-branch-${Snowflake.next()}`);
	try {
 ...
 * branch named `op/task/${taskId}`. Only root-repo changes go on the branch;
...
export async function commitToBranch(
	...
	const branchName = `op/task/${taskId}`;
...
export async function mergeTaskBranches(
	...
	return git.withRepoLock(repoRoot, async () => {
		...
		const didStash = await git.stash.push(repoRoot, "op-task-merge");
