 ...
 *
 * Discovers agent definitions from OP-native task-agent roots:
 *   - ~/.op/agent/agents/*.md (user-level)
 *   - .op/agents/*.md (project-level)
 *   - <ext>/agents/*.md for every OP extension package wired through
 ...
 *     `hooks/`, `tools/`, etc. by `discovery/op-plugins.ts`.
 *
 ...
 * are intentionally skipped because their frontmatter schema is not the OP
 * task-agent contract.
 *
 ...
 * Precedence (highest wins): project `.op/agents`, user `.op/agents`,
 * OP extension-package agents in `listOmpExtensionRoots` source order
...
export async function discoverAgents(cwd: string, home: string = os.homedir()): Promise<DiscoveryResult> {
	...
	// OP extension-package agents/ dirs. `listOmpExtensionRoots` returns roots in
	...
	// surface in `discovery/op-plugins.ts`. Gate on `op-plugins` so
	// disabledProviders suppresses the whole extension-package surface.
	const extensionRoots = isProviderEnabled("op-plugins")
