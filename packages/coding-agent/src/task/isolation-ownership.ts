/**
 * Ownership marker for task-isolation sandboxes under `~/.op/wt/`.
 *
 * Each isolation base dir (`ensureIsolation` in {@link ./worktree}) holds a
 * compact `m` mount plus this marker file naming the op process that created
 * it. `op worktree clear` consults the marker so it can distinguish a live
...
export const ISOLATION_OWNER_FILE = ".op-isolation-owner.json";

/** Recorded owner of a task-isolation sandbox. */
export interface IsolationOwner {
	/** PID of the op process that created and owns the sandbox. */
	pid: number;
 ...
 * `op worktree clear` never sees an owner-less sandbox mid-creation.
 */
...
/**
 * Whether a live op process still owns the sandbox at `baseDir`.
 *
