/**
 * Harness documentation index for the `op://` protocol.
 *
 ...
 * written by `gen:bundle`) — so `@openpaths/coding-agent/*` SDK consumers
 ...
 * SDK consumers importing `@openpaths/coding-agent/*` load TypeScript source, where
...
/** Empty index for when no docs corpus is reachable — degrades `op://` instead of throwing ENOENT at callers. */
function emptyIndex(): DocsIndex {
	logger.warn(
		"op:// docs corpus unavailable: no build-time embed, on-disk docs/ directory, or shipped dist embed found",
	);
