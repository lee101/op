 ...
 * `@openpaths/natives`, which infers the language from the file path and
 * returns the 1-indexed line span of the syntactic block beginning on the
 * requested line (or `null` when none can be resolved).
 */
import type { BlockResolver } from "@openpaths/hashline";
import { blockRangeAt } from "@openpaths/natives";
