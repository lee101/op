# OP Julia runner — subprocess wrapper used by the coding-agent host.
...
struct OpDisplay <: AbstractDisplay end

function Base.display(d::OpDisplay, value)
    rid = current_rid
    if rid !== nothing
