...
export const OP_SECURITY_WORKFLOW_VERSION = "1.0.0";

export function createNativeSecurityProducer(): SecurityProducer {
	return {
		kind: "op-native",
		name: "OP Native Security",
		version: OP_SECURITY_WORKFLOW_VERSION,
	};
}

export function createSecurityCredentialAffinity(account: SecurityAccountRef): string {
	return `op-security-credential/v1:sha256:${Bun.SHA256.hash(canonicalSecurityJson(account), "hex")}`;
}
...
export function createNativeSecurityProvenance(options: {
	...
	if (options.sessionId !== undefined) {
		metadata.sessionAffinity = `op-security-session/v1:sha256:${Bun.SHA256.hash(options.sessionId, "hex")}`;
	}
...
export function createSecurityWorkflowFingerprint(inputs: readonly string[]): string {
	return `op-security-workflow/v1:sha256:${Bun.SHA256.hash(
		canonicalSecurityJson({
			workflowVersion: OP_SECURITY_WORKFLOW_VERSION,
			upstream: CODEX_SECURITY_UPSTREAM,
			inputs,
		}),
		"hex",
