...
import { CONFIG_DIR_NAME, getConfigAgentDirName, getProjectDir } from "@openpaths/utils";
 ...
 * project with op's own assets and was the source of issue #1423 (the host
 * project's `CHANGELOG.md` rendered as op's startup changelog).
 */
...
/**
 * Path to op's own `CHANGELOG.md`, or `undefined` when the package directory
...
/**
 * User-level: ~/.op/agent, Claude's active config directory, ~/.codex, ~/.gemini
 * Project-level: .op, .claude, .codex, .gemini
 */
...
export interface ConfigDirEntry {
	path: string;
	source: string; // e.g., ".op", ".claude"
	level: "user" | "project";
}

export interface GetConfigDirsOptions {
	/** Include user-level directories (~/.op/agent/...). Default: true */
	user?: boolean;
	/** Include project-level directories (.op/...). Default: true */
	project?: boolean;
 ...
 * // → [{ path: "~/.op/agent/commands", source: ".op", level: "user" }, ...]
 *
 * @example
 ...
 * Returns one entry per config base (.op, .claude) - the nearest one found.
