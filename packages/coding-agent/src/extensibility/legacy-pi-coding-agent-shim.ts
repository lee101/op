 ...
 * `@openpaths/coding-agent` (or one of its aliased scopes like
 ...
 * the same module identity as a direct `@openpaths/coding-agent` import.
 */

import { Database } from "bun:sqlite";
import * as fs from "node:fs";
import * as path from "node:path";
import type { AgentToolResult, AgentToolUpdateCallback } from "@openpaths/agent-core";
import { type AuthCredential, SqliteAuthCredentialStore, type TSchema } from "@openpaths/ai";
import { piEscapeRegexLiteral, piJoinPath } from "@openpaths/ai/providers/cursor-pi-args";
import { getKeybindings, type Keybinding, Text } from "@openpaths/tui";
import {
	getAgentDbPath,
	getAgentDir,
	getProjectDir,
...
export function createEditToolDefinition(cwd: string, options?: EditToolOptions): ToolDefinition {
	if (options?.operations) {
		throw new Error(
			"Legacy EditToolOptions.operations is not supported: OP's built-in edit tool writes the local " +
...
export function createWriteToolDefinition(cwd: string, options?: WriteToolOptions): ToolDefinition {
	if (options?.operations) {
		throw new Error(
			"Legacy WriteToolOptions.operations is not supported: OP's built-in write tool writes the local " +
...
/**
 * Enumerates the extensions OP would load through the historical package
...
export class DefaultPackageManager {
	...
	/** Resolve enabled extension paths with their OP plugin provenance. */
 ...
 *
 * OP does the same discovery inline inside `createAgentSession()`, so this
 ...
 * override below translates them into OP's native session options
 ...
 * extensions in the wild — themes are silently dropped (OP has no
...
/**
 * Legacy pi extensions call `createAgentSession({ resourceLoader })`. OP's
 ...
 * `noExtensions`/`noSkills` opt-outs), re-run OP's own discovery, and
 ...
 *
 * Translate the loader's captured state into OP's option fields, then
 ...
 *
 * Modern OP auth storage is asynchronous, while older provider extensions
...
// `getAgentDir`; `getProjectDir` (a `@openpaths/utils` helper) and
...
export { getProjectDir } from "@openpaths/utils";
 ...
 *
 * op's canonical `getPackageDir()` (`../config`) returns `undefined` inside a
...
// `./core/compaction/index.ts`). In op they live in
// `@openpaths/agent-core/compaction`, and the coding-agent barrel below does
...
export { compact, estimateTokens, serializeConversation } from "@openpaths/agent-core/compaction";

// Same barrel gap for two more legacy package-root exports: pi re-exported the
// `CONFIG_DIR_NAME` constant and the CLI parser `parseArgs`. In op
// `CONFIG_DIR_NAME` lives in `@openpaths/utils` and `parseArgs` in
...
export { CONFIG_DIR_NAME } from "@openpaths/utils";
...
// tool name. op removed them from the public API in 10.2.3, and the barrel above
...
// `find`/`ls` tool results arrive through op's custom-event branch, so those
...
/** Legacy `find` result event represented by op's custom-event branch. */
...
/** Legacy `ls` result event represented by op's custom-event branch. */
