...
import { isCompiledBinary, stripWindowsExtendedLengthPathPrefix } from "@openpaths/utils";
...
// surfaces through `op-legacy-pi-bundled:<key>` synthetic modules.
...
const BUNDLED_VIRTUAL_SCHEME = "op-legacy-pi-bundled:";
const BUNDLED_VIRTUAL_NAMESPACE = "op-legacy-pi-bundled";
...
function ensureBundledModuleLoadersLoaded(): Promise<BundledModuleLoaders> {
	if (!IS_COMPILED_BINARY) {
		return Promise.reject(new Error("op:legacy-pi-shim: bundled modules are only available in compiled mode"));
	}
	if (!bundledModuleLoadersPromise) {
		bundledModuleLoadersPromise = import("op-legacy-pi-modules").then(module => {
...
async function loadBundledModule(moduleKey: string): Promise<void> {
	...
	if (!loader) {
		throw new Error(`op:legacy-pi-shim: no bundled module registered for ${moduleKey}`);
	}
...
export function resolveBundledVirtualSpecifier(specifier: string): BundledVirtualResolveResult {
	const registryKey = isBundledVirtualSpecifier(specifier)
		...
		: specifier;
	if (!registryKey) {
		throw new Error("op:legacy-pi-shim: bundled virtual specifier has no registry key");
	}
	return { path: registryKey, namespace: BUNDLED_VIRTUAL_NAMESPACE };
}
 ...
 * `op-legacy-pi-bundled:<key>` import.
 */
...
// fork, or the canonical @openpaths scope itself) are remapped to this scope and
// resolved against the bundled copy that ships inside the op binary. This
...
const CANONICAL_PI_SCOPE = "@openpaths";

// Scopes that have historically been used to publish (or alias) the same set
// of internal pi-* packages. `@openpaths` is intentionally included so direct
...
const PI_SCOPE_ALIASES = ["openpaths", "mariozechner", "earendil-works"] as const;

// Internal pi-* package basenames bundled inside the op binary.
...
// Extensions importing TypeBox directly are redirected to optype's TypeBox
// facade, keeping legacy builders while producing callable optype schemas at
...
// is served through the `op-legacy-pi-bundled:` virtual namespace (see the
 ...
 * bare `@openpaths/coding-agent` here: from a global install Bun can pick an
...
/**
 * Resolve the coding-agent compatibility surface that composes optype's
 ...
 * `op-legacy-pi-bundled:` virtual namespace (issue #3423). Dev, source-link,
...
// plus the borrowed `Type` runtime from the optype TypeBox facade. Subpath
// imports such as `@openpaths/ai/oauth` continue to resolve directly
...
// `node_modules/@openpaths/*`.
//
// Compiled-binary entries are `op-legacy-pi-bundled:<key>` specifiers handed
 ...
 * through to the canonical-resolution path. Virtual `op-legacy-pi-bundled:`
...
/**
 * Resolve a canonical `@openpaths/*` specifier to a filesystem path, preferring
...
function toImportSpecifier(resolvedPath: string): string {
	// Virtual `op-legacy-pi-bundled:` specifiers are served by the synthetic
...
/**
 * Rewrite the extension-owned specifiers OP must host-resolve — legacy
 ...
 * realpath of every reachable source module OP must rewrite at load time.
...
/**
 * Linkedom's canvas bridge uses its bundled fallback because OP does not ship
 * native canvas.
 */
...
function resolveLegacyPiSpecifier(args: { path: string; importer: string }): LegacyPiResolveResult | undefined {
	...
	// Primary: resolve the canonical @openpaths/* specifier from the host binary
	...
	try {
		...
		// plugin installed @openpaths peer deps, then try the original legacy
...
export function installLegacyPiSpecifierShim(): void {
	...
	Bun.plugin({
		...
		setup(build) {
			...
			build.onResolve({ filter: /^op-legacy-pi-bundled:.+$/, namespace: "file" }, args =>
				resolveBundledVirtualSpecifier(args.path),
			);
			build.onResolve({ filter: /.*/, namespace: BUNDLED_VIRTUAL_NAMESPACE }, args =>
				resolveBundledVirtualSpecifier(args.path),
			);
			// Compiled mode serves `op-legacy-pi-bundled:<key>` imports from
