...
import { getPluginsDir, getPluginsLockfile, isEnoent } from "@openpaths/utils";
 ...
 *
 * `home` controls which `<plugins>/op-plugins.lock.json` is read — pass it
...
/**
 * Load project-local plugin overrides (checks .op and .pi directories).
 */
 ...
 * `<root>/package.json#dependencies`, and `<root>/op-plugins.lock.json#plugins`.
...
async function collectPluginsAtRoot(
	...
	const lockPath = path.join(root, "op-plugins.lock.json");
	...
	for (const name of names) {
		...
		let pluginPkg: { version: string; op?: PluginManifest; pi?: PluginManifest };
		try {
		...
		const manifest: PluginManifest | undefined = pluginPkg.op || pluginPkg.pi;
		if (!manifest) {
			// Not an op plugin, skip
			continue;
		}
 ...
 * (`getPluginsDir(home)`) and, when a project anchor (`.op/` or `.git/`)
 * exists at or above `cwd`, the project root
 * (`<projectAnchor>/.op/plugins`). Each root contributes the union of its
 * `package.json#dependencies` and `op-plugins.lock.json#plugins`. Project
...
interface DeclaredManifestEntries {
	/** True when the directory's package.json declares a non-empty `op`/`pi` `extensions` array. */
	declared: boolean;
...
/**
 * Read the extension entries declared by `dir`'s own package.json `op`/`pi`
...
function readDeclaredManifestEntries(dir: string): DeclaredManifestEntries {
	...
	let pkg: { op?: { extensions?: unknown }; pi?: { extensions?: unknown } };
	try {
		pkg = JSON.parse(raw) as { op?: { extensions?: unknown }; pi?: { extensions?: unknown } };
	} catch {
		return { declared: false, files: [] };
	}
	const declared = (pkg.op ?? pkg.pi)?.extensions;
 ...
 *   1. the directory's own package.json `op`/`pi` `extensions` entries —
 ...
 *     {@link resolveDirectoryEntries} — its own package.json `op`/`pi`
 ...
 *     and OP's configured-directory (`-e`) extension loader
 *   - otherwise (tools/hooks/commands) only a direct index.{ts,js,mjs,cjs}.
 *     The sub-extension scan and the `op`/`pi` `extensions` manifest are
