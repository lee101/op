...
import { IR_BRAND } from "@openpaths/optype/ir";
import {
	type AnySchema,
	type ObjectOpts,
	Type as OpType,
	type TypeBuilder as OpTypeBuilder,
	type TUnsafe,
} from "@openpaths/optype/typebox";
import { upgradeJsonSchemaTo202012, validateJsonSchemaValue } from "@openpaths/ai/utils/schema";
 ...
 * JSON Schema, lowering any embedded optype schema to its wire JSON. Legacy
 * Pi extensions were written against real TypeBox, whose `Type.*` builders
 * return plain JSON-Schema objects; optype's builders return callable schema
 ...
 *    `DataCloneError: The object can not be cloned.` (issue #8420) and optype
 *    would drop its `toJsonSchema()` override during composition anyway.
 *  - Spreading — `Type.Unsafe({ ...Schema, description })`. Spreading a
 *    callable copies optype's internal fields (`ir`, `run`, `$`, …) instead
 ...
 *    caller's own additions (everything not an optype internal) are overlaid.
 */
...
function unsafe<T = unknown>(jsonSchema: Record<string, unknown> = {}): LegacyUnsafeSchema<T> {
	...
	// optype builder to its wire form), so it doubles as the detaching clone.
...
const object = ((properties: Record<string, unknown>, opts?: ObjectOpts) => {
	...
	if (!hasRawProperty) {
		return OpType.Object(properties as Record<string, AnySchema>, normalizedOpts);
	}
	...
	if (additionalProperties !== undefined && typeof additionalProperties !== "boolean") {
		// optype index signatures validate every string key, including declared
		...
		const document = OpType.Object(normalizedProperties, objectOpts).toJsonSchema();
	...
	return OpType.Object(normalizedProperties, normalizedOpts);
}) as typeof OpType.Object;

export const Type = { ...OpType, Object: object, Unsafe: unsafe } as unknown as OpTypeBuilder;
export type TypeBuilder = OpTypeBuilder;

const legacyTypeBox: { Type: OpTypeBuilder } = { Type };
