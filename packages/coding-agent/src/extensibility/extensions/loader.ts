...
import type { KeyId } from "@openpaths/tui";
import { hasFsCode, isEacces, isEnoent, logger } from "@openpaths/utils";
...
async function readExtensionManifest(packageJsonPath: string): Promise<ExtensionManifest | null> {
	try {
		const pkg = (await Bun.file(packageJsonPath).json()) as { op?: ExtensionManifest; pi?: ExtensionManifest };
 ...
 * `.op`/`.pi` extension capabilities, JS/TS hook factories, the
...
export async function discoverExtensionPaths(
	...
	if (ambient) {
		// 1. Discover extension modules via capability API (native .op/.pi only).
