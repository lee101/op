 ...
 * `@openpaths/ai` (or one of its aliased scopes like `@earendil-works/pi-ai`
 ...
 * bare package root. Subpath imports (`@openpaths/ai/oauth`, etc.)
...
/** Clamp a historical Pi thinking level against OP's model metadata. */
 ...
 * were authored against rather than OP's own `Error`-based classifiers.
 */
 ...
 * `@openpaths/catalog/models`. `getModel`/`getModels` are the historical
 ...
 * should wrap with `retryTransientCompletion` from `@openpaths/ai`.
 */
 ...
 * `@earendil-works/pi-ai` exposed from its package root but op's
 * `@openpaths/ai` barrel no longer forwards. Each symbol still exists in the
 ...
 * static named-export check (e.g. `op plugin install pi-blackhole`).
 *
 ...
 * `@openpaths/ai/error`) and the JSON-repair helpers that op relocated to
 * `@openpaths/utils`. Upstream root symbols with no op equivalent are
 * intentionally not shimmed — the package has diverged and there is nothing to
 * forward.
 */
export { isContextOverflow } from "@openpaths/ai/error";
export { parseJsonWithRepair, parseStreamingJson, repairJson } from "@openpaths/utils";
