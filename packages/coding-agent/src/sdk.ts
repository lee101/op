...
import { resolveApiKeyOnce } from "@openpaths/ai/auth-retry";
import type { Dialect } from "@openpaths/ai/dialect";
import {
	getOpenAICodexTransportDetails,
	prewarmOpenAICodexResponses,
} from "@openpaths/ai/providers/openai-codex-responses";
import { FALLBACK_DIALECT, preferredDialect } from "@openpaths/catalog/identity";
import type { Component } from "@openpaths/tui";
import { $env, $flag, getAgentDir, getProjectDir, logger, postmortem, prompt, Snowflake } from "@openpaths/utils";
...
import { withOmpExtensionRootScope } from "./discovery/op-extension-roots";
...
export interface CreateAgentSessionOptions {
	...
	/** Global config directory. Default: ~/.op/agent */
	agentDir?: string;
	...
	/**
	 * Pre-discovered custom-tool source paths from `.op/tools/`, `.claude/tools/`,
	...
	/** Prompt templates. Default: discovered from cwd/.op/prompts/ + agentDir/prompts/ */
 ...
 *
 * Broker mode: when `OP_AUTH_BROKER_URL` is set, credentials are pulled from
 ...
 * (`op bench`, dry-balance) build a bare {@link ModelRegistry} that only knows
 ...
 * `~/.op/agent/extensions/`) never reach model resolution. Mirrors the
 * session / `op models` path: drain the queued provider registrations, then
...
async function createAgentSessionScoped(options: CreateAgentSessionOptions): Promise<CreateAgentSessionResult> {
	...
	try {
		...
		if (!restrictToolNames) {
			...
			// Discover custom tools from `.op/tools/`, `.claude/tools/`, plugins, etc.
		...
		// cache that `op models find` uses before the online refresh continues in
		// the background.
		...
		if (!model && deferredModelPatterns.length === 0) {
			...
			if (!model) {
				...
				// `op models` (which awaits discovery) lists the model. Await one
		...
		// across op instances by the LSP mux daemon. Session-level because the
