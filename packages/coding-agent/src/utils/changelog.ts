...
import { getLastChangelogVersionPath, isEnoent, logger } from "@openpaths/utils";
...
export function formatStartupChangelogSummary(selection: StartupChangelogSelection): string {
	...
	if (!latestVersion || selection.selectedEntries === 0) {
		return "Updated op. Use /changelog for recent changes.";
	}
...
/**
 * Parse changelog entries from op's package asset when available, falling back
...
/**
 * Parse an op changelog marker version into comparable parts.
 */
...
/**
 * Last op version whose changelog the user has seen. Stored as a plain-text
 * marker file (`~/.op/agent/last-changelog-version`) rather than in
