...
import { formatBytes, isRecord, logger, readImageMetadata, SUPPORTED_IMAGE_MIME_TYPES } from "@openpaths/utils";
 ...
 * to PNG/JPEG instead — the automatic equivalent of `OP_NO_WEBP=1`.
 */
 ...
 * `OP_NO_WEBP` env fallback in {@link resizeImage} still applies. Feed straight
...
export interface LoadImageInputOptions {
	...
	/** Force non-WebP output (e.g. for Ollama). Leave unset to honor `OP_NO_WEBP`. */
	excludeWebP?: boolean;
}
