...
import { type ApiKeyResolver, completeSimple, retryTransientCompletion } from "@openpaths/ai";
import { hostMatchesUrl } from "@openpaths/catalog/hosts";
import type { Mnemopi } from "@openpaths/mnemopi";
import type * as MnemopiDiagnoseNs from "@openpaths/mnemopi/diagnose";
import type { DiagnosticSummary } from "@openpaths/mnemopi/diagnose";
...
async function loadMnemopiDiagnose(): Promise<typeof MnemopiDiagnoseNs> {
	if (!mnemopiDiagnoseMod) {
		mnemopiDiagnoseMod = await import("@openpaths/mnemopi/diagnose");
	}
