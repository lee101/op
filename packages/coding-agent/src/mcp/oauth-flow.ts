...
import type { OAuthCallbackFlowOptions } from "@openpaths/ai/oauth/callback-server";
import { OAuthCallbackFlow } from "@openpaths/ai/oauth/callback-server";
import type { OAuthController, OAuthCredentials } from "@openpaths/ai/oauth/types";
import type { FetchImpl } from "@openpaths/ai/types";
import { getActiveProfile } from "@openpaths/utils/dirs";
import type { OAuthCredential } from "../session/auth-storage";

/** Credential-id prefix for OP-managed MCP OAuth credentials keyed by profile and server URL. */
...
/**
 * Deterministic credential id for an MCP server URL scoped to an OP profile.
 *
...
/** Whether a credential id was minted by OP's MCP OAuth flows (either era). */
 ...
 *
 * Plane is stricter for OP-synthesized fallback resources (e.g. using the
...
export class MCPOAuthFlow extends OAuthCallbackFlow {
	...
	async generateAuthUrl(state: string, redirectUri: string): Promise<{ url: string; instructions?: string }> {
		...
		if (existingResource) {
			...
			// provider-authored, not OP's server-URL fallback. Preserve same-host
