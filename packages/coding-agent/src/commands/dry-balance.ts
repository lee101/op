...
export default class DryBalance extends Command {
	...
	static flags = {
		model: Flags.string({ description: "Model selector (same syntax as --model on op)" }),
	...
	static examples = [
		...
		"# Dry-run a specific model\n  op dry-balance anthropic/claude-sonnet-4-5",
		"# Larger run with bounded concurrency\n  op dry-balance --model openai-codex/gpt-5-codex --count 1000 --concurrency 64",
		"# Benchmark every OAuth account in parallel\n  op dry-balance --bench",
