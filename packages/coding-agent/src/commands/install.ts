/**
 * `op install <target>` — top-level convenience over `op plugin install` /
 * `op plugin link`.
 *
 * The docs (openpaths.io/docs/extension-authoring) advertise
 *
 *   op install ./my-extension
 *
...
export default class Install extends Command {
	...
	async run(): Promise<void> {
		...
		if (targets.length === 0) {
			process.stderr.write("Usage: op install <path | npm-spec | name@marketplace> [...]\n");
			process.exit(1);
		}
