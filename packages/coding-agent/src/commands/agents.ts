...
export default class Agents extends Command {
	...
	static flags = {
		...
		user: Flags.boolean({ description: "Write to ~/.op/agent/agents (default)" }),
		project: Flags.boolean({ description: "Write to ./.op/agents" }),
	};

	static examples = [
		"# Export bundled agents into user config (default)\n  op agents unpack",
		"# Export bundled agents into project config\n  op agents unpack --project",
		"# Overwrite existing local agent files\n  op agents unpack --project --force",
		"# Export into a custom directory\n  op agents unpack --dir ./tmp/agents --json",
	];

	async run(): Promise<void> {
		const { args, flags } = await this.parse(Agents);
		if (!args.action) {
			renderCommandHelp("op", "agents", Agents);
			return;
		}
