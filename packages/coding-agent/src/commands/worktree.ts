/**
 * List and clean up agent-managed git worktrees under `~/.op/wt`.
 */
...
export default class Worktree extends Command {
	...
	static args = {
		...
		// A positional action keeps `op worktree` (the no-arg form) useful.
		action: Args.string({
	...
	static examples = [
		"op worktree",
		"op worktree list --json",
		"op worktree clear",
		"op worktree clear --dry-run",
		"op worktree clear --all",
	];

	async run(): Promise<void> {
		const { args, flags } = await this.parse(Worktree);
		// Load settings so the `worktree.base` override is applied before we scan
		// — otherwise this command would inspect ~/.op/wt while the agent created
