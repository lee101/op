/**
 * `op auth-broker` — manage the op credential vault.
 */
...
export default class AuthBroker extends Command {
	...
	static examples = [
		"# Boot the broker against the local SQLite store\n  op auth-broker serve",
		"# Boot on a non-default port\n  op auth-broker serve --bind=127.0.0.1:9000",
		"# Print the bearer token\n  op auth-broker token",
		"# Rotate the bearer token\n  op auth-broker token --regenerate",
		"# List supported OAuth providers\n  op auth-broker list",
		"# Local login (run on the broker host)\n  op auth-broker login anthropic",
		"# Interactive provider selection\n  op auth-broker login",
		"# Remote login over SSH tunnel\n  op auth-broker login anthropic --via=user@broker",
		"# Log out of a provider (interactive without provider arg)\n  op auth-broker logout anthropic",
		"# Import a CLIProxyAPI auth dump\n  op auth-broker import ~/.cliproxy/auth",
		"# Import a single CLIProxyAPI JSON, overriding the provider mapping\n  op auth-broker import ~/.cliproxy/auth/claude-foo.json --provider anthropic",
		"# Preview a migration from local store + env vars to the configured broker\n  op auth-broker migrate --from-local --include-env --dry-run",
		"# Apply the migration\n  op auth-broker migrate --from-local --include-env",
		"# Health-check the configured remote broker\n  op auth-broker status",
	];

	async run(): Promise<void> {
		const { args, flags } = await this.parse(AuthBroker);
		if (!args.action) {
			renderCommandHelp("op", "auth-broker", AuthBroker);
			return;
		}
