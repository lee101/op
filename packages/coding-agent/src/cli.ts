...
import type { CliConfig, CommandMetadata } from "@openpaths/utils/cli";
import {
	APP_NAME,
	getActiveProfile,
	MIN_BUN_VERSION,
...
import { interceptUnhandledRejections } from "@openpaths/utils/postmortem";
import { setProcessName } from "@openpaths/utils/process-name";
import { declareWorkerHostEntry, installWorkerInbox, isWorkerHostSelector } from "@openpaths/utils/worker-host";
...
// `@openpaths/utils/env` eagerly loads `.env` from the agent directory at
...
async function runSmokeTest(): Promise<void> {
	const { smokeTestSyncWorker, startServer } = await import("@openpaths/stats");
...
async function runWorkerEntrypoint(arg: string | undefined): Promise<boolean> {
	...
	if (arg === STATS_SYNC_WORKER_ARG) {
		...
		await import("@openpaths/stats/sync-worker");
...
export async function runCli(argv: string[]): Promise<void> {
	...
	try {
		...
		if (extracted.profile !== undefined) {
			...
			// No explicit --profile: activate any OP_PROFILE/PI_PROFILE inherited
			...
			// OP_PROFILE after importing this module (profile aliases/tests). Surfacing
			// validation here turns `OP_PROFILE=.. op --version` into a clean error;
			// calling setProfile keeps every later path helper on the env-selected
			// profile instead of the default agent directory.
			setProfile(resolveProfileEnv(process.env.OP_PROFILE, process.env.PI_PROFILE));
		}
		if (extracted.aliasName !== undefined) {
			const profile = extracted.profile ?? getActiveProfile();
			if (!profile) {
				throw new Error("--alias requires --profile <name> or OP_PROFILE");
			}
	...
	// `@openpaths/utils/env` here would snapshot the wrong agent `.env`.
