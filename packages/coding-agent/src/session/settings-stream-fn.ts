 ...
 */
import type { StreamFn } from "@openpaths/agent-core";
import { type SimpleStreamOptions, streamSimple } from "@openpaths/ai";
import { isAnthropicFableOrMythosModel } from "@openpaths/catalog/identity";
...
export function createSettingsAwareStreamFn(settings: Settings, base: StreamFn = streamSimple): StreamFn {
	return (model, context, streamOptions) => {
		...
		const openrouterSortPreset = settings.get("providers.openrouterSort");
		const openrouterSort = openrouterSortPreset && openrouterSortPreset !== "none" ? openrouterSortPreset : undefined;
		...
		const merged: SimpleStreamOptions = {
			...
			openrouterSort: streamOptions?.openrouterSort ?? openrouterSort,
