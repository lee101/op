...
import { AsyncDrain, getAgentDbPath, getDbBusyTimeoutMs, getStatsDbPath, isRecord, logger } from "@openpaths/utils";
...
/** Row shape read from an `op stats` messages table during backfill. */
 ...
 * Delegates auth credential operations to AuthCredentialStore from @openpaths/ai.
...
ON CONFLICT(model_key) DO UPDATE SET
	...
	#initializeSchema(): void {
		...
		// recovery). Without this, concurrent op startups can crash here with
...
`);
	 ...
	 * `op stats` database (`~/.op/stats.db`) into model_perf. Fire-and-forget:
	 ...
	 * A missing stats.db leaves the marker unset so a later `op stats` run
	...
	/**
	 * Imports recent measurable request rows from an `op stats` database
