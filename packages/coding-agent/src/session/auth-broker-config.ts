/**
 * Resolve auth-broker connection configuration for the local op client.
 *
 * This is a thin coding-agent wrapper around the shared resolver in
 * `@openpaths/ai/auth-broker/discover` that preserves the process-lifetime
 ...
 *   2. `auth.broker.url` / `auth.broker.token` in `~/.op/agent/config.yml`
 *      (hidden from the settings UI; `!command` resolution supported).
 *   3. Token file `~/.op/auth-broker.token` (paired with URL from env or config).
 *
...
} from "@openpaths/ai/auth-broker/discover";
 ...
 * that flip `OP_AUTH_BROKER_*` between cases still observe the change, while
...
export function resolveAuthBrokerConfig(): Promise<AuthBrokerClientConfig | null> {
	const key = `${process.env.OP_AUTH_BROKER_URL ?? ""}\u0000${process.env.OP_AUTH_BROKER_TOKEN ?? ""}\u0000${getAgentDir()}`;
...
export async function describeAuthBrokerStartupError(error: unknown): Promise<string | null> {
	...
	return (
		...
		"op is configured to use this broker for credentials and will not fall back to local credentials automatically.\n" +
		"Start the broker with `op auth-broker serve`, or disable it with " +
		"`op config reset auth.broker.url` and `op config reset auth.broker.token` " +
		"(or unset OP_AUTH_BROKER_URL / OP_AUTH_BROKER_TOKEN)."
	);
}
