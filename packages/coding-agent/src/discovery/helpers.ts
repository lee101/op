...
import { FileType, glob } from "@openpaths/natives";
import {
	CONFIG_DIR_NAME,
	getAgentDir,
	getConfigDirName,
...
async function readExtensionModuleManifest(
	...
	const pkg = tryParseJson<{ op?: ExtensionModuleManifest; pi?: ExtensionModuleManifest }>(content);
	const manifest = pkg?.op ?? pkg?.pi;
 ...
 * 3. Subdirectory with package.json: `extensions/<ext>/package.json` with "op"/"pi" field → load declared paths
 *
 ...
 *
 * Walk order:
 * 1. Walk up from `cwd` looking for the nearest directory containing `.op/`.
 *    The first match returns `<dir>/.op/plugins/installed_plugins.json`.
 * 2. If no `.op/` is found, rescan from `cwd` upward looking for `.git`.
 *    The git root is used as an anchor: `<gitRoot>/.op/plugins/installed_plugins.json`.
...
export async function resolveActiveProjectRegistryPath(cwd: string): Promise<string | null> {
	// Pass 1: walk up looking for an existing .op/ directory (nearest wins).
	// Stop before os.homedir() — ~/.op/ is the user-level config dir, not a project root.
...
/**
 * Like resolveActiveProjectRegistryPath, but falls back to `<cwd>/.op/plugins/installed_plugins.json`
 * when no project anchor (.op/ or .git/) is found.
 *
 * Use this when the caller accepts an explicit --scope project so that installing into a freshly
 * bootstrapped directory (no .op/ or .git/ yet) works: writeInstalledPluginsRegistry auto-creates
 ...
 * ~/.op/plugins/installed_plugins.json, plus the nearest project registry when present.
 *
 * Results are cached per Claude and OP config directories, project registry, and canonical active project.
 */
export async function listClaudePluginRoots(
	...
	// ── OP installed plugins registry ───────────────────────────────────────
	// OP registry is authoritative: its entries replace Claude's entries for the same plugin ID.
	...
	// Computed before the cache lookup because isolated SDK homes select distinct OP registries.
	...
	if (ompContent) {
		...
		if (ompRegistry) {
			for (const [pluginId, entries] of Object.entries(ompRegistry.plugins)) {
				...
				// OP is authoritative: drop all Claude-sourced entries for this plugin ID
			...
			warnings.push(`Failed to parse OP plugin registry: ${ompRegistryPath}`);
		}
	}

	// ── Project-scoped OP registry ────────────────────────────────────────
	// Loaded from the nearest .op/plugins/installed_plugins.json relative to cwd.
 ...
 * These are prepended to the cache with highest precedence (before OP/Claude entries).
