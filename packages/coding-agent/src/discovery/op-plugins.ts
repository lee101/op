/**
 * OP extension-package sub-discovery provider.
 *
 ...
 * `rules/`, `prompts/`, and `.mcp.json` — are picked up by op's standard
 * discovery surfaces. The native `op` provider in `builtin.ts` only walks
 * `.op/` and `~/.op/agent/`, so without this provider those sub-trees are
 * silently ignored.
 *
 * Provider priority is set below the native `op` provider (100) so an
 * extension package never shadows the user's own `.op/` configuration on
 * dedup.
 *
 * @see ./op-extension-roots.ts
 * @see ../../docs/extension-loading.md
 */
import * as path from "node:path";
import { logger, parseFrontmatter, tryParseJson } from "@openpaths/utils";
...
import { listOmpExtensionRoots, type OpExtensionRoot } from "./op-extension-roots";
import { resolvePluginStdioPaths } from "./substitute-plugin-root";

const PROVIDER_ID = "op-plugins";
const DISPLAY_NAME = "OP Extension Packages";
const DESCRIPTION =
 ...
 */
async function allowedRoots(ctx: LoadContext, surface: "skills" | "mcp" | "other"): Promise<OpExtensionRoot[]> {
...
async function loadHooks(ctx: LoadContext): Promise<LoadResult<Hook>> {
	const roots = await allowedRoots(ctx, "other");
	const tasks: Array<{ root: OpExtensionRoot; hookType: "pre" | "post" }> = [];
...
async function loadMCPServers(ctx: LoadContext): Promise<LoadResult<MCPServer>> {
	...
	const tasks: Array<{ root: OpExtensionRoot; mcpPath: string }> = [];
	for (const root of roots) {
	...
	for (let i = 0; i < tasks.length; i++) {
		...
		if (!parsed) {
			warnings.push(`[op-plugins] Invalid JSON in ${mcpPath}`);
			logger.warn(`[op-plugins] Invalid JSON in ${mcpPath}`);
			continue;
		}
		...
		for (const [serverName, serverCfg] of Object.entries(servers)) {
			...
			if (typeof cfg.command !== "string" && typeof cfg.url !== "string") {
				warnings.push(`[op-plugins] Skipping MCP server "${serverName}" in ${mcpPath}: missing command or url`);
				continue;
			}
