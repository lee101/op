 ...
 * `prompts/`, `.mcp.json`) are wired into discovery by `op-plugins.ts`.
 *
...
import { getAgentDir, isEnoent, logger, tryParseJson } from "@openpaths/utils";
...
export interface OpExtensionRoot {
...
export interface InjectOmpExtensionCliRootOptions {
	 ...
	 * contribute sibling capabilities through the `op-plugins` provider.
	 */
	mode?: OpExtensionRootMode;
...
export function getInjectedOmpExtensionCliRoots(): readonly OpExtensionRoot[] {
 ...
 * 2. Project `<cwd>/.op/settings.json#extensions`
 * 3. User `~/.op/agent/settings.json#extensions`
 * 4. Enabled npm/link plugins installed under `<plugins>/node_modules/` (for
 *    `op install <pkg>` / `op plugin install` / `op plugin link`). Marketplace
 ...
 */
export async function listOmpExtensionRoots(ctx: LoadContext): Promise<OpExtensionRoot[]> {
 ...
 * "Claude Code Marketplace" and "OP Extension Packages".
 */
