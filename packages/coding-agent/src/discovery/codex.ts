...
import { logger, parseFrontmatter } from "@openpaths/utils";
...
async function loadHooks(ctx: LoadContext): Promise<LoadResult<Hook>> {
	...
	// OP hooks must be named `pre-<tool>.<ts|js>` or `post-<tool>.<ts|js>`.
	// Files without that prefix are not OP hooks (e.g. the standalone Codex
