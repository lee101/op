 ...
 * Builtin Provider (.op)
 *
 * Primary provider for OP native configs. Supports all capabilities.
 */
import * as path from "node:path";
import { getAgentDir, logger, parseFrontmatter, tryParseJson } from "@openpaths/utils";
...
const DISPLAY_NAME = "OP";
const DESCRIPTION = "Native OP configuration from ~/.op and .op/";
const PRIORITY = 100;
...
async function getConfigDirs(ctx: LoadContext): Promise<Array<{ dir: string; level: "user" | "project" }>> {
	...
	// profile's agent dir (~/.op/profiles/<name>/agent), like sessions and MCP.
...
async function loadSkills(ctx: LoadContext): Promise<LoadResult<Skill>> {
	// Walk up from cwd finding .op/skills/ in ancestors (closest first)
	...
	// User-level scan from ~/.op/agent/skills/
...
registerProvider<Skill>(skillCapability.id, {
	...
	description: "Auto-generated managed skills from ~/.op/agent/managed-skills",
...
async function loadRules(ctx: LoadContext): Promise<LoadResult<Rule>> {
	...
	// https://openpaths.io/docs/context-files as the file that gets "re-injected near
	// the current turn so they keep hold across long conversations".
	// User scope:    ~/.op/agent/RULES.md
	// Project scope: nearest .op/RULES.md walking up from cwd to repoRoot
...
registerProvider<ContextFile>(contextFileCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "Load AGENTS.md from .op/ directories",
	priority: PRIORITY,
