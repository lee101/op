 ...
 *
 * Handles `op usage` — fetches provider usage reports for every
...
import { formatDuration, formatNumber, sanitizeText } from "@openpaths/utils";
...
/**
 * Tombstones worth a row in `op usage`: OAuth credentials torn down
 ...
 * `op usage --provider xai` can still confirm the stored credential has no
 * usage endpoint.
 */
		...
		if (cmd.history) {
			...
			if (entries.length === 0) {
				...
				process.stderr.write(
					chalk.yellow(
						`No usage history recorded${scope} yet. Snapshots accumulate whenever usage is fetched (TUI footer, /usage, op usage).\n`,
					),
				);
				process.exitCode = 1;
				return;
		...
		if (filteredReports.length === 0 && accounts.length === 0) {
			...
			const message =
				storedAccounts.length > 0
					...
					: `No credentials found${scope}. Run \`op\` and use /login to add accounts.\n`;
