 ...
 *
 * Handles `op update` to check for and install updates.
 * Uses the installer that owns the active op executable when it can be detected.
 */
...
import { $env, $which, APP_NAME, compareVersions, isEnoent, VERSION } from "@openpaths/utils";
import chalk from "@openpaths/utils/chalk";
import { withFileLock } from "@openpaths/utils/file-lock";
import { $ } from "bun";
import { theme } from "../modes/theme/theme";
import { isTimeoutError, withTimeoutSignal } from "../utils/fetch-timeout";

const REPO = "lee101/op";
const PACKAGE = "@openpaths/coding-agent";
const HOMEBREW_FORMULA = "can1357/tap/op";
const MISE_TOOL = "github:lee101/op";
 ...
 */
const NATIVES_PACKAGE = "@openpaths/natives";

/**
 * Platform tags the release pipeline publishes as
 * `@openpaths/natives-<tag>` leaves. Mirrors `SUPPORTED_PLATFORMS` in
...
/** Parsed `op.rename` pointer: the new agent package name and optional new natives name. */
...
export interface ReleaseInfo {
	tag: string;
	version: string;
	/** Parsed `op.dist` from the registry manifest; undefined when absent. */
	dist?: ReleaseDist;
	/** npm names to install, resolved after following any `op.rename` pointers. */
...
/**
 * Parse the `op.dist` field from a published package manifest.
 *
 ...
 * `"op": { "dist": "binary" }` in its package.json. Any value other than
...
export function resolveReleaseDist(manifest: unknown): ReleaseDist | undefined {
	if (!isRecord(manifest) || !isRecord(manifest.op)) return undefined;
	const dist = manifest.op.dist;
	if (dist === undefined) return undefined;
	return dist === "npm" ? "npm" : "binary";
}

/**
 * Parse the `op.rename` pointer from a published package manifest.
 *
 ...
 * `"op": { "rename": { "package": "<new-agent-pkg>", "natives": "<new-natives-pkg>" }, "dist": "binary" }`.
...
export function resolveReleaseRename(manifest: unknown): ReleaseRename | undefined {
	if (!isRecord(manifest) || !isRecord(manifest.op)) return undefined;
	const rename = manifest.op.rename;
 ...
 *
 * An explicit `op.dist` wins in both directions. Without one, a release with
 ...
 * `@openpaths/natives*` companions ({@link buildBunInstallArgs}) may not
...
function isPathInDirectory(filePath: string, directoryPath: string): boolean {
	...
	// PATH-resolved op path can refer to the same directory through different
	...
	// links like Homebrew's `bin/op -> Cellar/.../bin/op`; the parent fallback
...
interface UpdateMethodResolutionOptions {
	...
	bunGlobalDir?: string;
	/**
	 * Whether the resolved op path is a plain file (the standalone binary)
...
/**
 * Get the latest release info from the npm registry, following `op.rename`
 ...
 * (`react@19.2.6@@@1`). Global `op` updates can leave one full copy per
...
async function pruneBunCacheAfterGlobalInstall(): Promise<BunInstallCachePruneResult | undefined> {
	...
	const packageNames = globalNodeModulesDir
		...
		: new Set<string>();
	if (packageNames.size === 0 && !path.basename(cacheDir).toLowerCase().includes("op")) return undefined;
...
/**
 * Resolve the path that `op` maps to in the user's PATH.
 */
...
/**
 * Run the PATH-resolved op binary and check if it reports the expected version.
 */
...
async function printVerification(expectedVersion: string): Promise<void> {
	...
	console.log(chalk.yellow(`You may need to reinstall: curl -fsSL https://openpaths.io/install | sh`));
}
...
/**
 * Build the bun argv used to globally install a specific op version.
 *
 ...
 *
 * Together these two flags make `op update` produce exactly the registry
 * lookup the version check just performed. See #1686.
 *
 * Also pins {@link NATIVES_PACKAGE} and the platform-specific
 * `@openpaths/natives-<tag>` leaf to `expectedVersion`. `bun install -g`
 ...
 * `@openpaths/coding-agent` and the loader aborts at
 ...
 *
 * `force` is set only for rename migrations: npm refuses to write the `op`
 ...
 * never leaves the user without a working `op`.
 */
...
export interface RenameMigrationSteps {
	/** Globally install the new package names. MUST be idempotent: re-running re-links the `op` bin. */
	install(): Promise<number>;
	/** Remove the old-name globals. */
	removeOld(): Promise<number>;
	/** Check the PATH-resolved `op` against the expected version. */
...
/**
 * Migrate a package-manager install across an `op.rename` hop without a
 * window where no working `op` exists:
 *
 ...
 * 3. Verify the PATH-resolved `op`. If the removal deleted the shared bin
...
export async function migrateRenamedInstall(release: ReleaseInfo, steps: RenameMigrationSteps): Promise<void> {
	...
	if (!verification.ok) {
		throw new Error(
			`${formatVerificationFailure(verification, release.version)}; reinstall with: curl -fsSL https://openpaths.io/install | sh`,
		);
	}
...
export async function updateViaBinaryAt(
	...
	// Unique per attempt so two overlapping `op update` runs never share a temp
	...
	// overlapping `op update` runs never replace the same binary concurrently
 ...
 * `op.exe`. Rewriting matters for the shims that outrank `.exe` at command
 ...
 * (`op`/`op.cmd`/`op.ps1`) that cannot be overwritten with a native
 * executable. The release binary is installed as `op.exe` beside them and
...
function installerHint(): string {
	return process.platform === "win32"
		? "& ([scriptblock]::Create((irm https://openpaths.io/install.ps1))) -Binary"
		: "curl -fsSL https://openpaths.io/install | sh -s -- --binary";
}
...
export async function runUpdateCommand(opts: { force: boolean; check: boolean }): Promise<void> {
	...
	// Choose update method based on the prioritized op binary in PATH. For
	...
	try {
		...
		if (target.method === "nix") {
			...
			console.log(chalk.dim("Update the flake input or profile that provides op, then rebuild."));
