 ...
 * `@openpaths/utils` (or anything that transitively does). That package's
 ...
 * runtime imports from `@openpaths/utils`, which is the whole reason it can
 ...
 */
export const PROFILE_BOOTSTRAP_BOUNDARY_ARG = "--op-profile-boundary";
 ...
 * argument — `op --print --profile work` still selects a profile) apart from an
 ...
 * a flag's value instead of mistaking it for the subcommand — `op --model acp`
