/**
 * `op models` — list, search, and refresh available models.
 *
 * Subcommands:
 ...
 *   then list. This is the supported replacement for `rm -rf ~/.op/models.db`
 ...
 */
import type { Api, Effort, Model } from "@openpaths/ai";
import { getSupportedEfforts } from "@openpaths/catalog/model-thinking";
import { formatNumber, getProjectDir } from "@openpaths/utils";
 ...
 * name doubles as an `op models <provider>` shortcut.
 */
...
/** `op models ls`/`find`: provider-grouped listing (one box table per provider). */
...
/**
 * Entry point for the standalone `op models` command: bootstraps auth storage,
...
export async function runModelsCommand(command: ModelsCommandArgs): Promise<void> {
	...
	if (action === "find" && (!pattern || pattern.trim().length === 0)) {
		process.stderr.write("`op models find` requires a search substring, e.g. `op models find minimax`\n");
		process.exitCode = 1;
		return;
	}
