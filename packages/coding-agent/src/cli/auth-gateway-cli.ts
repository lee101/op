/**
 * `op auth-gateway` command handlers.
 *
 ...
 * `OP_AUTH_BROKER_URL` / `auth.broker.url` precedence used elsewhere).
 *
 * Sub-verbs:
...
} from "@openpaths/ai/auth-broker";
import { DEFAULT_AUTH_GATEWAY_BIND, startAuthGateway } from "@openpaths/ai/auth-gateway";
import { type GeneratedProvider, getBundledModels } from "@openpaths/catalog/models";
import { getConfigRootDir, isEnoent, logger, VERSION } from "@openpaths/utils";
...
async function runServe(flags: AuthGatewayCommandArgs["flags"]): Promise<void> {
	...
	if (!brokerConfig) {
		throw new Error(
			"`op auth-gateway serve` requires OP_AUTH_BROKER_URL (or `auth.broker.url`/`auth.broker.token` in config.yml). The gateway is itself a broker client.",
		);
	}
	...
	// cached + broker-discovered), so the discovery-only models op itself
...
async function runStatus(flags: AuthGatewayCommandArgs["flags"]): Promise<void> {
	...
	if (!brokerConfig) {
		...
		if (flags.json) {
			...
			process.stdout.write(`${chalk.yellow("No broker configured.")} Set OP_AUTH_BROKER_URL.\n`);
			process.stdout.write(
	...
	try {
		...
		if (flags.json) {
			...
			if (!tokenPresent) {
				process.stdout.write(
					"Run `op auth-gateway token` or `op auth-gateway serve` to create a bearer token.\n",
				);
			}
		}
...
/**
 * `op auth-gateway check` — probe each broker-supplied credential and print
...
async function runCheck(flags: AuthGatewayCommandArgs["flags"]): Promise<void> {
	...
	if (!brokerConfig) {
		throw new Error(
			"`op auth-gateway check` requires OP_AUTH_BROKER_URL (or `auth.broker.url`/`auth.broker.token` in config.yml). It probes the same credentials the gateway would serve.",
		);
	}
