import type { ResolvedThinkingLevel } from "@openpaths/agent-core";
import type {
	Api,
	ApiKeyResolver,
	AssistantMessage,
...
import { resolveModelServiceTier, streamSimple } from "@openpaths/ai";
import { buildModelProviderPriorityRank } from "@openpaths/catalog/identity";
import { replaceTabs, truncateToWidth } from "@openpaths/tui";
import { formatDuration, getProjectDir, prompt } from "@openpaths/utils";
...
interface BenchRequestOptions {
	...
	sessionId: string;
	prompt: string;
	/** Native OP messages; cache mode splits the stable prefix from the suffix. */
...
export async function runBenchCommand(command: BenchCommandArgs, deps: BenchDependencies = {}): Promise<BenchSummary> {
	...
	if (command.models.length === 0) {
		throw new Error("Pass at least one model selector, e.g. `op bench opus gpt-5.2`");
	}
		...
		for (const { selector, model, thinking } of targets) {
			...
			if (!preflightKey) {
				const failure: BenchRunFailure = {
					ok: false,
					error: `No credentials for provider "${model.provider}". Run \`op\` and use /login, or set the provider API key.`,
				};
