 ...
 *
 * Handles `op stats` subcommand for viewing AI usage statistics.
 */

import { truncateToWidth } from "@openpaths/tui/utils";
...
async function printStatsSummary(): Promise<void> {
	const { getDashboardStats } = await import("@openpaths/stats");
	const stats = await getDashboardStats();
