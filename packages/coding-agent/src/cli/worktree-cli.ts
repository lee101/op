/**
 * CLI handler for `op worktree` — list and clean up agent-managed worktrees.
 *
 * Layout under `~/.op/wt/`:
 *
 ...
 *     marker naming the live op process; a
...
import { getWorktreesDir, isEnoent } from "@openpaths/utils";
...
export interface WorktreeEntry {
	/** Absolute path to the worktree dir (or stray container) under `~/.op/wt/`. */
	path: string;
	...
	branch?: string;
	/** When set, the entry is unhealthy and `op worktree clear` will remove it. */
	orphanReason?: string;
}
...
async function scanWorktrees(): Promise<WorktreeEntry[]> {
	...
	for (const name of topLevel) {
		...
		// Legacy nesting: ~/.op/wt/<encoded-project>/<branch-or-id>
		let children: string[];
		try {
