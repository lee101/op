...
import { normalizeProfileName } from "@openpaths/utils/dirs";
...
function validateAliasName(aliasName: string, shell: ProfileAliasShell): string {
	...
	if (normalized.toLowerCase() === "op") {
		throw new Error('Invalid alias "op". Refusing to shadow the base op command.');
	}
...
function resolveShellConfigPath(
	...
	switch (shell) {
		...
		case "fish": {
			...
			return posixJoinUnc(configHome, "fish", "conf.d", "op-profiles.fish");
		}
		case "pwsh":
...
function renderAliasBlock(
	...
	const start = `# >>> op profile alias: ${aliasName} >>>`;
	const end = `# <<< op profile alias: ${aliasName} <<<`;
	let body: string;
	switch (shell) {
		case "fish":
			body = [
				`function ${aliasName} --wraps op --description 'OP profile ${profile}'`,
