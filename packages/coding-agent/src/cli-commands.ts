 ...
 */
import type { CommandEntry } from "@openpaths/utils/cli";
...
// commands. Without a guard `resolveCliArgv` rewrites e.g. `op marketplace add
// xyz` to `op launch marketplace add xyz`, silently forwarding the argv to the
...
// The real commands live under `op plugin <action>`; each entry maps a verb to
...
const RESERVED_TOP_LEVEL_WORDS: Record<string, string> = {
	extensions:
		'`op extensions` is not a management command. Use `op plugin list` / `op plugin install`, or run `op launch extensions` if you meant to send "extensions" as a prompt.',
	list: '`op list` is not a top-level command. Use `op plugin list` to list installed plugins, or run `op launch list` if you meant to send "list" as a prompt.',
	remove:
		'`op remove` is not a top-level command. Use `op plugin uninstall <name>` to remove a plugin, or run `op launch remove` if you meant to send "remove" as a prompt.',
	uninstall:
		'`op uninstall` is not a top-level command. Use `op plugin uninstall <name@marketplace>` to remove a plugin, or run `op launch uninstall` if you meant to send "uninstall" as a prompt.',
	marketplace:
		'`op marketplace` is not a top-level command. Use `op plugin marketplace <add|remove|update|list>` to manage marketplaces, or run `op launch marketplace` if you meant to send "marketplace" as a prompt.',
	discover:
		'`op discover` is not a top-level command. Use `op plugin discover [marketplace]` to browse available plugins, or run `op launch discover` if you meant to send "discover" as a prompt.',
	upgrade:
		'`op upgrade` is not a top-level command. Use `op plugin upgrade [name@marketplace]` to upgrade plugins, or run `op launch upgrade` if you meant to send "upgrade" as a prompt.',
	enable:
		'`op enable` is not a top-level command. Use `op plugin enable <name@marketplace>` to enable a plugin, or run `op launch enable` if you meant to send "enable" as a prompt.',
	disable:
		'`op disable` is not a top-level command. Use `op plugin disable <name@marketplace>` to disable a plugin, or run `op launch disable` if you meant to send "disable" as a prompt.',
};

// Sub-actions that make `op marketplace <sub>` unambiguously a management
// command even when multi-word (the reporter's `op marketplace add xyz`,
 ...
 *
 * A bare verb (`op marketplace`) always hints. A multi-word invocation only
 * hints when the arguments follow the documented plugin grammar — a marketplace
 * sub-action (`op marketplace add …`) or a `name@marketplace` plugin id
 * (`op uninstall foo@bar`) — so genuine prompts that merely begin with one of
 * these words (`op list all my files`, `op upgrade the deps`) still launch.
 *
...
export function resolveCliArgv(argv: string[]): ResolvedCliArgv {
	...
	// (`op --approval-mode=yolo acp`). `run` dispatches strictly on argv[0], so
