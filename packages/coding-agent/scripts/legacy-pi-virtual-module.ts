...
import { isEnoent } from "@openpaths/utils/fs-error";

/** Build-time specifier resolved to bundled legacy Pi module namespaces. */
export const LEGACY_PI_MODULES_SPECIFIER = "op-legacy-pi-modules";

const VIRTUAL_NAMESPACE = "op-legacy-pi-modules-build";
...
export async function createLegacyPiVirtualModulePlugin(): Promise<Bun.BunPlugin> {
	...
	return {
		name: "op:legacy-pi-modules",
		setup(build) {
			build.onResolve({ filter: /^op-legacy-pi-modules$/ }, () => ({
