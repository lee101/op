...
Shared utilities for [openpaths](https://github.com/lee101/op) packages. Zero ceremony, Bun-first.

## Notable modules

| Module | Purpose |
| --- | --- |
| `logger` | Centralized logger writing to `~/.op/logs/` with rotation (TUI-safe — never stdout) |
| `prompt` | Handlebars-based prompt templating and formatting helpers |
| `dirs` | Path helpers for op config directories (`~/.op`, XDG-aware on Linux) |
...
Import from the root barrel or per-module subpaths (`@openpaths/utils/<module>`).

## Install

```sh
bun add @openpaths/utils
```
...
- [Monorepo README](https://github.com/lee101/op#readme)
