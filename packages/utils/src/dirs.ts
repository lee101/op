/**
 * Centralized path helpers for op config directories.
 *
 * Uses PI_CONFIG_DIR (default ".op") for the config root and
 * PI_CODING_AGENT_DIR to override the agent directory.
 *
 * On Linux, if XDG_DATA_HOME / XDG_STATE_HOME / XDG_CACHE_HOME environment
 * variables are set, paths are redirected to XDG-compliant locations under
 * $XDG_*_HOME/op/. This requires running `op config migrate` first to
 * move data to the new locations. No filesystem existence checks are performed
 * — if the env var is set, op trusts that the migration has been done.
 */
...
/** Default User-Agent header string (e.g. "op/17.2.12") */
export const USER_AGENT = `op/${VERSION}`;

/** Minimum Bun version */
export const MIN_BUN_VERSION: string = engines.bun.replace(/[^0-9.]/g, "");

const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9._-]{0,63}$/;
const PROFILE_ENV_KEYS = ["OP_PROFILE", "PI_PROFILE"] as const;
 ...
 *
 * Exported so consumers of `@openpaths/utils/dirs` (CLI bootstrap, tests,
...
export function normalizeProfileName(profile: string | undefined): string | undefined {
	...
	if (
		...
		throw new Error(
			`Invalid OP profile "${profile}". Profile names must match ${PROFILE_NAME_RE.source}, ` +
...
/**
 * Resolve the active profile from the two profile env vars. `OP_PROFILE` is the
 * canonical variable and takes precedence; `PI_PROFILE` is the legacy
 * compatibility fallback, consulted only when `OP_PROFILE` is undefined. An
 * explicitly-empty `OP_PROFILE` therefore selects the default profile rather
 ...
 */
export function resolveProfileEnv(op: string | undefined, pi: string | undefined): string | undefined {
	return normalizeProfileName(op !== undefined ? op : pi);
}

function getProfileFromEnv(): string | undefined {
	return resolveProfileEnv(process.env.OP_PROFILE, process.env.PI_PROFILE);
}

/**
 * Module-load profile resolution. Unlike {@link getProfileFromEnv}, an invalid
 * OP_PROFILE/PI_PROFILE value does NOT throw here — a bad env var must not
 ...
 * user still gets a clean "Invalid OP profile" message.
 */
...
/** Get the config agent directory name relative to home (e.g. ".op/agent" or PI_CONFIG_DIR + "/agent"). */
...
/**
 * Resolves and caches all op directory paths. On Linux, when XDG environment
 * variables are set, paths are redirected under $XDG_*_HOME/op/. A new
...
class DirResolver {
	...
	// With XDG on Linux, they point to $XDG_*_HOME/op/.
	...
	constructor(options: { agentDirOverride?: string; profile?: string } = {}) {
		...
		// resolves under $XDG_*_HOME/op once `op config init-xdg` has migrated
		...
		// profile could resolve to `~/.op/profiles/<name>` on first activation
		// (when no $XDG_*_HOME/op exists yet) and then silently move to
		// `$XDG_*_HOME/op/profiles/<name>` the moment the base appeared, orphaning
		...
		// XDG flattens the agent/ prefix: ~/.op/agent/sessions → $XDG_DATA_HOME/op/sessions
		this.#agentDirs = {
 ...
 * `PI_PROFILE` that was bypassed because `OP_PROFILE` explicitly selected the
 ...
 * `~/.op/agent`. Captured at module load — ignoring a profile-derived value
...
/** Get the config root directory (~/.op). */
...
export function setProfile(profile: string | undefined): void {
	...
	if (activeProfile) {
		...
		process.env.OP_PROFILE = activeProfile;
...
/** Get the agent config directory (~/.op/agent). */
export function getAgentDir(): string {
	return dirs.agentDir;
}

/** Get the project-local config directory (.op). */
export function getProjectAgentDir(cwd: string = getProjectDir()): string {
	return path.join(cwd, CONFIG_DIR_NAME);
}

// =============================================================================
// Config-root subdirectories (~/.op/*)
// =============================================================================

/** Get the reports directory (~/.op/reports). */
export function getReportsDir(): string {
	return dirs.rootSubdir("reports", "state");
}

/** Get the logs directory (~/.op/logs). */
...
/**
 * Get the plugins directory (~/.op/plugins or its XDG equivalent).
 *
 ...
 * (PR checkout, task isolation) and cleanup (`op worktree`). A relative value
 * would resolve against whatever cwd happened to launch `op`, so checkout and
 ...
 * isolation, and `op worktree` cleanup all read the same base). Driven by the
 * `worktree.base` setting in coding-agent; pass `undefined`/empty to clear and
 * fall back to `OP_WORKTREE_DIR` or the `~/.op/wt` default.
 *
 ...
 * `OP_WORKTREE_DIR` env var, then the {@link setWorktreesDir} override (the
 * `worktree.base` setting), then the `~/.op/wt` default. The env var and the
 * override are both `~`-expanded and must be absolute; a relative value is
 * ignored and resolution falls through.
 */
export function getWorktreesDir(): string {
	return resolveWorktreeBase(process.env.OP_WORKTREE_DIR) ?? worktreesDirOverride ?? dirs.rootSubdir("wt", "data");
}

/** Get the SSH control socket directory (~/.op/ssh-control). */
export function getSshControlDir(): string {
	return dirs.rootSubdir("ssh-control", "state");
}

/** Get the remote host info directory (~/.op/remote-host). */
export function getRemoteHostDir(): string {
	return dirs.rootSubdir("remote-host", "data");
}

/** Get the managed Python venv directory (~/.op/python-env). */
export function getPythonEnvDir(): string {
	return dirs.rootSubdir("python-env", "data");
}

/** Get the shared Python gateway state directory (~/.op/agent/python-gateway; XDG default: $XDG_STATE_HOME/op/python-gateway). */
export function getPythonGatewayDir(): string {
	return dirs.agentSubdir(undefined, "python-gateway", "state");
}

/** Get the puppeteer sandbox directory (~/.op/puppeteer). */
export function getPuppeteerDir(): string {
	return dirs.rootSubdir("puppeteer", "cache");
}

/** Get the browser relay extension install directory (~/.op/browser-relay). */
...
/** Get the auto-QA grievances SQLite database path (~/.op/autoqa.db; XDG: $XDG_DATA_HOME/op/autoqa.db). */
 ...
 * (e.g. PR-checkout and task-isolation worktree dirs under `~/.op/wt/`).
...
/** Get the path to a single worktree directory (~/.op/wt/<segment>). */
...
/**
 * Get the encrypted auth-broker snapshot cache path (~/.op/cache/auth-broker-snapshot.enc).
 * Honors the `OP_AUTH_BROKER_SNAPSHOT_CACHE` env var when set so tests and
...
/** Get the last-seen-changelog-version marker file (~/.op/agent/last-changelog-version). */
...
/** Get the tiny title model cache directory (~/.op/agent/cache/tiny-models). */
...
/** Get the document conversion cache directory (~/.op/agent/cache/document-conversions; XDG default: $XDG_CACHE_HOME/op/cache/document-conversions). */
...
/** Get the debug log path (~/.op/agent/op-debug.log). */
 ...
 * persisted transcripts). The legacy file is left in place for older op
...
/** Get the secret placeholder key path (~/.op/agent/secret-placeholder.key; XDG default: $XDG_STATE_HOME/op/secret-placeholder.key). Adopts a legacy key on first XDG resolution. */
...
/** Get the marketplaces registry path (~/.op/marketplaces.json; XDG default: $XDG_DATA_HOME/op/marketplaces.json). Adopts a legacy registry on first XDG resolution. */
...
/**
 * Persistent per-install UUID stored at `~/.op/install-id`.
 *
 ...
 * `~/.op/agent/` does not regenerate it. Server-side dedup for grievance
 * pushes (and similar telemetry) keys on this id.
 *
 * Anchored to the base config root (`~/.op/install-id`) regardless of the
