 ...
 * crash restore). Unlike codex we redirect to the op log file — not
...
export interface SuppressTerminalStderrOptions {
	/** Redirect target path; defaults to today's op log file, then /dev/null. */
	redirectPath?: string;
...
export function suppressTerminalStderr(options?: SuppressTerminalStderrOptions): boolean {
	...
	try {
		...
		// on a fresh profile ~/.op/logs may not exist yet. Create it here so
