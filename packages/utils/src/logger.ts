 ...
 *
 * Each entry includes `process.pid` so concurrent op instances stay
 * traceable.
 */
...
const PROCESS_LOG_PATTERN = /^op\.(\d{4}-\d{2}-\d{2})\.(\d+)\.log(?:\.(\d+))?$/;
const PROCESS_AUDIT_PATTERN = /^\.op\.(\d+)-audit\.json$/;
