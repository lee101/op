 ...
 * - **Dev runs only.** In the compiled `op` binary every module is pre-bundled
...
const MODULE_COMPLETE_KEY: symbol = Symbol.for("op.moduleLoadComplete");
const MODULE_BODY_START_KEY: symbol = Symbol.for("op.moduleBodyStart");
...
function bodyStartMarker(path: string): string {
	return `;globalThis[Symbol.for("op.moduleBodyStart")]?.(${JSON.stringify(path)});\n`;
}

function completionMarker(path: string): string {
	return `\n;globalThis[Symbol.for("op.moduleLoadComplete")]?.(${JSON.stringify(path)});\n`;
}
