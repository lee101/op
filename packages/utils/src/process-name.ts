/**
 * Set the OS-visible process name (`/proc/self/comm`) so `op` shows up as
 * `op` — not `bun` — in `ps`, `pgrep`, `killall`, `top`, `htop`, and systemd.
 *
 ...
 * `bun` and process-name-based tooling can't target op (and `pkill bun` becomes
