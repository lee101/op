...
export function filterChildShellEnv(
	...
	for (const key in launchEnv) {
		...
		if (launchEnvValues || projectEnvNamesLoadedByOmp.has(key)) {
			...
			// absent from it, or OP itself injected the value — either way it came
 ...
 * `OP_` variables to their `PI_` aliases.
 */
 ...
 *
 * All users should import this env module (import { $env } from "@openpaths/utils")
 ...
 * (SQLITE_BUSY during WAL recovery/checkpoint — see openpaths#2421): the
