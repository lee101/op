...
describe("document conversion cache directory", () => {
	...
	beforeEach(async () => {
		...
		originalOmpProfile = process.env.OP_PROFILE;
	...
	afterEach(async () => {
		...
		restoreEnv("OP_PROFILE", originalOmpProfile);
	...
	it("uses XDG_CACHE_HOME for the default agent dir when $XDG_CACHE_HOME/op exists", async () => {
		...
		expect(getDocumentConversionCacheDir()).toBe(
			path.join(process.env.XDG_CACHE_HOME, "op", "cache", "document-conversions"),
		);
	});
...
describe("test directory state cleanup", () => {
	it("restores the active profile from the current env after setAgentDir mutations", () => {
		...
		const originalOmpProfile = process.env.OP_PROFILE;
		const originalPiProfile = process.env.PI_PROFILE;
		const originalXdgCacheHome = process.env.XDG_CACHE_HOME;
		try {
			process.env.OP_PROFILE = "cache-profile";
...
describe("legacy file adoption on XDG paths", () => {
	...
	it("adopts legacy files at the XDG paths without clobbering existing XDG files", async () => {
		...
		await fs.mkdir(path.join(xdgState, "op"), { recursive: true });
		await fs.mkdir(path.join(xdgData, "op"), { recursive: true });
		// Legacy layout: key under ~/.op/agent, registry under ~/.op.
		await fs.mkdir(path.join(tempRoot, ".op", "agent"), { recursive: true });
		await fs.writeFile(path.join(tempRoot, ".op", "agent", "secret-placeholder.key"), "legacy-key");
		await fs.writeFile(path.join(tempRoot, ".op", "marketplaces.json"), '{"legacy":true}');
		// The XDG registry is already populated: adoption must not overwrite it.
		await fs.writeFile(path.join(xdgData, "op", "marketplaces.json"), '{"xdg":true}');
		...
		expect(key).toBe(path.join(xdgState, "op", "secret-placeholder.key"));
		expect(registry).toBe(path.join(xdgData, "op", "marketplaces.json"));
	...
	it("keeps the legacy paths canonical when XDG is inactive", async () => {
		if (process.platform === "win32") return;
		activateTempHome({});
		expect(getSecretPlaceholderKeyPath()).toBe(path.join(tempRoot, ".op", "agent", "secret-placeholder.key"));
		expect(getMarketplacesRegistryPath()).toBe(path.join(tempRoot, ".op", "marketplaces.json"));
	});
});
