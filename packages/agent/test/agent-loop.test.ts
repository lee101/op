...
} from "@openpaths/agent-core/agent-loop";
import type {
	AgentContext,
	AgentEvent,
	AgentLoopConfig,
	...
	AgentToolContext,
	ToolCallContext,
} from "@openpaths/agent-core/types";
import { ASIDE_MESSAGE_COMMIT, ASIDE_MESSAGE_DISCARD } from "@openpaths/agent-core/types";
import type { AssistantMessage, AssistantMessageEvent, Context, Message, ToolResultMessage } from "@openpaths/ai";
...
declare module "@openpaths/agent-core/types" {
...
describe("agentLoop kCursorExecResolved (issue #4348)", () => {
	it("skips execute for a toolCall block marked as already run by Cursor's exec channel", async () => {
		const { kCursorExecResolved } = await import("@openpaths/ai/utils/block-symbols");
