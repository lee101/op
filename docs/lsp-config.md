# LSP configuration in OP

This guide explains how to configure language servers for the OP coding agent.
...
When no config file contributes a server override, OP auto-detects built-in servers by intersecting two conditions:
...
OP merges LSP config from multiple sources, lowest to highest precedence:
...
|            | Cwd config dirs: `<cwd>/.op/lsp.*`, `<cwd>/.claude/lsp.*`, `<cwd>/.codex/lsp.*`, `<cwd>/.gemini/lsp.*`      |
...
The native user config directory follows `PI_CONFIG_DIR` and active profiles; `~/.op/agent/lsp.json` is the default-profile spelling. This shared config lookup does not use `PI_CODING_AGENT_DIR` as an arbitrary replacement base. Project and cwd sources do not walk ancestors.

**Recommended locations:**

- User-wide preferences → active native agent directory's `lsp.json`
- Project-specific overrides → `<cwd>/.op/lsp.json`

> **Note:** Auto-detection mode is skipped only when at least one readable config contributes a non-empty server map. A config that only sets `idleTimeoutMs` still uses built-in auto-detection. With server overrides, OP first merges them onto all defaults, then keeps servers whose root markers match the cwd, whose binary resolves, and whose merged config is not `disabled`.
...
The `capabilities` object enables optional server-specific features that OP supports on a per-server basis:
...
Place the override in `<project>/.op/lsp.json`:
...
The user-level config in `~/.op/agent/lsp.json` is unaffected; pylsp is only suppressed in this project.
