...
The SDK is the in-process integration surface for `@openpaths/coding-agent`.
...
```bash
bun add @openpaths/coding-agent
```
...
The package root, `@openpaths/coding-agent`, is the complete embedding surface. It includes `createAgentSession` and the focused `/sdk` exports, plus lower-level session, auth, model, mode, extension, and tool APIs.
...
- `cwd`: `getProjectDir()`
- `agentDir`: `~/.op/agent` (via `getAgentDir()`)
...
```ts
import { createAgentSession, SessionManager } from "@openpaths/coding-agent";
...
```ts
import { SessionManager } from "@openpaths/coding-agent";
...
```ts
import type { AgentSession } from "@openpaths/coding-agent";
