...
The compiled macOS `op` binaries shipped on GitHub Releases can be signed with a
...
submission (see [#776](https://github.com/lee101/op/issues/776)).
...
- `curl https://openpaths.io/install | sh` — `curl` sets no quarantine bit, so
...
Drop these into a working directory (default `~/op-signing`):
...
```sh
scripts/ci-macos-upload-secrets.sh ~/op-signing --dry-run   # validate first
scripts/ci-macos-upload-secrets.sh ~/op-signing             # upload all five
gh secret list --repo lee101/op                       # confirm
```
...
APPLE_API_KEY_ID=… APPLE_API_ISSUER_ID=… APPLE_API_KEY=… \
  bash scripts/ci-macos-sign.sh packages/coding-agent/binaries/op-darwin-arm64
```
