...
`op auth-gateway`. It is **not a textual tool-call dialect**: there is no
...
provider credentials—for example, a containerized op talking to a host
...
`baseUrl` MUST identify an `op auth-gateway` (or compatible service). Missing
`baseUrl` fails with:
