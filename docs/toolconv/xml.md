...
OP's `xml` dialect is a generic, prompt-driven in-band protocol. The model writes one `<invoke>` element per tool call directly in assistant text; OP parses those calls and returns one ordered `<tool_response>` block per result in the next user turn. Neither side carries tool-call ids, and result blocks do not carry tool names, so ordering is the correlation mechanism.
...
Select the dialect in `~/.op/agent/config.yml`, project config, or an overlay:
...
When selected, OP removes native structured tools from the provider request, appends the in-band tool catalog and XML guide to the system prompt, converts prior structured calls/results to text, and scans assistant text back into structured tool-call events.

## Tool definitions and prompt injection

OP injects the shared `# Tools` prompt. Available functions appear inside `<tools></tools>` as one compact OpenAI-style function object per line, using each tool's normalized wire schema:
...
| `<tool_calls>…</tool_calls>` | Optional model-emitted wrapper accepted by the guide/scanner; OP's renderer does not add it. |
...
Tool and parameter names are XML-escaped when OP renders attributes. Parameter bodies are not XML-escaped because the format is delimiter-matched, not parsed by an XML DOM. Write `a & b < c`, not `a &amp; b &lt; c`; only a literal `</parameter>` conflicts with the body's close delimiter.
...
Non-string bodies are trimmed for parsing and passed through OP's repair-capable JSON parser. If repair fails, the original body is retained as a string. Empty bodies remain empty strings. A parameter without a usable name is discarded.

## Multiple and parallel calls

OP renders a batch as consecutive invokes:
...
The scanner mints one internal call id per invoke; there is no id in the XML. OP can dispatch the calls as a batch. Results must preserve call order because `<tool_response>` has neither id nor name.

## Tool-result format

OP returns each result in its own block:
...
OP renders preserved thinking as:
...
```ts
import { createInbandScanner } from "@openpaths/ai/dialect";
...
For the generic XML dialect, the first model-authored `<tool_response>` is treated as a fabricated-result boundary. OP preserves calls/text before it and stops projection there. The default `tools.abortOnFabricatedResult: true` aborts provider generation; disabling the setting drains but discards the fabricated continuation.
...
- **Renderer and scanner acceptance differ.** OP renders bare consecutive invokes; the default scanner additionally accepts two wrappers and `antml:` variants.
