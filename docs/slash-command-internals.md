...
- [`src/discovery/op-plugins.ts`](../packages/coding-agent/src/discovery/op-plugins.ts)
...
1. `native` (OP) — priority `100`
2. `op-plugins` (extension packages) — priority `90`
...
Search roots come from `.op` directories:

- project: `<cwd>/.op/commands/*.md`
- user: active profile agent directory `commands/*.md` (`~/.op/agent/commands/*.md` for the default profile; `~/.op/profiles/<name>/agent/commands/*.md` for a named profile)

`getConfigDirs()` returns project first, then user, so **project native commands beat user native commands** when names collide.

## `op-plugins` provider (`op-plugins.ts`)
...
Loads plugin command roots via `listClaudePluginRoots(...)`, which reads `~/.claude/plugins/installed_plugins.json`, `~/.op/plugins/installed_plugins.json`, and the nearest project-scoped registry resolved from cwd. For each root it scans `<pluginRoot>/commands/*.md` (the directory can be remapped by plugin config keys `commands`/`slash-commands`), and command names are prefixed with the plugin name: `<plugin>:<command>`.

Across the three registries, roots are merged by precedence rather than sorted: `--plugin-dir` injected roots come first, then project-scoped entries (which shadow user entries for the same plugin id), then user entries, with the OP registry authoritative over Claude's for the same plugin id. Within each registry, per-plugin entry order from the JSON data is preserved; there is no additional sort step.
