...
That is a working extension. Drop it into `~/.op/agent/extensions/hello.ts` and restart op to see the notification.
...
op loads extension modules from these sources:

1. Native `.op` locations discovered through the capability system:
   - `<cwd>/.op/extensions/`
   - `~/.op/agent/extensions/`
   - legacy extension paths listed in `.op/settings.json#extensions` or `~/.op/agent/settings.json#extensions`
2. Enabled installed plugins under `~/.op/plugins/node_modules` or a project plugin root — including npm, marketplace, and `op plugin link` installs — via their `op.extensions`/`pi.extensions` manifests.
3. Explicit configured paths passed by the CLI (`op --extension ./my-ext.ts`, also `-e`; `--hook` is treated as an alias) and by the `extensions:` setting in config.

The runtime de-duplicates by resolved absolute path — first seen wins.

The user directory is the active profile's agent directory: the default is `~/.op/agent`, while `op --profile <name>` uses `~/.op/profiles/<name>/agent` (and `PI_CODING_AGENT_DIR` overrides it).

When a path points to a directory, op resolves the entry point in this order:

1. `package.json` with `op.extensions` (or legacy `pi.extensions`) field
2. `index.ts`
3. `index.js`

When scanning an `extensions/` directory, op also loads direct `*.ts`/`*.js` files and one-level subdirectories that have `index.ts`, `index.js`, or a manifest.

Extension packages can also bundle sibling capability directories. When a package is loaded through `extensions:` or `--extension`/`-e`, the `op-plugins` provider discovers its `skills/`, `hooks/pre|post/`, `tools/`, `commands/`, `rules/`, `prompts/`, and `.mcp.json`.

## package.json manifest

To package an extension as an installable plugin, add an `op` field to `package.json`:

```json
{
  "name": "my-op-extension",
  "op": {
...
Zod-compatible optype builder; `pi.arktype` and the legacy-compatible
...
| Legacy hook module already exists | **Hook** (`HookAPI` from `@openpaths/coding-agent/extensibility/hooks`) |
...
op writes structured logs under the active state root's `logs/` directory (by default `~/.op/logs/`; debug level is always on, and nothing is written to the console because that would corrupt the TUI). Each filename includes the process ID. Tail today's default-profile logs to see extension load diagnostics:

```
tail -f ~/.op/logs/op.$(date +%F).*.log
```
