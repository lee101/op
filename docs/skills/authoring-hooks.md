...
description: Use when creating a new op hook. Covers HookAPI, event catalog, blocking/overriding tool calls, and context modification.
---
...
export default function myHook(op: HookAPI): void {
  op.on("tool_call", async (event, ctx) => {
...
```ts
op.on("tool_result", async (event, ctx) => {
...
export default function rmRfBlocker(op: HookAPI): void {
  op.on("tool_call", async (event, ctx) => {
...
export default function apiKeyRedactor(op: HookAPI): void {
  op.on("tool_result", async (event) => {
...
export default function contextFilter(op: HookAPI): void {
  op.on("context", async (event) => {
