...
description: Use when creating a new op marketplace. Covers marketplace.json schema, source types, install commands, and publishing.
---
...
The catalog file lives at either `.op-plugin/marketplace.json` or `.claude-plugin/marketplace.json` in the repository root. op prefers the `.op-plugin/` path and falls back to the Claude path; a repository may publish both to expose tool-specific catalogs from a single source tree.
...
my-plugin/
  ...
  package.json                   ← optional version and `op.extensions`
...
```
op plugin marketplace add owner/repo
op plugin install name@marketplace-name
```
