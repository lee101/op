...
An `openpaths` extension that demonstrates `tool_call` blocking. It intercepts `bash` tool calls and returns `{ block: true, reason: "..." }` when the command contains `rm -rf /` with normal whitespace, preventing the tool from executing.
...
```
cp -r . ~/.op/agent/extensions/safety-hook
```

Restart `op`. The hook is active for all sessions.

Or load once:

```
op --extension ./safety-hook
```
