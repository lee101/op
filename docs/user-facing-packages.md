...
- **Include** root docs coverage for package-local CLIs, extension features, dashboards, and benchmark runners that users can run directly or through `op`.
...
- Feature: self-hosted service that receives GitHub webhooks for allowlisted repositories, classifies issues, resumes an `op --mode rpc` session per issue, comments or opens a fix PR, and handles follow-up issue and PR conversations.
...
- Inputs/storage: configuration comes from `python/robomp/.env` and the mounted `~/.op/agent/models.container.yml`; GitHub webhook events feed a SQLite-backed queue. The Compose deployment persists the database, per-issue worktrees, session transcripts, and logs in the `robomp_data` volume under `/data`.
...
- Package: `@openpaths/stats`; bin: `op-stats`; main user path: `op stats`.
- Feature: local observability dashboard for AI usage statistics from session JSONL logs.
- CLI modes: `op stats` starts the dashboard server, opens `http://localhost:3847`, and keeps running; `op stats --port <port>` changes the port; `op stats --summary` prints a console summary; `op stats --json` prints JSON and exits.
- Programmatic API: exports helpers such as `syncAllSessions()` and `getDashboardStats()` for embedding.
- Inputs/storage: reads `~/.op/agent/sessions/`; stores aggregates in `~/.op/stats.db`.
...
### `packages/optype` — schema validation library

Sources: [`packages/optype/README.md`](../packages/optype/README.md), [`packages/optype/package.json`](../packages/optype/package.json), and the repository [optype authoring guide](./optype-guide.md).

- Package: public `@openpaths/optype`; install with `bun add @openpaths/optype`; requires Bun 1.3.14 or newer.
...
- Public surfaces: `@openpaths/optype` for native authoring, `/typebox` and `/zod` for compatibility builders, and `/ark` for the alias-free ArkType compatibility facade.
...
- Package: private `@openpaths/typescript-edit-benchmark`; support library with no standalone bin.
...
- Package: private `@openpaths/metaharness`; bin: `metaharness`.
...
- Package: private `@openpaths/browser-relay`; user command: `op browser-relay`.
- Setup: run `op browser-relay install`, load the unpacked extension from
  `~/.op/browser-relay/extension`, then set `browser.relay` or use `app.relay: true`.
...
- Package: private `@openpaths/collab-web`; production client: <https://my.openpaths.io/>.
...
- Package: public `@openpaths/snapcompact`; install with `bun add @openpaths/snapcompact`; requires
  Bun 1.3.14 or newer.
...
- Runtime constraint: rasterization and PNG encoding require `@openpaths/natives`.
...
- Package: public `@openpaths/mnemopi`; bin: `mnemopi`; requires Bun 1.3.14 or newer. Install globally with `bun add --global @openpaths/mnemopi`, then run `mnemopi <command>`. From a source checkout, `bun packages/mnemopi/src/cli.ts <command>` runs the same entrypoint.
...
- Integration: `mcp` starts the package's MCP server. The standalone CLI operates directly on Mnemopi storage; select `memory.backend: mnemopi` instead when integrating memory into OP sessions, as described in the backend guide.
