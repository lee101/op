...
- `packages/coding-agent/src/session/auth-storage.ts` — re-exports `AuthStorage` from `@openpaths/ai`; API key + OAuth resolution order
  ...
  my-provider:
    ...
    models:
      - id: some-model-id
        ...
        reasoning: false
        input: [text]
        imageInputDecoder: stb # local STB decoder; OP converts WebP before dispatch
        cost:
          input: 0
          output: 0
          cacheRead: 0
...
- `transport`: `pi-native` only. When set, every model under that provider is sent to an `op auth-gateway` compatible `baseUrl` via `POST /v1/pi/stream`; `apiKey` is the gateway bearer.
- `imageInputDecoder`: `stb` only. Set this on a custom model or `modelOverrides` entry when the serving backend uses an STB-compatible image decoder that cannot accept WebP; OP converts attached and historical WebP images before provider dispatch.
...
providers:
  openai:
    ...
    headers:
      X-Team-Key: "!bw get password op-team-key"
```
...
1. Load built-in providers/models from `@openpaths/catalog` (`getBundledProviders` / `getBundledModels`).
...
When `OP_AUTH_BROKER_URL` (or `auth.broker.url`) is set, the local SQLite credential store is replaced by `RemoteAuthCredentialStore`. Layers 3, 4, and 6 above (stored OAuth and API-key credentials) are served from a broker-supplied snapshot whose `refresh` tokens are redacted; expiry triggers `POST /v1/credential/:id/refresh` on the broker rather than a local refresh.

`AuthStorage.setConfigApiKey` lets a `models.yml` `apiKey` win over a broker-resolved OAuth token without overriding a runtime `--api-key`. See [`auth-broker-gateway.md`](./auth-broker-gateway.md) for the full broker / gateway design and env surface (`OP_AUTH_BROKER_URL`, `OP_AUTH_BROKER_TOKEN`, `auth.broker.url`, `auth.broker.token`).
...
`disableStrictTools` is a provider-level flag that applies to all models in the provider. It disables the Anthropic `strict` marker only for tools that OP would otherwise mark strict; it does not change runtime tool argument validation. OP can automatically retry without strict tools after Anthropic reports a strict-grammar-too-large error before the first streamed token, but proxies that reject the `strict` field for other reasons should set this flag explicitly.
...
The built-in vLLM provider can be pointed at a non-default endpoint without declaring a custom discovery type. OP uses vLLM's `/v1/models` metadata and preserves vLLM's `max_model_len` field as the discovered context window.
...
Most model configuration now flows through `models.yml` / `models.yaml` via `ModelRegistry`. Explicit `.json` / `.jsonc` paths remain supported only when passed programmatically to `ModelRegistry`; the default user config prefers `~/.op/agent/models.yml`, then falls back to `~/.op/agent/models.yaml`.
