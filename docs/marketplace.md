...
A **marketplace** is a Git repository (or local directory) containing a catalog file at `.op-plugin/marketplace.json` (preferred) or `.claude-plugin/marketplace.json` (Claude Code-compatible fallback). The catalog lists available plugins with their sources, descriptions, and metadata.

A **plugin** is a directory containing Claude/OP plugin content such as skills, commands, agents, hooks, tools, MCP servers, or LSP servers. Marketplace installs also load extension modules declared by `package.json` `op.extensions`: installation symlinks the cached plugin into the scope's `node_modules` tree and records it in `op-plugins.lock.json`, the same runtime surfaces used by npm-installed and `op plugin link`ed plugins. Plugins are identified by `name@marketplace` (e.g. `code-review@claude-plugins-official`).

**Scopes**: marketplace plugins can be installed at two scopes:

- **user** (default) -- available in all projects, stored in the user plugins data root's `installed_plugins.json` (`~/.op/plugins/installed_plugins.json` by default)
- **project** -- available only in the active project, stored in the nearest project `.op/plugins/installed_plugins.json`
...
On Linux and macOS, `op config init-xdg` creates the XDG data, state, and cache roots; it does not move existing data. Once the relevant roots exist and `XDG_DATA_HOME`, `XDG_STATE_HOME`, and `XDG_CACHE_HOME` are set, new user marketplace/plugin state resolves under `$XDG_DATA_HOME/op` (including `marketplaces.json` and `plugins/`). The `~/.op` paths below are the non-XDG defaults.
...
```
op plugin marketplace add <source>
op plugin marketplace remove <name>
op plugin marketplace update [name]
op plugin marketplace list
op plugin discover [marketplace]
op plugin install [--force] [--scope user|project] name@marketplace
op plugin uninstall [--scope user|project] name@marketplace
op plugin upgrade [--scope user|project] [name@marketplace]
op plugin enable [--scope user|project] name@marketplace
op plugin disable [--scope user|project] name@marketplace
op plugin list
...
Git and local sources must contain a catalog at `.op-plugin/marketplace.json` (preferred) or `.claude-plugin/marketplace.json` (Claude Code-compatible fallback). Direct catalog URLs cache only the JSON catalog; plugins in URL-sourced catalogs cannot use relative string sources like `"./plugins/foo"`.

## Catalog format (marketplace.json)

A marketplace catalog lives at `.op-plugin/marketplace.json` in the repository root. When op is the only intended consumer, prefer this path. To remain Claude Code-compatible (op loads the same shape from either path), publish at `.claude-plugin/marketplace.json` instead — op uses it as a fallback when `.op-plugin/marketplace.json` is absent. A repository may ship both: op reads the `.op-plugin/` copy, Claude Code reads the `.claude-plugin/` copy. Same catalog format either way:
...
- `op plugin upgrade name@marketplace` reinstalls every installed scope when `--scope` is omitted. `/marketplace upgrade name@marketplace`, uninstall, and enable/disable require `--scope user|project` when the plugin exists in both scopes.
...
~/.op/
  ...
  plugins/
    ...
    op-plugins.lock.json         # Runtime enable/feature state
...
<project>/.op/
  plugins/
    ...
    op-plugins.lock.json         # Project runtime enable/feature state
    node_modules/<package>        # Symlink to the cached plugin
```
