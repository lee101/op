...
- [`src/discovery/op-extension-roots.ts`](../packages/coding-agent/src/discovery/op-extension-roots.ts)
...
- `thinking-level` / `thinking` selects the agent's configured effort. When `task.enableEffort` (default `false`) exposes it, a task item's coarse `effort` (`lo`, `med`, `hi`) takes precedence at launch. OP maps that hint to the selected model's lowest, middle, or highest supported effort, then clamps it to `task.maxEffort` (default `max`). The ceiling is carried across retry-fallback model switches. If the selected model has no supported effort at or below the ceiling, the spawn fails; models without a controllable effort surface instead fall back to their normal selector.
...
OP discovers user agents from `~/.op/agent/agents/*.md` and project agents from `.op/agents/*.md`.
...
Set the role mapping in `~/.op/agent/config.yml`:
...
`discoverAgents(cwd, home)` (`src/task/discovery.ts`) merges agents from OP-native roots, OP extension packages, and Claude marketplace plugin roots before appending bundled definitions. Direct cross-harness roots such as `.claude/agents`, `.codex/agents`, and `.gemini/agents` are intentionally skipped — their frontmatter schema is not the OP task-agent contract (`TASK_AGENT_CONFIG_SOURCE = ".op"` filters the native config-dir lists).

### Discovery inputs and precedence

1. Nearest project `.op/agents` dir from `findAllNearestProjectConfigDirs("agents", cwd)` (first `.op` hit only)
2. User `.op/agents` dir from `getConfigDirs("agents", { project: false })` (first `.op` hit only)
3. `<extension-root>/agents` for every enabled OP extension package returned by `listOmpExtensionRoots(...)`, in this order:
   - CLI `--extension` roots
...
The OP extension-package surface is disabled when the `op-plugins` capability provider is disabled. Marketplace roots are excluded from `listOmpExtensionRoots` and enter only through the separately gated Claude-plugin path.
