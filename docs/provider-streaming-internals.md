...
This document explains how token/tool streaming is normalized in `@openpaths/ai`, then propagated through `@openpaths/agent-core` and `coding-agent` session events.
...
## Unified stream contract in `@openpaths/ai`
