...
- `nativesDir`, normally `~/.op/natives`; it uses `$XDG_DATA_HOME/op/natives` only when `$XDG_DATA_HOME/op` exists;
- `versionedDir`: `<nativesDir>/<packageVersion>`;
- legacy compiled-binary directory: `%LOCALAPPDATA%/op` (or `~/AppData/Local/op`) on Windows, `~/.local/bin` elsewhere;
...
1. Every selected filename in `@openpaths/natives-<tag>`.
