...
- JS/TS hook factories discovered through `hookCapability` (for example `.op/hooks/pre/*.ts`) are loaded as extension modules so their `pi.on(...)` handlers bind to the runtime event bus
...
- use the injected Zod-compatible builder `pi.zod`, native optype builder `pi.arktype`, legacy `pi.typebox`, and package exports via `pi.pi`
...
`packages/coding-agent/src/extensibility/hooks/index.ts` and the package subpath `@openpaths/coding-agent/extensibility/hooks` export:
...
The package root (`@openpaths/coding-agent`) does not re-export `HookAPI`; import legacy hook types from the hooks subpath.
