...
- Project directory: `<cwd>/.op/extensions`
- User directory: the active agent directory's `extensions/` (default `~/.op/agent/extensions`)
- Native legacy/settings JSON entries: `<cwd>/.op/settings.json#extensions` and the active agent directory's `settings.json#extensions`

The project root is the native provider's `.op` directory (`SOURCE_PATHS.native.projectDir`), cwd-only; it does not walk ancestors. The user root is the active profile's agent directory via `getAgentDir()`, so under `op --profile <name>` it becomes `~/.op/profiles/<name>/agent/extensions` (and it honors `PI_CODING_AGENT_DIR`). See [Profiles](./config-usage.md#profiles).

Notes:

- Native auto-discovery is currently `.op` based.
...
Plugin extension entries come from package `op.extensions` / `pi.extensions` manifests, including enabled feature entries.
...
- User: the active agent directory's `config.yml` (default `~/.op/agent/config.yml`; with `--profile <name>`, `~/.op/profiles/<name>/agent/config.yml`; `PI_CODING_AGENT_DIR` can override the agent directory)
- Project/native settings capability: `<cwd>/.op/config.yml` and `<cwd>/.op/settings.json`

Native extension-module discovery also reads legacy JSON extension lists from:

- The active agent directory's `settings.json` (default `~/.op/agent/settings.json`)
- `<cwd>/.op/settings.json`
...
```json
{
  "extensions": ["./.op/extensions/my-extra"]
}
```
...
- SDK: when `disableExtensionDiscovery=true`, ambient extension factories are
  ...
  (including package directories with `package.json#op.extensions`).
- CLI: `--no-extensions` follows the same explicit-only contract. Explicit
  ...
  `extensions:` settings and installed OP extension packages are excluded from
  that sibling surface.

This flag governs extension factories and OP extension-package sibling roots;
...
1. `package.json` in that directory with `op.extensions` (or legacy `pi.extensions`) -> use declared entries
2. `index.ts`
3. `index.js`
4. Otherwise scan one level for extension entries:
   - direct `*.ts` / `*.js`
   - subdir `index.ts` / `index.js`
   - subdir `package.json` with `op.extensions` / `pi.extensions`
