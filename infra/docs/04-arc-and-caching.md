...
- **Scale set** (`op-kata` release, ns `arc-runners`) - the `AutoscalingRunnerSet`
...
**Scale set** (`op-kata`), using the runner cache PVC and values file from step 3:
```bash
helm install op-kata \
  --namespace arc-runners --create-namespace \
  --version 0.14.2 \
  -f arc-op-values.yaml \
...
# op-kata  arc-runners   deployed  gha-runner-scale-set-0.14.2             0.14.2
...
# op-kata-<hash>-listener                 1/1   Running
```

---

## 3. Scale-set values (`arc-op-values.yaml`)
...
This is the live `arc-op-values.yaml` verbatim, with only the repo owner/name in
`githubConfigUrl` redacted:

```yaml
githubConfigUrl: "https://github.com/<OWNER>/<REPO>"
githubConfigSecret: arc-github
runnerScaleSetName: op-kata
minRunners: 0
maxRunners: 8
...
template:
  spec:
    ...
    containers:
      - name: runner
        image: op-kata-runner:2026-07-27-072222
...
1. The **listener** (ns `arc-systems`) long-polls the GitHub Actions service for
   jobs whose `runs-on` matches `op-kata`.
...
```bash
kubectl -n arc-runners get autoscalingrunnerset op-kata
...
| --- | --- | --- |
| op-kata runner pods (trusted `push`/main + release) | `grpcs://bazel-remote.bazel-cache.svc.cluster.local:9092` | yes - `ci` credentials injected via the `bazel-remote-ci` secret |
...
- **Secrets** (all maintained by `setup.sh`):
  ...
  - `arc-runners/bazel-remote-ci` - `BAZEL_REMOTE_USER` / `BAZEL_REMOTE_PASSWORD`,
    ...
    ([step 3](#3-scale-set-values-arc-op-valuesyaml); `infra/reload-runner.sh`
    inserts the `envFrom` entry into `arc-op-values.yaml` idempotently on the
    next image reload).
...
On op-kata the credentials come from the injected pod env
...
the runner-cache PVC (`OP_XWIN_CACHE_DIR`).
...
wraps `bun install --frozen-lockfile`. On op-kata, the pod template mounts
...
- `ci.yml` routes every pull-request job to GitHub-hosted runners
  (`runs-on` resolves to `op-kata` only for `push`/main, manual dispatch, and
...
- `bazel-cache/bazel-remote-data` - bazel-remote evicts LRU at `--max_size 90`
  ...
  if hit rates drop.
- `arc-runners/runner-cache` - coarse manual cleanup: scale `op-kata` to zero,
...
# OP_NATIVE_CACHE_DIR from arc-op-values.yaml (+ helm upgrade).
```
...
- **No cluster rights.** Jobs run under `op-kata-gha-rs-no-permission` with no
...
```bash
kubectl -n arc-runners get autoscalingrunnerset op-kata   # min/max/current runners
...
**Verify the caches are being used.** A warm Bazel build on op-kata logs
...
**Resize a job's VM** - edit the `resources` block in `arc-op-values.yaml`
([step 3](#3-scale-set-values-arc-op-valuesyaml); requests = guaranteed VM size,
limits = hotplug ceiling) and roll out:

```bash
helm upgrade op-kata \
...
convenience wrapper, `op-add-repo-runner <OWNER>/<REPO> [label]`, performs exactly
this install.)

**Uninstall** (leaves k3s/Kata in place):

```bash
helm uninstall op-kata -n arc-runners
