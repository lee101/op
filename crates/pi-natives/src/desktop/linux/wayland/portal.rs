...
/// Resolves the `op` state directory (`$XDG_STATE_HOME/op` or
/// `~/.local/state/op`) that holds portal tokens.
fn op_state_dir() -> Option<PathBuf> {
...
mod tests {
	...
	fn removes_orphaned_remote_desktop_token() {
		let dir = std::env::temp_dir().join(format!("op-token-test-{}", std::process::id()));
